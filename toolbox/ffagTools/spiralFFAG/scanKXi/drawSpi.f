...
c dtta0 causes a global rotation of ring and COs
      data dttGlb / 0. /   !!! 1.570796326794896619
c cells to draw (from i to j). Location of cavity
C      data kCDraw, kCCav / 1, 10 , 2 /
      data kCDraw, kCCav / 1, 10 , 999 /
c draw theoretical trajectories at Cell # (1 - nCell, 99 for all)
C      data kTDraw / 99 /
      data kTDraw /  0 /
c draw spiral trajectory 
      logical kSDraw
      data kSDraw / .FALSE. /
c      data kSDraw / .TRUE. /
...
c      dr2 = (r2-r1)/3.d0
      dr2 = 0.2d0 
      if(dr2.le.1.d-4) dr2 = .2d0
      rp1 = r1 - dr2  !  rayon min pole
      rp2 = r2 + dr2 !  rayon max pole
 ...
 1    continue
     ...
     >' m,    drift_xtr = ',2.d0*pi*r2*(1.d0-pf)/nCell,' m'  
      ...
      dtta0 = dttGlb - DltACN
        ...
        write(*,*) 'ikbbibbbbbbbbb ',dtta0, dttGlb, DltACN

C          goto 77

C--------------------------------------------------
Compute spiral EFBs and magnets' footprints. 
Can be plotted using zpop, option Menu/7/20
      do j = 1, kCDraw(2)
        i = j-1
c        plot EFBs
        tta0 = 2.d0*pi * float(i)/float(nCell)  + dtta0
        if(kCDraw(1).le.j .and. j.le.kCDraw(2)) then 
...
c        return yokes
        ...
        tta = tta0 + log(rp1/rRef) / b 
        if(kCDraw(1).le.j .and. j.le.kCDraw(2)) then 
          call ACrcl(0.,0.,ryo1,tta,-Af/2.,Af/2.,npt,lunOut)
          write(lunOut,*) '%'
          call radi(0.,0.,tta-Af/2.,ryo1,rp1,npt,lunOut)
          write(lunOut,*) '%'
          call radi(0.,0.,tta+Af/2.,ryo1,rp1,npt,lunOut)
          write(lunOut,*) '%'
        endif
        tta = tta0 + log(rp2/rRef) / b 
        if(kCDraw(1).le.j .and. j.le.kCDraw(2)) then 
          call ACrcl(0.,0.,ryo2,tta,-Af/2.,Af/2.,npt,lunOut)
          write(lunOut,*) '%'
          call radi(0.,0.,tta-Af/2.,rp2,ryo2,npt,lunOut)
          write(lunOut,*) '%'
          call radi(0.,0.,tta+Af/2.,rp2,ryo2,npt,lunOut)
          write(lunOut,*) '%'
        endif
...
C--------------------------------------------------
Compute gap footprint. 
      dGap1 = .68d0
      dGap2 = .65d0
      do j = kCCav, kCCav, 1
        i = j-1
        tta0 = 2.d0*pi * (float(i) + dGap1)/float(nCell) + dtta0
          call arcSpi(rRef,b,tta0-Af/2.,rp1,rp2,npt,lunOut)
          write(lunOut,*) '%'
        tta0 = 2.d0*pi * (float(i) + dGap2)/float(nCell) + dtta0
          call arcSpi(rRef,b,tta0-Af/2.,rp1,rp2,npt,lunOut)
          write(lunOut,*) '%'
      enddo
      write(lunOut,*) '%'
...
C Arc de cercle ttf de rayon rho centre en O' 
          if(kTDraw.eq.99 .or. kTDraw.eq.j) then
c            call ACrcl(cx,cy,radius,tta,ami ,ama ,npt,lunOut)
          endif
        enddo
      enddo
C--------------------------------------------------
 ...
 78   continue
      if(.not. kSDraw) goto 79


C--------------------------------------------------
      write(*,*) ' Drawing a spiral trajectory, busy... '
      ...
      ddr = 0.d0
      if(nCO.gt.1) ddr = DR / float(nCO-1) 
      bb = dlog(1.d0 + ddr/r1) / (2.d0 * pi)

      do a = 0.d0, nCO * 2. * pi, 2.d0*pi/200.d0
        radius = r1 * exp(bb * a)
        x= radius * cos(a)    
        y= radius * sin(a)    
        write(lunOut,*) x, y
...
C--------------------------------------------------
 ...
 79   continue
      ...
      dttCav =  ttab+tta0 - (tta2+Af )
      write(*,*)
      write(*,*)' Room for cavity : '
      write(*,*)' rp1*dttCav = ',rp1*dttCav,' m,  rp2*dttCav = ',rp2*dttCav,' m'
...
C      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      ...
      subroutine radi(cx,cy,ang,rmi,rma,npt,lun)
      dr = (rma-rmi)/(npt-1.)
      do r = rmi, rma, dr
        x= cx + r * cos(ang)    
        y= cy + r * sin(ang)    
