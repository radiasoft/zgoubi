...
C Retains only traj #1
        if(jo.eq.1) then 
              p0 = sqrt(T0 * (T0 + 2.* am*1.d6))/1.d6
              p1 = sqrt(T1 * (T1 + 2.* am*1.d6))/1.d6
              p2 = sqrt(T2 * (T2 + 2.* am*1.d6))/1.d6
          dj = p1/p0 + (p2/p0-p1/p0)/dble(nCO-1)*jo
C          dj = d(1)
          xj = x(1)
          xpj = xp(1)
        else
          ...
          xj = xjok
          xpj = xpjok
c               write(*,*) '  djnew =', xj,xpj,djnew,
c     >           p0,p1,p2,(p2/p0-p1/p0)/dble(nCO-1),am
c                pause
        endif
...
C           do not write possible subsquent lines of "1's"
     ...
     >                                  IS)) then 
            ...
            P0 = BORO*CL9*Q/QE
c                write(*,*) ' p0 = ', p0, boro, q, qe
c                pause

            if(jo.eq.1 .and. nCO.gt.1) then
C              p0 = sqrt(T0 * (T0 + 2.* am*1.d6))/1.d6
              p1 = sqrt(T1 * (T1 + 2.* am*1.d6))/1.d6
              p2 = sqrt(T2 * (T2 + 2.* am*1.d6))/1.d6
            endif
            djnew = p1/p0 + (p2/p0-p1/p0)/dble(nCO-1)*jo
          else
