...
C----------------------------------------
C Changref for KPOS = 2 with POLARMES
        write(6,*) ' CHANGREF :  RE ALE RS ALS = 0. ',chrfe,' 0. ',chrfs
C----------------------------------------

      pMax = sqrt(TMax * (TMax + 2.d0 * 938.27231e6))
      BRhoMax = pMax/c*1e3
      write(*,*) ' TMax, pMax, BrhoMax : ',  TMax, pMax, BrhoMax 
      z = 0.0001d0
      zp = 0.d0
      dr = .05D0
      r = R0 
      do r = rmin, 2.04D0, dr
        ...
        vv = OMEGA - ttaUp 
        if (r.le.r0) then
          rCO = r * cos(ttaDrft/2.D0) / cos(vv)
        else
          rCO = 0.D0
        endif
        pRel = (r/R0)**(1.D0+AK)
        write(*,fmt='(1p,2g13.5,2g12.4,a,g12.4,a,i1,a)') 
     >    r*100, vv*1000,  z, zp, '  0.  ',pRel,' ''',i,''''
C        write(*,fmt='(1p,6e12.4,a,i2,a)') 
C     >             r,r/R0, ttaUp, vv, rCO,pRel,' ''',i,''''
      enddo
C----------------------------------------
...
C      FUNCTION RTNEWT(XB,YB,D,E,X1,X2,XACC,TTARF)
C      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C      PARAMETER (JMAX=10000)
C      RTNEWT=.5D0*(X1+X2)
C      DO 11 J=1,JMAX
C         temp = RTNEWT
C         CALL FUNCD(RTNEWT,XB,YB,D,E,F,DF,TTARF)
C         DX=F/DF
C         RTNEWT=RTNEWT-DX
CC         IF((X1-RTNEWT)*(RTNEWT-X2).LT.0.)PAUSE 'jumped out of brackets'
C         IF((X1-RTNEWT)*(RTNEWT-X2).LT.0.D0) then
C           RTNEWT = temp
C              return
C         endif
C         IF(ABS(DX).LT.XACC) RETURN 
C 11      CONTINUE
CC      PAUSE 'rtnewt exceeding maximum iterations'
C      RETURN
C      END
