...
C Tunes from MATIX
      ...
      call system('mv periodicParameters.out_cat 
     >             periodicParameters.out_cat_old  ')
      call system('rm -f periodicParameters.out') 
      call system('rm periodicParameters.out_cat') 
...
C Cat output of tunesFromMatrix procedure into scanKXi.out_Qs
          ...
          if(existCO) then
            write(*,*)
            write(*,*) 'Will compute periodic parameters from searchCO',
     >        ' outputs'
            call system('cp zgoubi_searchCO-In.dat temp.dat')
            call system('cp zgoubi_searchCO-Out_periodicParam.dat 
     >                 temp2.dat')
            call system('cp zgoubi_searchCO-Out_MATRIX.dat temp3.dat')
            call system('cp zgoubi_searchCO-Out_periodicParam.dat 
     >               zgoubi_searchCO-In.dat  ')
          call system('~/zgoubi/struct/ffag/tools/spiralFFAG/geneFieldMa
     >p/searchCO')
            call system('mv zgoubi_searchCO-Out.dat zgoubi.dat')
            write(*,*) ' '
            write(*,*) 'Periodic off-momentum closed orbits have been',
     >      ' copied into zgoubi.dat'
            write(*,*) ' '
            call system('mv temp.dat  zgoubi_searchCO-In.dat ')
            call system('mv 
     >         temp2.dat zgoubi_searchCO-Out_periodicParam.dat ')
            call system('mv temp3.dat zgoubi_searchCO-Out_MATRIX.dat')
            write(*,*) 'Will compute periodic parameters from ',
     >       'searchCO outputs'
            write(*,*)
            call system('~/zgoubi/source/zgoubi')

            call system('~/zgoubi/struct/ffag/tools/spiralFFAG/geneField
     >Map/periodicParameters')
            call system(
     >      'cat periodicParameters.out >> periodicParameters.out_cat')
            call system('rm -f periodicParameters.out') 
            write(*,*) ' '
            write(*,*) 'Periodic parameters concatenated in ',
     >        'periodicParameters.out_cat'
            write(*,*) ' '
          endif
