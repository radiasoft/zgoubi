...
c---------------- Hypothesis data : 
      call readat
     >   (lunIn,'geneMap.data', 
     >    nCell, AK, ZTA, pf, R0,T0,G0, akappa, A,Q, T1,T2,nCO,map,
     >    kaseV,AT,DltACN)
...
c      AT = 2. * PI / NCELL
      ...
      AMAX =  AT/2.D0 - ACN
           ...
           write(6,*)  '  AMIN, AMAX, ACN ',amin,amax,acn

        chrfe =  -((AT - TTF )/2.D0 + ACN)
        chrfs =    (AT - TTF )/2.D0 - ACN
        write(6,*) ' Total angle of map AT, ACENT : ',
     >                         AT*r2d,' deg.',ACN*r2d,' deg.'
        write(6,*) ' RE ALE RS ALS =  0. ',chrfe,' 0. ',chrfs

C----------------------------------------
C Generate closed orbit
C and make zgoubi_geneMap-Out.dat file
      OPEN(UNIT=12,FILE='zgoubi_geneMap-Out.dat')
      write(12,fmt='(2(a,F10.4),a)') 
     >'Data generated by geneMap. K=',AK,' xi=',ZTA*R2D,' deg'
      write(12,*) '''OBJET'''      
      write(12,*) Brho0*1000.,'     ',T0/1.d6,' (MeV)'
      write(12,*) '2'
      write(12,*) nCO,'   1'
      if(kaseV.eq.1) then 
        z = 1.d-4 
...
c      write(12,*) txt120
      ...
      yc = (rmax+rmin)/2. *100.
      zl = 2.* G0cm
      write(12,fmt='(3(a,f9.4),a)')  ' 1 ',yl,' ',zl,' ',yc,'  0.'
      txt120 = '''FAISCEAU'''
      write(12,*) txt120
C      stepM = G0cm/300.d0 
      stepM = G0cm/20.d0 
