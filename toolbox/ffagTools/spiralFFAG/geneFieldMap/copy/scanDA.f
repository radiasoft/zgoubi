      ...
      data first / .true. /
      data lunIn, lunData, lunLat / 7, 13, 15 / 

      CHARACTER  HMS*8, DMY*9

      write(6,*) 
      write(6,*) '--------------------------------------------'
      write(6,*) ' scanDA procedure now launched...' 

      call system('cat scanDA.out_COs >> scanDA.out_COs_old ; 
     >      rm -f  scanDA.out_COs ')
...
C Scratch old scanDA.out
      inquire(file='scanDA.out',EXIST=EXS)
      if (exs) call system('cat scanDA.out >> scanDA.out_old ; 
     >                    rm -f scanDA.out')
...
C          searchCO.temp is a signal liable to be sent by searchCO, means no CO found
     ...
     >                                        zgoubi_searchCO-In.dat')
          call system('~/zgoubi/struct/ffag/tools/spiralFFAG/geneFieldMa
     >p/searchCO')
          call system('cat searchCO.out_COs >> scanDA.out_COs')
          inquire(file='searchCO.temp',exist=exs)
          existCO = .not. exs

C Starting from closed orbits, get DA
          call system('cp -f zgoubi_searchCO-Out.dat 
     >                                         zgoubi_searchDA-In.dat ')

C Search DA
          call system('~/zgoubi/struct/ffag/tools/spiralFFAG/geneFieldMa
     >p/searchDA')

          call system('cat searchDA.out >> scanDA.out')
