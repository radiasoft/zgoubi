C Transform the  orbit duration vs Energy dependence  - as obtained from prior 
C run of zgoubi so to have time on closed orbits, for instance using  - into freq as a function of turn number. 
...
C the prgrm works with MeV
      ...
      dE = dE * 1.d-6
      E1 = E1 * 1.d-6
      E2 = E2 * 1.d-6  * 1.2  !!!! because cubspl merde

      write(6,fmt='(1p,a,5g12.4,I7,2x,g12.4)') 
     > '  Vp, phsD, E1, E2, dE, #turns, K :', 
     >         Vp, phsD, E1, E2, dE, nbTrn, ak
...
C read closed orbit coordinates and duration (tau)
        read(7,*,err=598,end=599) x,xp,z,zp,s,d,tco,let,E
...
c                write(88,fmt='(1p,4e16.8,2x,i5,2X,a)') 
c     >           turn(i), freq(i), tau(i)/nCell, Ekin(i), i,
c     >           'turn, freq, tauCell, Ekin, i   '
 3            continue
            else
              do 5 i = 1, n
...
C        write(*,*) '% ---------------- ',n,nint(turn(1)),int(turn(n))
...
C Print frequency law trn, frq :
     ...
     >   write(8,fmt='(1p,6e20.12,4x,a)') trn, 1.d0/frq/nCell,
     >            phase, stt, ek,
