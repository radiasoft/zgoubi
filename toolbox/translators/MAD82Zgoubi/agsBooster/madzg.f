...
C----- HKIC VKIC SOLE RCOL MATR
       goto (1,2 ,2 ,4,5,6,7,8,9,10,11,12,12,11,11,13,1,1,12) ik
...
c          write(*,*) ' last, name, name(last-1:last) : ',
c     >   last, name, name(last-1:last),xlmag
c            stop

! quadrupole field at .1 m (T)
      b2 = ak1*bro * x10/100.d0               
! sextupole field at .1 m (T)
      b3 = ak2 * bro *(x10/100.d0)**2 / 2.d0

      write(lw,fmt='(E14.6,F9.4,3F13.8,7F4.1)')
...
C err. FM June 2009      write(lw,fmt='(2F14.7,F15.8)') xl,tilt,b 
      ...
      write(lw,fmt='(A)') txfd
      step = ro * sin(ang/2.) * 2.d0 
      write(lw,fmt='(1P,E12.4,A)') dble(nint(step)),' cm  Bend'
