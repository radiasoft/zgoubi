C23456789012345678901234567890123456789012345678901234567890123456789012
      ...
      parameter(lr=9, lw=10, lw2=12, lw3=14)
      parameter (mxl=15000)
      dimension iq(mxl)
...
C                                                alfy bety gamy dy dyp ?
 ...
 88     CONTINUE
        ...
        notth = notth + 1
        WRITE(*,100) KLEY,notth
        WRITE(lw3,100) KLEY,notth
 100    FORMAT(/,9X,'Key ',A,' not translated. Total not trnsltd : ',I4)
        goto 86 
 ...
 87     CONTINUE
        ...
        bro1 = 1.d0
        call lmnt
     >    (lw,bro1,pmom,frf,ik,noel,kley,name,xl,ang,ak1,ak2,ak3,ak4,
...
C      it = it + 1
 ...
 999  continue
      ...
      write(*,*) ' '
      write(*,*) ' ------------------------------------------------'
      write(*,*) ' WARNING : ',nott+notth,' elements not translated'
      write(*,*) '        Check trad.out for "Not translated"'
      write(*,*) ' ------------------------------------------------'
      write(*,*) ' '
...
c      read(*,*,err=1) bro
      ...
      write(lw,fmt='(A)') '1  4 '
      write(lw,fmt='(A)') 'MULTIPOL QUAD '
      write(lw,fmt='(A)') '   -1   ' 
      write(lw,fmt='(1P,E16.8)') BRO
      write(lw,fmt='(A)') '1  '
      write(lw,fmt='(A)') 'MULTIPOL SBEN SEXT'
      write(lw,fmt='(A)') '   -1   ' 
      write(lw,fmt='(1P,E16.8)') BRO
      write(lw,fmt='(A)') '1  '
...
C----- HKIC VKIC SOLE RCOL MATR, RFCA
       goto (1,2 ,2 ,4,5,6,7,8,9,10,11,12,12,11,11,13,1,77,77) ik
 77    continue
       nott = nott + 1
       write(lw,fmt='(4A,1x,I6,T111,I6)')
     > '''MARKER''  ',kley,name,' Not translated : sums up to ',nott,it
       goto 99
