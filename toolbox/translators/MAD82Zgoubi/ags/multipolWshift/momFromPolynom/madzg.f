...
! quadrupole field (T) at .1 m , and shift
        ...
        if    (name(last-1:last) .eq. 'BF') then
          ...
          b2 = bk1(1)* Fac 
          b3 = -bk2(1)* Fac *(x10/100.d0)**2 / 2.d0
          yce = 23.17d0
        elseif(name(last-1:last) .eq. 'CD') then  
          xlmag = 94.d0 * 0.0254d0
          b2 =  bk1(2)* Fac 
          b3 =  -bk2(2)* Fac *(x10/100.d0)**2 / 2.d0
          yce = -24.14d0
        elseif(name(last-1:last) .eq. 'AF') then  
          xlmag = 94.d0 * 0.0254d0
          b2 = bk1(3)* Fac 
          b3 = -bk2(3)* Fac *(x10/100.d0)**2 / 2.d0
          yce = 23.d0
        elseif(name(last-1:last) .eq. 'BD') then  
          xlmag = 79.d0 * 0.0254d0
          b2 = bk1(4)* Fac 
          b3 = -bk2(4)* Fac *(x10/100.d0)**2 / 2.d0
          yce = -24.d0
        elseif(name(last-1:last) .eq. 'CF') then  
          xlmag = 94.d0 * 0.0254d0
          b2 = bk1(5)* Fac 
          b3 = -bk2(5)* Fac *(x10/100.d0)**2 / 2.d0
          yce = 23.d0
        elseif(name(last-1:last) .eq. 'AD') then  
          xlmag = 94.d0 * 0.0254d0
          b2 = bk1(6)* Fac 
          b3 = -bk2(6)* Fac *(x10/100.d0)**2 / 2.d0
          yce = -24.14d0
        endif

! sextupole field at .1 m (T)
C      b3 = - ak2 * Fac *(x10/100.d0)**2 / 2.d0

      write(*,*) xlmag*cm, x10, x0, b2, b3*10.d0
      write(lw,fmt='(E16.8,1X,F8.4,3(1X,F12.8),1X,7F4.1)')
...
C----- QUAD
     ...
     >xl*cm,x10,x0,ak1*Fac,x0,x0,x0,x0,x0,x0,x0,x0
      txt = txffq
      txt=txt//name
      if(frf .eq. 0.) txt = '0. 0. '//txt
      write(lw,fmt='(A)') txt
...
C----- SEXT ***
      ...
      b3 = 10.d0 * ak2 * Fac *(x10/100.)**2 / 2.d0
      write(*,*) ' SEXT         b3 = ',b3,' kG'
...
! sextupole field at .1 m (kG)
      b3 = ak2 * Fac *(x10/100.)**2 / 2.d0  / xl * 10.d0
...
C        it = it - 1
      ...
      write(lw,fmt='(1P,G14.7,G10.4,G14.7)') xl*cm,r0,e1*Fac*t2kg
...
C  K2AD :
      AK2(1)= TKADM3/(p*p*p)+ TKADM2/(p*p)+ TKADM1/p + TKAD0+TKAD1*p 
     >  +TKAD2*p*p+TKAD3*p*p*p+TKAD4*p*p*p*p+TKAD5*p*p*p*p*p  
     >  +TKAD6*p*p*p*p*p*p - TKDBDCOEF*BDOT*LENA/LENA

C  K2AF :
      AK2(2)= TKAFM3/(p*p*p)+ TKAFM2/(p*p)+ TKAFM1/p + TKAF0+TKAF1*p 
     >  +TKAF2*p*p+TKAF3*p*p*p+TKAF4*p*p*p*p+TKAF5*p*p*p*p*p  
     >  +TKAF6*p*p*p*p*p*p - TKFBDCOEF*BDOT*LENA/LENA

C  K2BD :
      AK2(3)= TKBDM3/(p*p*p)+ TKBDM2/(p*p)+ TKBDM1/p + TKBD0+TKBD1*p 
     >  +TKBD2*p*p+TKBD3*p*p*p+TKBD4*p*p*p*p+TKBD5*p*p*p*p*p  
     >  +TKBD6*p*p*p*p*p*p - TKDBDCOEF*BDOT*LENB/LENA

C  K2BF :
      AK2(4)= TKBFM3/(p*p*p)+ TKBFM2/(p*p)+ TKBFM1/p + TKBF0+TKBF1*p 
     >  +TKBF2*p*p+TKBF3*p*p*p+TKBF4*p*p*p*p+TKBF5*p*p*p*p*p  
     >  +TKBF6*p*p*p*p*p*p - TKFBDCOEF*BDOT*LENB/LENA

C  K2CD :
      AK2(5)= TKCDM3/(p*p*p)+ TKCDM2/(p*p)+ TKCDM1/p + TKCD0+TKCD1*p 
     >  +TKCD2*p*p+TKCD3*p*p*p+TKCD4*p*p*p*p+TKCD5*p*p*p*p*p  
     >  +TKCD6*p*p*p*p*p*p - TKDBDCOEF*BDOT*LENC/LENA

C  K2CF :
      AK2(6)= TKCFM3/(p*p*p)+ TKCFM2/(p*p)+ TKCFM1/p + TKCF0+TKCF1*p 
     >  +TKCF2*p*p+TKCF3*p*p*p+TKCF4*p*p*p*p+TKCF5*p*p*p*p*p  
     >  +TKCF6*p*p*p*p*p*p - TKFBDCOEF*BDOT*LENC/LENA
       return
       end
