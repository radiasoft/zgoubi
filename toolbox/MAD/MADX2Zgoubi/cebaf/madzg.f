...
C Get momentum
 32   continue
        read(lr,fmt='(A)',end=85,err=99) txt666
        ok =       strcon(txt666,'@ PC',
     >                                  IS) 
      if(.not. ok) goto 32        

      read(txt666(is+5:666),*) pmom
...
c     >  hkic,vkic
        ...
        call strget(txt666,29,
     >                        nst,stra)
        name = stra(27)
        kley = stra(28)


c        write(*,*) ' nst i, stra ',nst,(i,stra(i),i=1,nst)
c        write(*,*) ' Just read xl, ang, ak1,ak2,ak3 etc. '
c        write(*,*) 'name,kley :  ', name, kley, xl, ang, ak1, ak2
c              read(*,*)
...
C      it = it + 1
 ...
 99   write(*,*) ' error during read of madzg.in file'
      goto 999
 90   write(*,*) ' error during read txt666 from madzg.in'
      goto 999
 91   write(*,*) ' error during read txt40 from madzg.in'
      goto 999
 92   write(*,*) ' error during read txt40 from txt666 from madzg.in'
      goto 999
 ...
 1    continue
     ...
     >',  G.gamma = ',1.79284735d0*gamma
      ...
      write(lw,FMT='(A)') '0.51099892 1.60217653e-19 '//
     >                                        '1.15965218076e-3 0 0' 
...
c      write(lw,fmt='(1P,3(1X,E17.9))') xxl*cm,tilt,fac*b 
