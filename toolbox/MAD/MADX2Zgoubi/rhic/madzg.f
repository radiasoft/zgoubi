C23456789012345678901234567890123456789012345678901234567890123456789012
     ...
     >'SOLE', 'RCOL', 'MATR', 'MONI','TKIC' /

C      data devTot / dpi /
      data devTot / 6.27818487198d0 /
      data ksnk / 1 /
...
c     >                         s,sxl,s-sxl,xl,ds,kley,name,warn
 ...
 85   continue
     ...
     >,-2.d0*aleTot,'    (expected is 2pi = ',dpi,')'
      ...
      write(*,*) ' '
      write(*,*) ' '
      write(*,*) 'If not 2*pi, recompile and '
     >//'re-run with devTot = that very value,  ',-2.d0*aleTot
      write(*,*) '-----------------------------------------------'
      write(*,*) '-----------------------------------------------'
      write(*,*) ' '
...
c     > ' 2   Print out transport coeffs to zgoubi_MATRIX_out' 
      ...
      write(lw,fmt='(1P,E16.8)') BRO
      write(lw,fmt='(A)') '1      '
      write(lw,fmt='(A)') 'MULTIPOL HKIC VKICK'
      write(lw,fmt='(A)') '-1         '
      write(lw,fmt='(1P,E16.8)') BRO
      write(lw,fmt='(A)') '1      '
...
C----- 14   15   16   17   18      
C----- HKIC VKIC SOLE RCOL MATR MONI TKIC
       goto (
...
C        it = it - 1
      ...
      write(lw,fmt='(F12.6,1x,F6.2,1x,2(F16.10,1x),8(F9.4,1x))')
