...
C Search zgoubi.res for TWISS outputs
      iico = 1
      jjco = 1
 61   continue
...
c            write(*,*) ' Path length of traj. # ',I,' is ',F6m,' cm'
 ...
 62   close(lunR)
     ...
     >                   rm -f tunesFromMatrix.out ')
            ...
            ENDIF
      WRITE(LUNW,*) 
     >'% xco, xpco, XNU, ZNU, alfx, betx, alfz, betz, Dx, tof(s), path(s
     >), E-Kin'
      WRITE(LUNW2,*) 'E_kin(eV) xco(m) xpco(rad) TOF(s) Qx Qy'
...
C     >  Dx(ico),tof(ico),dpco(ico) !!!!, xK, xiDeg
 179    FORMAT(1P,13(E16.8,1X))
!! 179    FORMAT(1P,11G14.6,1X,2G12.4)
