...
C-----------------------------
 ...
 20   continue
        read(lunDa2,*) M,Qz,aJn2im,aJ2in,ymu,zma,vkick
...
C aJn2im is Jn^2/zmax^2
          ...
          epspi = zero
C          dE = 300.d0 * sqrt(aJn2*zma2) / G * am
C          dE = 150.d0 * sqrt(aJn2*zma2) / G * am
          dE = 71.d0 * sqrt(aJn2*zma2) / G * am
...
C aJ2in is Nn^2/epsilon/pi
          ...
          zma2 = 0.d0
C          dE = 300.d0 * sqrt(aJn2*epspi) / G * am
C          dE = 150.d0 * sqrt(aJn2*epspi) / G * am
          dE = 71.d0 * sqrt(aJn2*epspi) / G * am
        else
...
C Complete zgoubi_geneZGDat4Xing-Out.dat from content of zgoubi_geneZGDat4Xing-In.dat
 1      continue
     ...
     >                                        IS)) then 
          ...
          elseif(strcon(txt132,'DVCA02',8,              ! vertical kicker in AGS
     >                                        IS)) then 
            write(lunW,*) txt132
            read(lunR,fmt='(a)',end=10) txt132
            write(lunW,*) txt132
...
C Complete zgoubi_searchCO-In.dat from content of zgoubi_geneZGDat4Xing-Out.dat
 12   continue
...
C          write(lunW,*) 
          write(lunW,fmt='(1p,4(e12.4,1x),e9.1,1x,e14.6,1x,3a1)') 
     > yo/1d2, ypo/1d3, 0.*zo/1d2, 0.*zpo/1d3, xo, do, '''',let1,''''
C     >      yo/1d2, ypo/1d3, zo/1d2, zpo/1d3, xo, do, '''',let1,''''
          read(lunR,fmt='(a)',end=16) txt132   !  '  1  '
          write(lunW,*)  alfy, bety, epsy
          write(lunW,*)  alfz, betz, epsz
          write(lunW,*)  alfl, betl, epsl
          write(lunW,*)  '''FAISTORE'''
          write(lunW,*)  'b_zgoubi.fai  #End '
          write(lunW,*)  '1 '
