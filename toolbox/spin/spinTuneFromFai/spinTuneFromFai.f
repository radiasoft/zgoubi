...
c            SI=SI + spin(1,nt)*sin(nt*val) - spin(2,nt)*cos(nt*val)
            ...
            ff = sqrt((spin(1,nt)-sav(1))**2 + (spin(2,nt)-sav(2))**2 + 
     >                 (spin(3,nt)-sav(3))**2 ) 
            SR=SR + ff*cos(nt*val) 
            SI=SI + ff*sin(nt*val) 
          ENDDO
