...
C RACCAM FFAG
C      data  kaseV, nCO, T1, T2 / 0, 7, 15d6, 180d6 /
C EMMA
      data  kaseV, nCO, T1, T2 / 0, 6, 10d6, 20d6 /
      data am, q /  0.51099892, 1.602176487e-19  /
...
C      call system('ln -sf lattice.data searchCO.data')
     ...
     >               ,' Now default : ', kaseV
        write(*,*)  '  nCO = a few units (nb of co-s).'
     >               ,' Now default : ', nCO
        write(*,*)  '  T1 = initial energy (eV)'
     >               ,' Now default : ', T1
        write(*,*)  '  T2 = final energy (eV)'
     >               ,' Now default : ', T2
c        stop
      endif
C-----------------------------
...
C Read till "KOBJ"
     ...
     >        'Data generated by searchCO '
          ...
          read(lunR,*) kobj
          if(kobj.ne.2)  stop '  OBJET must be type KOBJ=2 '
          write(lunW,*) txt132
C Read till "IMAX IMAXT"
          read(lunR,*) imax, imaxt
          if(imax.gt.nCOmx) stop ' Too many co''s...'
...
C           do not write possible subsquent lines of "1's"
     ...
     >                                           IS)) then 
            ...
            if(jo.eq.1 .and. nCO.gt.1) then
              p1 = sqrt(T1 * (T1 + 2.d0* am*1.d6))/1.d6
              p2 = sqrt(T2 * (T2 + 2.d0* am*1.d6))/1.d6
            endif
            djnew = p1/p0 + (p2-p1)/p0/dble(nCO-1)*dble(jo)
...
C Write all co coordinates (clorb(i,j)) into zgoubi_searchCO-Out.dat
        do j=1,jok
          ...
          write(lunW,fmt='(1p,2e14.6,2e9.1,e9.1,e16.8,4a,f16.6,a)') 
     >    clorb(1,j), clorb(2,j), clorb(3,j), clorb(4,j),
     >    clorb(5,j), clorb(6,j),' ','''',let(1),''' ',Tj/10.,' MeV'
C            write(77,*) clorb(6,j),pj,p0,Tj
        enddo
c        do j=1,nCO-jok
...
C 200 tours pour recherche tunes
 ...
 62   close(lunR)
     ...
     >        e13.5, e16.8, 4a, f16.6, a)') 
C     >        e13.5, e16.8, 4a, f16.6, a, 2e12.4)') 
     >  clorb(1,j), clorb(2,j), clorb(3,j), clorb(4,j),
     >  clorb(5,j), clorb(6,j), clorb(7,j),
     >       ' ','''',let(1),''' ',Tj/10.,' MeV' !,xK,xiDeg
      enddo
      write(33,*) ' '
      close(33)
...
C Write all co coordinates (clorb(i,j)) into zgoubi_searchCO-Out_MATRIX.dat
        ...
        do j=1,jok
          ...
          Tj = sqrt(pj*pj+am*am) - am
          write(lunW,fmt=
     >                '(1p,2e14.6,2e9.1,e9.1,e16.8,4a,f16.6,a)') 
     >    clorb(1,j), clorb(2,j), clorb(3,j), clorb(4,j),
     >     clorb(5,j), clorb(6,j), !!!!!!clorb(7,j),
     >          ' ','''',let(1),''' ',Tj/10.,' MeV'
        enddo
...
C Write all co coordinates (clorb(i,j)) and +/- samples
     ...
     >      clorb(1,j), clorb(2,j), clorb(3,j), clorb(4,j),
     >      clorb(5,j), clorb(6,j)*1.001,' ','''',let(1),''''
          write(lunW,fmt=
     >      '(1p,2e16.8,2e9.1,e9.1,e16.8,4a)') 
     >      clorb(1,j), clorb(2,j), clorb(3,j), clorb(4,j),
     >      clorb(5,j), clorb(6,j)*0.999,' ','''',let(1),''''
        enddo
        do j=1,nCO-jok
          read(lunR,fmt='(a)') txt132
        enddo
...
C          xx = clorb(1,j)+.01
          ...
          write(lunW,fmt=
     >      '(1p,2e16.8,2e9.1,e9.1,e16.8,4a,f16.6,a)') 
     >      xx, clorb(2,j), clorb(3,j), clorb(4,j),
     >      clorb(5,j), clorb(6,j),' ','''',let(1),''' ',Tj/10.,' MeV'
        enddo
        do j=1,nCO-jok
          read(lunR,fmt='(a)') txt132
