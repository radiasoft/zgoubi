...
c      write(*,*) txt590
 ...
 86   continue
        ...
        read(lr,fmt='(a)',end=85,err=99) txt610
        write(*,*) txt610

        read(txt610,fmt='(24e19.0,3e19.0,a)',end=85,err=99)
...
c     >  hkic,vkic
        ...
        name = txt20
        write(*,*) '-----------------------------------'
        write(*,*) kley, '  ', name, xl, ang, ak1, ak2
...
c          read(*,*)
     ...
     >am, ' 1.60217653e-19 ',G,' 0. 0.'
      ...
      write(lw,fmt='(A)') 'MULTIPOL HQF '
      write(lw,fmt='(A)') '-1         '
      write(lw,fmt='(2P,E16.8)') BRO
      write(lw,fmt='(A)') '1 '

      write(lw,fmt='(A)') 'MULTIPOL HQD '
      write(lw,fmt='(A)') '-1         '
      write(lw,fmt='(2P,E16.8)') BRO
      write(lw,fmt='(A)') '1 '


      write(lw,fmt='(A)') 'SOLENOID SOLO '
      write(lw,fmt='(A)') '-1         '
      write(lw,fmt='(2P,E16.8)') BRO
      write(lw,fmt='(A)') '1 '
...
C case SBEN
      ...
      if(b1<0.d0) then
        ...
        write(lw,fmt=txtfrm) '''MULTIPOL''',name,' NEG_B',it
      else
        b11 = b1
        write(lw,fmt=txtfrm) '''MULTIPOL''',name,' '//kley,it
      endif
c      write(lw,fmt=txtfrm) '''MULTIPOL''',name,' '//kley,it
...
C         write(*,*) ak2, bro, b3, x10
     ...
     >xlmag*cm, x10, b11*10.d0, b2*10.d0, b3*10.d0, x0,x0,x0,x0,x0,x0,x0
      ...
      if(kpos.ne.4) then
         ale1 = ale
         if(b1<0.d0) ale1 = -ale
         write(6,fmt='(1P,I2,3(1x,e18.10))')  kpos,xce,yce,ale1
         write(lw,fmt='(1P,I2,3(1x,e18.10))')  kpos,xce,yce,ale1
      else
...
c        else
c          write(lw,fmt=txtfrm) '''DRIFT''',name,' '//kley,it
c          write(lw,fmt='(1P,E14.6)') xl*cm
c        endif
c        goto 99
c      endif

c      write(lw,fmt=txtfrm) '''BEND''',name,' '//kley,it
c      write(lw,fmt='(I1,A)') i0,'  .Bend'
c      ro=xl/ang
c      xxl = 2.d0*ro*sin(ang/2.d0)
c      b=bro/ro*t2kg
c              write(lout,*) ' ********* tilt ',tilt
...
C----- QUAD
     ...
     >xl*cm,x10,x0,ak1/xl*bro,x0,x0,x0,x0,x0,x0,x0,x0
      ...
      write(lw,fmt='(A)') txt
      write(lw,fmt='(A)') txfq
      write(lw,fmt='(F15.9,A)') tilt,' 0. 0. 0. 0. 0. 0. 0. 0. 0.'
...
C----- SEXT ***
      ...
      write(lw,fmt='(F10.4,1x,F7.2,3(1x,F15.9),6(1x,F3.1))')
...
C        it = it - 1
      ...
      write(lw,fmt='(1P,G14.7,G10.4,G14.7)') xl*cm,r0,aksi*bro*t2kg/xl
      write(lw,fmt='(2F10.2)') r0, r0
      write(lw,fmt='(A,2X,A)') '400.  Soleno',name
      write(lw,fmt='(A)') '1 0. 0. 0. '
      goto 99
