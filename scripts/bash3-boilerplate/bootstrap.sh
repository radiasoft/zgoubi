...
source "${BASH3_BOILERPLATE}"/set_environment_and_color.sh         # turn on errexit, nounset, pipefail, default log level
# shellcheck source=./set_magic_variables.sh
source "${BASH3_BOILERPLATE}"/set_magic_variables.sh "$(caller 0)" # set __dir, __file, __filename, __base, __os
# shellcheck source=./define_functions.sh
source "${BASH3_BOILERPLATE}"/define_functions.sh                  # help/usage function and debug/info output functions
# shellcheck source=./parse_command_line.sh
source "${BASH3_BOILERPLATE}"/parse_command_line.sh "${@:-}"       # parse the command line
# shellcheck source=./set_common_switches.sh
source "${BASH3_BOILERPLATE}"/set_common_switches.sh               # provide defaults for -h, -V, and -d
