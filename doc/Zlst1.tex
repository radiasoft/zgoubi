...
\footnotesize
    ...
    {\renewcommand{\arraystretch}{1}
	 ...
	 & & & & & \textsl{OBJET/KOBJ=5} \\
		\multicolumn{1}{|c|}{\textbf{parameters}} &   & 7 & i & $i\ne 8$~: YY-determinant~; i=8~: YZ-det.  & & & & &  \\
...
\textsl{OPTICS}  causes a write out in zgoubi.res of optical functions, following options \textsl{KOPT} and 'label'~: 

~

IF \textsl{KOPT=0}~: Off

IF \textsl{KOPT=1}~: Will transport the optical functions with initial values as specified in OBJET, option 
 \textsl{KOBJ=5.01}. 

IF \textsl{KOPT=11}~: Will deliver the Twiss matrix as computed by identification with the local transport matrix. 
This normally means  such 'label' (see below) that the 
calculation  be performed at the end of the presumably periodic structure described in zgoubi.dat. 


~

An optional argument, 'label',  allows print out after all elements in the zgoubi.dat structure (if 'label' is 
not specified or if label='all'), or at the particular element at location \#NOEL in the structure (if label 
is an existing, first,  \LABEL(NOEL) - 
see section~\ref{sec4.6.8}, page~\pageref{sec4.6.8}, as to  elements' labels). 
...
The AGS main magnet is a combined function dipole with straight axis (curves of constant field are straight lines).  

\noindent The simulation of  \textsl{AGSMM}  works like \textsl{MULTIPOL}, with the following three particularities~: 

\begin{itemize}
  \item the dipole field $B_0$ in \textsl{AGSMM} is drawn for the reference rigidity, $B\rho_{ref}$ so to preserve 
...
using \textsl{OBJET}, \textsl{MCOBJET}, 
     \item in pessence of acceleration using \textsl{CAVITE}, $B\rho_{ref} \equiv BORO(1+D)$, with $D$ the relative momentum 
increase, a quantity that \zgou\ updates at cavity traversal.
  \end{itemize}
...
so-called ``transfer functions'' momentum-dependent polynomials.  
  ...
  \noindent A consequence of the previous two items is that  no field value is required in defining the AGS main magnets in zgoubi data list ``zgoubi.dat''.
%
...
treatment of any type of field (\emph{e.g.}, dipole field  with arbitrary $Z$ elevation 
- the map needs not be a mid-plane map, solenoidal field, etc.). 

\noindent The field map data file has to be be filled with a 
format that fits the  \FORTRAN\ reading sequence (presumably compatible 
...
A flag \textsl{MOD} determines wether Cartesian or Z-axis cylindrical mesh is used. \textsl{MOD} can 
take various values depending also on the map data file formatting. (To be documented - see the \FORTRAN\ 
subroutine FMAPW and its entries FMAPR, FMAPR2.)
...
delivers the beam matrix and betatron phase advances or (periodic structure) Twiss\index{TWISS} matrix and tunes, 
...
\noindent The way  \textsl{KTWISS} works is the following: 

\medskip

- It assumes that the reference particle (\emph{e.g.}, particle \#1 of 11, when using \textsl{OBJET}, \mbox{\KOBJ= 5}) is located on 
the closed orbit. \textsl{This condition has to be satisfied for TWISS to work correctly. }

- A first pass through the structure allows computing the Twiss and beam matrices from the  rays, including the periodic 
dispersions. 

- The periodic dispersions are used to define chromatic closed orbits at $\pm \delta p / p$. A second and a third pass with 
chromatic objects centered respectively  on $\pm \delta p / p$ chromatic orbits will compute the chromatic 
