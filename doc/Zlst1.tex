...
photons in magnetic fields and the ensuing particle energy perturbation, following the 
method described in section~\ref{SRLoss}.  

\textsl{SRLOSS}  must be preceded by \textsl{PARTICUL}\index{PARTICUL} for defining  mass and charge values as they enter in the 
...
\noindent The field in  \textsl{AGSMM}  is the same as in \textsl{MULTIPOL} 
...
\begin{itemize}
  ...
  \item The dipole field $B_0$ is drawn for the reference rigidity, $B\rho_{ref}$ so to preserve 
...
using \textsl{OBJET}, \textsl{MCOBJET}, 
     \item in pessence of acceleration using \textsl{CAVITE}, $B\rho_{ref} \equiv BORO(1+D_{ref})$, with $D_{ref}$ 
...
so-called ``transfer functions'' momentum-dependent polynomials.  

  \item The AGS main dipole has backleg windings, used for instance for injection and extraction orbit bumps. 
Windings turn number and Ampere-turns are part of the data in tthe input data list. The intensity in the windings is  accounted for in the calculation of the transfer function from coil current to magnetic field in \textsl{AGSMM}. 
\end{itemize}

\bigskip

  \noindent Note~: A consequence of  items 2 and 3 is that  no field value is required in defining the AGS main magnets in zgoubi data list ``zgoubi.dat''.
...
The simulation of  \textsl{AGSQUAD}  works like \textsl{MULTIPOL}. 
However some of the AGS quadrupoles  have two superimposed coil circuits, with separate power supplies. 
...
\noindent The field in \textsl{AGSQUAD} is computed using transfer functions from the intensity in the coils to 
the magneitc field,  accounting for non-linearities. 
