...
- or (option \textsl{K=99.1} or \textsl{K=99.2}) back to a particular \LABEL\ - upon request, 

\noindent and then  a new pass starts, and so on untill \textsl{NPASS+1}\index{NPASS} passes have been performed. 

\noindent The last pass normally ends, as the previous ones, at \REBELOTE, however it may be ended, 
upon request (option \textsl{K=99.2}),  at a particular second dedicated \LABEL\ placed at arbitrary location between 
the first, above mentionned, \LABEL\ and \REBELOTE. 
In both cases, following the end of this ``multiple-pass'' procedure, 
the execution  continues from the keyword which follows \REBELOTE. 
