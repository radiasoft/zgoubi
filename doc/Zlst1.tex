...
the post-processor program \zpop, as well as many examples with template input data files (``zgoubi.dat'') and reference output result 
files (``zgoubi.res''). 
...
as involved in these equations are known, sub-products of the integration of the  motion of the 
...
%
They are calculated in a regular way, without use of FFT techniques, namely from 
...
hypothesis of a periodic structure, 
 allows multi-turn tracking with initial conditions at pass number $\IPASS$ 
...
Indeed, \IPtwo $>$ \IPone\ assumes prior filling of \textsl{FNAME} in the course of a run (\emph{e.g.}, multi-turn tracking) 
...
\medskip 
 ...
 \noindent 
The  earlier \textsl{FIT} was drawn from  the matrix transport code BETA~\cite{Biblio10}. 
...
\textsl{REBELOTE} \index{REBELOTE} for successive FIT 
...
\noindent  The optimization process may be stopped by means of a penalty value. \index{fit, penalty|textbf} 
Otherwise, \textsl{FIT} stops after a 
maximum number of iterations on the step size (of the order of 15, see \texttt{mino1.f} subroutine). 
...
expansions  to the  $ \vec  u^{(IO)}_0$ term  in eq.~\ref{eq2-2-4}. Default functioning 
is $IO=4$.
...
to $ \vec  u^{(5)}_0$ Taylor development order~; it may not 
...
\index{REBELOTE}\index{multi-particle}\index{multi-turn\index{multi-turn} tracking}\index{acceleration}
...
with \textsl{REBELOTE\index{REBELOTE}}, for the simulation of multi-turn\index{multi-turn} tracking - possibly including  acceleration
...
$$B\rho_{ref} = \BORO \longrightarrow B\rho_{ref} = \BORO + \delta \Br_s$$ \index{BORO@{\BORO}$\times (1+D_{ref})$}
...
to $ BORO\times (1+D_{ref})$  \index{BORO@{\BORO}$\times (1+D_{ref})$} at each passage in the cavity, with $D_{ref}$ 
...
the simulation of multi-turn\index{multi-turn} tracking with synchrotron acceleration (see section~\ref{sec4.6.4}). 
...
$$B\rho_{ref} = \BORO \longrightarrow B\rho_{ref} = \BORO + \delta \Br_s$$ \index{BORO@{\BORO}$\times (1+D_{ref})$|textbf}
...
$$	A_{kl} = \frac{1 }{ k!l!}\,  \frac{\partial^{ k+l}B_Z }{ \partial \theta^k\partial r^l} $$
...
variable.  If it is set equal to `S', only secondary particles (they are tagged with an 'S') will be   \index{tag, tag character} 
...
These quantities are derived from the 1-turn transport matrix as obtained in the way described above, 
 in the coupled hypothesis, based on the Edwards-Teng method~\cite{Coupling}. \index{coupling|textbf}
\end{itemize}
...
occur is that of multi-turn tracking, where in general strengths have to follow the  
...
Multi-turn\index{multi-turn} tracking  in circular machines can be performed by
...
\subsubsection{Positioning\index{positioning of optical elements|textbf}\index{alignment (mis-) of optical elements |textbf}, 
(Mis-)Alignment\index{misalignment|textbf}, 
...
2 \frac{\BORO}{B1} \sin({\theta_Z}/{2})$. However magnet mis-alignment (alignment errors) are handled in a 
...
Monte Carlo\index{Monte Carlo} multi-particle \index{multi-particle} simulations involving an 
...
\item[$-3$]~:  deviation happened to exceed $\pi / 2 $ in an optical element not design to allow that
