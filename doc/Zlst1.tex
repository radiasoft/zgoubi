...
The previous release of the Zgoubi Users' Guide as a Lab. report dates from 1997, making the present one  the last in 
a series of~five~\cite{Biblio1}-\cite{Biblio2c}. 
...
spin manipulations  and other synchrotron radiation damping receipes. 


\bigskip

\noindent The code has been installed and made fully, and freely, available on SourceForge with the collaboration of J.~S.~Berg, 
on Sep 17, 2007~\cite{SFZ}. The SourceForge package evolves and is maintained at the 
pace of on-going project design studies. 
It includes the sources, 
the postprocessor progam \zpop, as well as many examples with template input data files (``zgoubi.dat'') and reference output result 
files (``zgoubi.res''). 


\bigskip

\noindent A series of auxiliary computing tools have been developped in addition, 
...
In addition ``python'' interfaces are being developped by several users,  possibly made available on web 
by their authors. 
...
\addcontentsline{toc}{section}{\numberline{}INTRODUCTION TO THE 4th  EDITION (1997), EXCERPT}
...
synchrotron radiation  based diagnostic miniwigler. In the mean 
...
\begin{equation}
 T(M_1) \approx   T(M_0) +  T' (M_0)\, \Delta s+ T'' (M_0)\,\frac{\Delta s^2}{2} + 
     T''' (M_0)\,\frac{\Delta s^3}{3!} + T^{\quaprime} (M_0)\,\frac{\Delta s^4}{4!} 	\label{eq2-2-Time}
\end{equation}
...
derivatives with respect to $ X $ or $ Y$ may be deriveded  from analytical
...
\noindent Accomodated with Maxwell's equations, this results 
...
3-D extrapolation of these off the median palne is drawn from Taylor expansions and Maxwell's equations. 
...
\noindent  and similarly for $ \vec  E $ with opposit sign for the gradients. 
...
\hangcaption[Fig3]{\label{fig3}Mesh in the $ (X,Y) $ plane. The grid is centered 
...
\noindent$ q$, $m$, $c$, $\gamma$ and $ G $ are respectively the charge, 
mass, speed of light, Lorentz relativistic factor  and anomalous magnetic moment of the 
...
as for the Lorentz equation, page~\pageref{eq2-2-2}. Let $ b=\parallel\vec  b\parallel $, $v=||\vec v||$, $ \vec v= v \vec u$, 
...
$ \vec  S^{\,\prime} = \dfrac{d\vec  S }{ ds} = \dfrac{1 }{ v}\dfrac{d\vec  S }{ dt} $ 
 the derivative of the spin with respect to the path. 
...
$ \vec B_{\paral} = \dfrac{\vec  b_{\paral} }{ \Br} $, $ \vec  E= \dfrac{\vec  e }{ \Br} $,  
 ...
 \begin{equation}
   (B\rho)' \, \vec S +   (B\rho) \, S^{\,\prime}  = \vec  S \times  \vec  \omega
 	\label{eq3-4-S}
 \end{equation}
 
\noindent Note that  the precession vector as well may be re-written under a normalized  form 
...
\noindent that will be used in the sequel. 


\noindent This equation is then solved in the same way as the reduced Lorentz equation~(\ref{eq2-2-3}). 
From the values of the magnetic factor $ \vec  \omega (M_0) $ and the spin 
$\vec  S(M_0) $ 
of the particle at position $ M_0 $ on its trajectory, the spin $ \vec  S(M_1)$ 
...
In purely magnetic fields  $ \vec  e=0 $ and 

 \begin{equation}
 \vec \omega \equiv \vec \omega_b = (1+\gamma G)\vec  B + G(1-\gamma )\vec  B_{\paral} 
 \label{eq3-2-B}  
\end{equation}

\noindent thus  eq.~(\ref{eq3-4-S}) reduces to 
\begin{equation}
...
$\vec  S $ at $ M_0 $ are obtained by recursive differentiation
...
%		          +\vec S\times\vec  \omega_b^{\prime\prime\prime}
 ...
 \noindent  where the derivatives $ d^n \vec  \omega_b / ds^n $ are obtained from eq.~(\ref{eq3-3-S}) taken for $\vec b \equiv 0$. 
  
\noindent The last point consists in getting $ \vec  B_{\paral} $ and its derivatives. 
...
as involved in these equations are picked while solving the equation of motion of the 
...
In purely electric fields  $ \vec  b=0, $ and 
\begin{gather}
\vec  \omega \equiv \vec \omega_e = \dfrac{\beta \gamma}{c}(G + \dfrac{1}{1+\gamma}) \, \vec E\times \vec u  \label{eq3-2-E}  
\end{gather}
\noindent thus eq.~(\ref{eq3-4-S}) reduces to 
\begin{equation}
	(\Br )^\prime\vec  S + \Br \, \vec S^{\,\prime} = \vec  S \times \vec \omega_e
	\label{eq3-4-E}
\end{equation} 
\noindent which, by successive differentiations, yields
\begin{equation}
	\begin{array}{ll}
		 (\Br )^{\prime\prime}\vec  S + 2 (\Br)^\prime \vec  S^{\,\prime} 
		     +\Br\vec  S^{\,\prime\prime}  & =  \vec S^{\,\prime}  \times \vec \omega_e + \vec  S \times \vec \omega_e'       \\
		(\Br)^{\prime\prime\prime}\vec  S + 3(\Br)^{\prime\prime}\vec  S^{\,\prime}
		      + 3(\Br)^\prime \vec  S^{\,\prime\prime} 
...
expansion~(eq.~\ref{eq3-5-S}).  
 The derivatives $ (\Br)^{(n)} =   d^n(\Br ) / ds^n $  above are obtained from the integration of the 
Lorentz equation, eq.~(\ref{eq2-2-15}). The derivatives of $\omega_e$ (eq.~\ref{eq3-4-E}) are obtained 
...
\noindent the interest being that the $\left( \dfrac{1}{\beta} \right)^{(n)}$ are already known from the particle dynamics, eq.~(\ref{eq2-2-17}), 
...
can be split into independent, magnetic and electric parts, namely
$$  \omega = \vec \omega_b + \vec \omega_e , ~ ~ \omega^{\,\prime} = \vec \omega_b^{\,\prime} + \vec \omega_e^{\,\prime}  , ~ ~ ~ \textrm{~ ~  ~ etc.}$$
 As a consequnce, 
...
\begin{equation}
 ...
 &  \vec  S \times \vec \omega_e  
 & + \vec  S \times \vec \omega_e   - (\Br )^\prime\vec  S \\
    	 \multicolumn{3}{l}{\Br\vec  S^{\,\prime\prime} =}  \\
 &    \vec  S^{\,\prime}  \times \vec \omega_b + \vec  S \times \vec \omega_b^{\,\prime} 
 &  +    \vec  S^{\,\prime}  \times \vec \omega_e + \vec  S \times \vec \omega_e^{\,\prime} 
          -\left(  (\Br )^{\prime\prime}\vec  S + 2 (\Br)^\prime \vec  S^{\,\prime} \right)   \\
...
\vec  S^{\,\prime\prime}  \times \vec \omega_b + 2\vec  S^{\,\prime} \times \vec \omega_b^{\,\prime}   + \vec S \times \vec \omega_b^{\,\prime\prime} 
              }_{\textrm{Magnetic field component, eq.~(\ref{eq3-6-3-S})}}  
&   \underbrace{ 
     + \vec  S^{\,\prime\prime}  \times \vec \omega_e + 2\vec  S^{\,\prime} \times \vec \omega_e^{\,\prime}   + \vec S \times \vec \omega_e^{\,\prime\prime}    
    -\left(  (\Br)^{\prime\prime\prime}\vec  S + 3(\Br)^{\prime\prime}\vec  S^{\,\prime}
		      + 3(\Br)^\prime \vec  S^{\,\prime\prime}  \right) 
...
\zgoubi\ provides the  simulation of two distinct types of synchrotron radiation (SR) related effects 
 namely, on the one hand the 
energy loss by stochastic emission of photon and the ensuing perturbation 
on particle dynamics  and, on the other hand the 
...
Most of the content in the present section is drawn from Ref.~\protect\cite{FMSEA-00-01}\label{SRLoss}.
...
\noindent The effects on the dynamic of the emitting particle 
...
Particle position is supposed not to  change upon emission of a photon. These calculations 
 ...
 centimers  to tens of centimers steps in smoothly varying magnetic fields (a quantity to 
be determined before any simulation, from convergence trials). 

\medskip 


\noindent Main aspects of the method are developped in the following. 
...
(\ie, the probability of emission of a photon with  energy in  $[0,\epsilon]$) writes 

\begin{equation}
\label{EqSK53}
\mathcal{P}(\epsilon/\epsilon_c) = \frac{3}{5\pi} \int_0^{\epsilon/\epsilon_c} K_{5/3}(x) dx
\end{equation}
\begin{wrapfigure}{r}{9cm}
  \begin{center} 
\vspace{-2mm}
\includegraphics*[bbllx=20,bblly=100,bburx=550,bbury=450,width=6cm]{G.ps}
\vspace{-0mm}
...
- energy loss per particle $\Delta E(eV)= \frac{2}{3}r_0 c \gamma^3 B(T) \Delta \theta$,   with $B$ 
...
magnet length, and from the reference rigidity \BORO\ (as defined with, \eg, \textsl{OBJET}) \\
-  critical energy $\epsilon_c(eV)=\frac{3 \gamma^3 c}{2 \rho} \frac{\bar{h}}{e}$, with 
$\rho =$\BORO$/B$ \\
- average energy of the photons radiated $<\epsilon> = \frac{8}{15 \sqrt{3}} \epsilon_c$, \\ 
...
is made available in order to allow inspection, or easier comparison with other codes, for instance. 



\subsection{Spectral-Angular Radiated Densities}\label{secSRSpec}

Most of the content in the present section is drawn from Refs.~\protect\cite{FMSL/94-22,FMLPYellow}.
...
acceleration, as shown in Fig.~\ref{newfig5}, this is developped in section~\ref{secN4.1}. 
These ingredients also allow calculating the spectral-angular densities radiated by particles in magnetic 
fields\footnote{These calculations have been installed in the 
post-processor  \textbf{zpop}. \index{zpop}},   this is developped in section~\ref{secN4.2}. 
...
particles, must be the first procedure in the  \zgoubi\ inpuet data file, zgoubi.dat.  
...
- Non-random object, with various geometries~: 6-D window, grids, ellipses, etc.

- Monte Carlo distribution, with various geometries~: 6-D window, grids, ellipses, etc.

 \bigskip

\noindent A recurrent quantity appearing in these procedures is \IMAX, the  number of particules to be ray-traced. 
...
hypothesis of a periodic structure, 
 allows multiturn tracking with initial conditions at pass number $\IPASS$ 
...
$ IY  \ast  IT  \ast  IZ \ast   IP \ast IX \ast  ID $ particles with  initial coordinates 
...
\noindent\textbf{KOBJ = 1.01}: Same as \KOBJ = 2    except for the units, meter and radian in that case. 
...
\noindent \IPone, \IPtwo, \IPStep\  cause the code to read coordinates belonging in the sole pass  
 \IPone\ through  \IPtwo,  step \IPStep. 
Indeed, \IPtwo $>$ \IPone\ assumes prior filling of \textsl{FNAME} in the course of a run (\emph{e.g.}, multiturn tracking) 
...
\noindent\textbf{KOBJ = 5.NN:} Like \textsl{KOBJ = 5}, except for  $NN = 00 - 99$ references needed now
(thus \textsl{NN} additional input data lines), rather than just one. 
Zgoubi will generate  \textsl{NN} sets of 11 particles with initial  coordinates in each set taken \wrt\ one 
of the  \textsl{NN} reference. 

\medskip 

\noindent A subsequent use of \textsl{MATRIX} for instance, would then cause the computation of \textsl{NN} transport matrices. 
...
\noindent (idem for the ($Z, P$) or ($X, D$) planes). 
...
\noindent Possible use of \textsl{PARTICUL\index{PARTICUL}} will have no effect~: it will not change the mass
 and charge assumptiuons as set  by \textsl{OBJETA}.
...
- \textsl{SPNTRK\index{SPNTRK}}~:  switch-on spin tracking, 
 ...
 - \textsl{GETFITVAL\index{GETFITVAL}}~: get parameter values as saved by earlier run of \textsl{FIT[2]}, 
  
\medskip 

\noindent  some may appear further down in the struture (in zgoubi.dat), for instance 

- \textsl{MCDESINT\index{MCDESINT}}~: switch-on in-flight decay, could be after a target, 

- \REBELOTE\index{REBELOTE}~: for multi-turn tracking, including  an extraction line section for instance, 
...
- \REBELOTE\index{REBELOTE}~: for tracking more than \textsl{IMAX}  particles 
(the maximum value for \IMAX\ is defined in the include file \texttt{'MAXTRA.H'}).
...
\noindent  the number of files to be translated [\texttt{READ} format, a single digit integer, optional],  
...
\noindent They are compatible with the use of (\emph{i.e.}, can be encompassed in)  \textsl{REBELOTE} \index{REBELOTE} for successive FIT 
trials using various sets of parameters (option $K=22$ in \textsl{REBELOTE}). 

\noindent \textsl{FIT2} has been implemented  recently~\cite{NedlerMead}>  
The  earlier \textsl{FIT} was drawn from  the matrix transport code BETA~\cite{Biblio10}. One or the other may converge 
faster, or and may have some advantages/disadvantages,  depending on the problem. 

\noindent Any physical parameter of any element (\emph{i.e.},~keyword) may 
be varied. Examples of available constraints  are, amongst others~:  any of the $6 \times 6$
...
\end{pmatrix}
$$
\noindent and any trajectory coordinates $ F(J,I) $ as defined in \textsl{OBJET} 
($ I $  =  particle number, $ J $  = coordinate number = 1 to 6 for 
respectively $ D$, $Y$, $T$, $Z$,$ P $ or $ S= $path length)~;
spin coordinates~;
transmission efficiency of an optical channel. 
A full list of the constraints available is given in the table~page~\pageref{TabFIT}. 

\noindent Tunes $\nu_{Y,Z}$ and  periodic betatron functions $\beta_{Y,Z}, \alpha_{Y,Z}, \gamma_{Y,Z}$ are adjustable 
as well~; they are defined by identification of the transfer matrix  of the full optical structure,  
...
\noindent The first input data in \textsl{FIT\index{FIT}} is the number of variables
...
A simple way to get $ IR $ once the zgoubi.dat file has been built, is to do a preliminary run~: \zgou\ will then
...
\noindent All the data that follow a keyword are numbered -  except for \textsl{SCALING}. 

\noindent In most of the keywords, the numbering follows the principle hereafter~: 
\begin{center}
...
in the case of \textsl{DIPOLE-M} (the left column below represents the input data, the 
right one the corresponding numbering to be used for the \textsl{FIT[2]\index{FIT}}\index{FIT2} procedure)~: 
...
are skipped, this is illustrated in the example hereafter which covers all possible working modes of \textsl{SCALING} 
 (all details regarding the numbering can be found in the \FORTRAN\ routine \texttt{rscal.f})~: 
...
MULTIPOL COH1  \\
1.10    &&           &&     No numbering with 1.* type of option  \\
...
\paragraph{Variation range  \index{fit, variable range}} 
...
\paragraph{\textit{CONSTRAINTS}} \index{constraint (FIT, FIT2)|textbf} \index{fit, constraint}

\noindent The next input data in \textsl{FIT} is the number of constraints,
\textsl{\NC}. Each constraint is defined by the following list of data~: 
...
%  \newlength{\LL}
\settowidth{\LL}{\textbf{Beam matrix\ }}  \index{fit, constraint}
{\footnotesize
	\begin{center}
    {\renewcommand{\arraystretch}{1}
...
\noindent  {\it IC=3~: } If $1<I<\IMAX$ then the value of coordinate type $J$ ($J=1,6$ for respectively 
 $D,~Y,~T,~Z,~P,~S$) of particle number $I$ ($1<I<\IMAX$) is constrained.  However $I$ can take 
special meaning, sa follows. 
...
\noindent  {\it IC=3.2~: } Absolute value of the sum of the local and initial $J$-coordinate of particle $I$.

\noindent  {\it IC=3.3~: } Minimum (\NP=1) or maximum value (\NP=2) of the local $J$-coordinate of particle $I$.
...
(for respectively $Y,~Z,~D$) phase-space emittance value. Then, \NP=1 and followed by the emittance value. 
...
\paragraph{\textit{THE FITTING METHODS \protect\cite{Biblio10,NedlerMead}}}    %%%%[10]
...
\noindent The \textsl{FIT2} procedure is based on the  Nedler-Mead method, it has various specificities, 
details can be found in Ref.~\cite{NedlerMead}.


\medskip

\noindent  The optimization process may be stopped by means of a penalty value. Otherwise, \textsl{FIT} stops after a 
maximum number of iterations on the step size (of the order of 15, see \texttt{mino1.f} routine). 
\textsl{FIT2} stops when a lower step size value is reached. 
...
\subsubsection*{GETFITVAL~: \GETFITVALTitl} \label{GETFITVAL}\index{GETFITVAL|textbf}
\medskip
...
That file is expected to contain a copy-paste of the data under the \textsl{FIT} procedure as 
...
\noindent Lines beginning with '\#' are commented and not taken into account, in this procedure. 
...
(\emph{e.g.}, use of \textsl{HISTO\index{HISTO}}) the daughter  particle~2 
...
the parent  particle  is  ``resurected `` with its original coordinates 
...
to $ \vec  u^{(5)}_0$ Taylor developement order~; it may not 
...
so to avoid pushing Taylor expansions as high as  $ \vec  u^{(5)}_0$. 
...
\textsl{PARTICUL} only needs  be introduced (normally close to the top of the input data file, zgoubi.dat) 
...
\noindent
If \textsl{PARTICUL} is ommited, which is in general the case when ray-tracing ions
in purely magnetic optical assemblies, then \zgoubi\ only knows the rigidity 
(from \textsl{[MC]OBJET}) and will thus proceed quietly,  ignoring such quantities as the time of flight. 
...
the first above mentionned \LABEL, and \REBELOTE. 
...
must begin with a new object definition (using \textsl{MCOBJET\index{MCOBJET}, OBJET\index{OBJET}}). 
Nevertheless, when calling upon certain keywords, flags, 
...
\begin{tabular}{>{\sl}l!{~:}p{13cm}}
  CHAMBR 
       & number of stopped particles\index{stopped particles} reset to 0~;  \textsl{CHAMBR} option  switched  off \\
  COLLIMA 
       & number of stopped particles\index{stopped particles} reset to 0\\
  HISTO 
       &  Histograms are emptied\\
  INTEG 
       & number of particles out of field map boundaries reset to 0 \\
   MCDESINT 
       & Decay in flight option switched off, counter reset \\
  PICKUPS & 
               Pick-up signal calculation switched off\\
  SCALING 
...
\noindent An example of data formatting for the simualtion of an acceleration cycle in a circular 
...
$\bullet ~ NT = -1$~: this is convenient for synchrotron acceleration. In this case 
...
$\bullet ~ NT = 1.10$~: allows taking the scaling law from an external data file, as in the following example~:
...
\subsubsection*{AGSMM~:  \AGSMMTitl} \label{AGSMM}\index{AGSMM|textbf}
\medskip 
...
\begin{itemize}
  ...
  \item The dipole field $B_0$ is drawn from the reference rigidity, $B\rho_{ref}$, and  preserves
...
using  \textsl{[MC]OBJET}, 
     \item in pessence of acceleration using \textsl{CAVITE}, $B\rho_{ref} $ is changed 
to $ BORO\times (1+D_{ref})$ at each passage in the cavity, with $D_{ref}$ 
the relative synchronous momentum increase, a quantity that \zgou\ updates at cavity traversal.
 ...
 momentum-dependent polynomials, taken from Ref.~\cite{EJBleser}.  
  ...
  \item The AGS main dipole has backleg windings, used for instance for injection and extraction orbit bumps. 
The number of winding turns and the  number of Ampere-turns 
are part of the data in tthe input data list. The intensity in the windings is  accounted for in the conversion 
...
magneitc field that  account for the non-linearity of the magnetic permeability~\cite{MADXAGSModel}. 
...
whose content must fit the following \FORTRAN\ reading sequence (possible FORMAT updates are to be found in \texttt{fmapw.f}).   
...
normalized with \textsl{BNORM} factor prior to ray-tracing, as well  X is normalized with 
 the coefficient  \textsl{XNORM}  (usefull to convert to centimeters, the working units in  \zgoubi). 
...
the closest  nodes of the mesh. Note that such extrapolation process may induce eratic behavior if the distance from the mesh gets 
too large. 
...
A major feature of \textsl{CAVITE} is the following~:
...
optical element have their \textsl{strengths} maintained constant is to 
...
\mbox{K  =  99},  while  R.F.  and optical elements  time dependent functions are simulated by means of
...
or exit of field maps, or to simulate misalignements (see also  \textsl{KPOS} option).  
 \CHANGREF\      can be placed anywhere in a structure. 

\medskip

\noindent  Effects of \CHANGREF\ on spin tracking\index{spin tracking}, 
particle decay and gas-scattering are taken into account,  \textsl{except } 
in the case  of energy loss by synchrotron radiation (\textsl{SRLOSS} keyword).
...
matched ellipse to particle population, so only $\epsilon_{Y,Z}/\pi$ need be specified by the user. 
...
the array \textsl{SORT}  for possible further statistical purposes (\emph{e.g.}~with 
...
\noindent The fringe  modelling for for $G(X)=G_0 \times$fall-off form factor  is addressed under \textsl{QUADRUPO}, p.~\pageref{QUADRUPO}. 
...
\noindent The dimensionning of the magnet is defined by (Fig.~\ref{fig9}, p.~\pageref{fig9})
...
the \textsl{DIPOLES} procedure (``Interpolation method'', page~\pageref{CalcMidPlaneFieldInterpol}). 
...
used, or else to  order zero (derivatives of $g(R)$ are not considered) if the analytic method is used. 
...
\noindent {\it Analytical method~: }\label{AnalMeth}
...
$ \frac{\pi }{2n} $ rotation  ($n= 1, 2, 3, ...$)  (see also \textsl{ELMULT\index{ELMULT}}). $ \vec  B $ and its
...
\noindent In the mirror mode (option  $\MT=11$ for vertical mid-plane  or $\MT12$ 
...
\noindent In the lens mode (option flag $\MT=21$ for vertical mid-plane  or $\MT22$
for horizontal mid-plane) stepwise integration starts
...
Normally, $R1-R\!E$ and $R1-R\!S$ should both  exceed  $3D$ (enough  
 that $V(r<R\!E)$ and $V(r<R\!S)$  have negligible effect in terms of trajectory tails).  
...
order off-axis Taylor series extrapolation (see sections~\ref{sec2.5.2} and~\ref{sec2.4.1}).  
...
3-D Cartesian  mesh field maps representing the EMMA FFAG cell quadrupole doublet. 

\medskip

 \textsl{EMMA}    works much like  \textsl{TOSCA}.  Refer to that keyword, and to the \FORTRAN\ file 
EMMAC, for details. 
...
\subsubsection*{FFAG~: \FFAGTitl~\cite{reportNIMFFAG,reportICFAFFAG}} \label{FFAG} \index{FFAG magnet, radial|textbf}
\medskip
...
\subsubsection*{FFAG-SPI~: \FFAGSPITitl~\cite{reportICFAFFAG,reportNIMFFAGSPI}} \label{FFAG-SPI} \index{FFAG magnet, spiral|textbf}
\medskip
...
treatment of any type of field (\emph{e.g.}, solenoidal field, or dipole field,  at  arbitrary $Z$ elevation 
- the map needs not be a mid-plane map). 

\medskip

\noindent The field map data file has to be be filled with a 
format that fits the  \FORTRAN\ reading sequence below (in principle compatible 
...
derivatives are calculated by a second or fourth degree polynomial interpolation followed 
by a $ Z $ extrapolation (see sections~\ref{sec2.3.3} page~\pageref{sec2.3.3},~\ref{sec2.4.3} page ~\pageref{sec2.4.3}). 

\noindent Interpolation grid is 3*3 for 2nd order (option \textsl{IORDRE = 2\index{IORDRE}}) or 5*5 
...
generated at the distance \textsl{\XL} from the entrance (Fig.~\ref{fig25}).  %%%
\vfill
...
\subsubsection*{SPINR~: \SPINRTitl\  } \label{SPINR} \index{SPINR|textbf} 
...
\mbox{($X$, $Y$)} mesh at a given $Z$ coordinate. $IZ = 1$ for a 2-D~map, and 
...
For a 3-D~map with mid-plane symmetry, then  \textsl{MOD=0} and  $IZ\ge 2$, and thus, the first 
...
\noindent A pair of flags, \textsl{MOD}, \textsl{MOD2}, determine wether Cartesian or Z-axis cylindrical mesh is used. 
In addition to the \textsl{MOD=1, 2} cases above, one can have \textsl{MOD=12} and in that case a single file 
contains the all 3-D field map. 
See  table below and the \FORTRAN\  subroutine FMAPW and its entries FMAPR, FMAPR2, for more details, in particular 
...
{\renewcommand{\arraystretch}{1}
\begin{tabular}{llll}
\bf         MOD   &\bf      MOD2 &   & \multicolumn{1}{c}{\bf \multirow{2}{30mm}{\centering \FORTRAN\ reading format} }\\[1ex]
 \multicolumn{3}{l}{\underline{MOD $\leq$ 19 :    Cartesian mesh}}    \\[0.5ex]
 0 and $IZ=1$ &     none                &  \multirow{2}{80mm}{2-D map, a single  data file for  $B_Z(X,Y)|_{Z=0}$, mid-plane symmetry} \\[3ex]
0  and $IZ>1$ &     none                &  \multirow{2}{80mm}{3-D map, 1+IZ/2 data files of upper half of magnet, one per (X,Y)$|_{0\leq Z\leq Z_{max}}$ plane, mid-plane symmetry} \\[3ex]
        0     &    1, 2, 3              &  As previous case, just a different reading format \\
        1     &  none                   &  \multirow{2}{80mm}{2- or 3-D map, IZ data files, one per (X,Y) plane, no symmetry assumed} \\[3ex]
        1     &    1, 2, 3              &  As previous case, just a different reading format \\
        12    &              0          &  \multirow{2}{80mm}{3-D map, single file, upper half of magnet, symmetry with respect to (X,Y)  mid-plane} \\[3ex]
        12    &              1          &  \multirow{2}{80mm}{3-D map, single file, whole magnet, no symmetry assumed}  \\[3ex]
        12    &              2          &  \multirow{2}{80mm}{3-D map, single file, 1/8th of the magnet,  symmetry wrt. (X,Y), (X,Z), (Y,Z) planes}  \\[3ex]
        3     &     none                &  \multirow{2}{80mm}{AGS main magnet field map, 2-D, mid-plane symmetry assumed} \\[3ex]
 \multicolumn{2}{l}{\underline{MOD $\geq$ 20   : Cylindrical mesh}}   \\[0.5ex]
       20, 21  &                        &  \multirow{2}{80mm}{3-D map, single file, half a magnet, cyl. symmetry with respect to (Y,Z) plane} \\[3ex]
       22, 24  &                        &  \multirow{2}{80mm}{3-D map, single file, half a magnet, cyl. symmetry with respect to (X,Y)  mid-plane} \\[3ex]
\end{tabular}}
...
\textsl{BNORM} in a similar way as in \textsl{CARTEMES\index{CARTEMES}}.  
 As well the  coordinates  X and Y (and Z in the case of a 3-D field map) are normalized by  the
  \textsl{X-[Y-,Z-]NORM} coefficient (usefull to convert to centimeters, the working units in  \zgoubi. 
...
in the subroutine \textsl{impfai.f}, where details and possible updates are to found. 
...
histogrammed.  If it is set equal to `P', then only  parent particles will be histogrammed.  
...
structure, from the \textsl{OBJET} up to the location where \textsl{MATRIX}  is introduced in the structure, or, 
upon option, up to the horizontal focus closest to that location.  
...
delivers the beam matrix and betatron phase advances or (case of periodic structure) periodic beam matrix and tunes, 
chromaticities\index{chromaticity|textbf}  and other global parameters. 
...
\begin{eqnarray}
\Delta \phi_Y =  A\!\tan \frac{R_{12}}{(R_{11}\beta_{Y,\textsl{objet}} - R_{12}\alpha_{Y,\textsl{objet}})}, ~ ~ \Delta \phi_Z =  A\!\tan\frac{R_{34}}{(R_{33}\beta_{Z,\textsl{objet}} - R_{34}\alpha_{Z,\textsl{objet}})} , \\
...
periodic parameters are computed such as chromaticities\index{chromaticity}, beta-function momentum dependence, 
 etc. 

These quantities are derived from the first order perturbed and unperturbed transfer 
matrices as obtained in the way described above, and by identification  $ [R_{ij}] 
= I \cos \mu + J \sin \mu$. 
\end{itemize}
...
the periodic structure.      \index{closed orbit} 
...
This is achieved by giving the storage file a name of the form \textsl{b\_FNAME} 
or \textsl{B\_FNAME}  (\emph{e.g.}, `b\_zgoubi.spn'). The \FORTRAN\ \textsl{WRITE} ouput list 
is the same as in the \textsl{FORMATTED} case above.  
...
\textsl{TWISS}   causes the calculation of 
  transport coefficients and various other parameters, in particular 
periodical quantities  such as tunes, chromaticies\index{chromaticity}, etc. 
...
\noindent \textsl{TWISS} works in a way similar to \textsl{MATRIX}, iterating the \textsl{MATRIX} process whereever necessary, 
...
- A first pass (the only one if \textsl{KTW=1}) through the structure allows computing the periodic beam matrix from the  rays, including the periodic 
dispersions. 
...
\noindent When dealing with maps (\emph{e.g.}, \textsl{CARTEMES\index{CARTEMES}}, \textsl{ELREVOL
...
not follow $ \Br (T)$,  for the simulation of special processes (\emph{e.g.}, 
...
\subsubsection{Positioning\index{positioning of Optical Elements|textbf}\index{alignement (mis-) of optical elements|textbf}, 
(Mis-)Alignement\index{Misalignement|textbf}, 
...
\noindent \textsl{KPOS} $=3 $ option is available for a number of  
 magnets  (\emph{e.g.}, \textsl{BEND}, \textsl{MULTIPOL}, \textsl{AGSMM} \index{AGSMM} (AGS main magnet))~; 
...
2 \frac{\BORO}{B1} \sin({\theta_Z}/{2})$ wherein L = geometrical length, 
...
\noindent \textsl{KPOS} $=4 $ applies to \textsl{AGSMM} (AGS main magnet). \index{AGSMM} 
...
2 \frac{\BORO}{B1} \sin({\theta_Z}/{2}$. However magnet mis-alignement (alignement errors) are handled in a 
specific way, as follows. 

All 6 types of misalignements, namely, X-, Y-, Z-shift, X-, Y-, Z-rotation, can be accounted for, in an arbitrary order. 
They are specified using the ``new style'' \textsl{CHANGREF} method as described in page~\pageref{CHANGREFNew}. 
Longitudinal rotation ``XR'' is taken \wrt\ the logitudinal axis, whereas radial and axial rotations, ``YR'' and ``ZR'', 
 are taken around an axis going through the center of the magnet. 
 Transformations are as follows, see Fig.~\ref{figPitch}~: 

\medskip

{\small $\bullet$} Y-rotation (pitch) by an angle $\varphi$~:  new coordinates (at $M$) as well as path 
...
$Z_{new} = Z_{old} \cos p/\cos (p-\varphi)$, ensuing from $\varphi = \frac{\pi}{2} - p $, and $\tan p = \tan P / \cos T$ (since
...
$ 1+DP=\Br /\BORO $ 
relative rigidity will reach position ($ RM$, $\frac{AT }{ 2} $) in 
