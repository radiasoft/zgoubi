...
\noindent The Introductory section of the Guide deserved updating, since so  much developments have been 
accomplished these last years in the frame of a number of 
...
So did it go 
with the list of optical elements and the compendium of numerical methods, 
so-called ``Glossary of keywords'' list, pp.~\pageref{GOK1A} and~\pageref{GOK1B},
 that has stretched with new 
simulation and computing procedures, ranging from fitting to overlapping magnetic fields capabilities  via 
spin  and other radiation damping tools. 
...
on Sep 17, 2007~\cite{SFZ}. The SourceForge package evolves with the various projects dealt with and is continuously maintained.  
...
\noindent A series of computing tools have been developped in addition, planned to be made available on \zgou\ SourceForge 
site in near future, 
that render the designer's life easier, as, 
 search for closed orbits in periodic machines, computation of optical functions and parameters, 
tune scans, dynamic aperture scans,  gnuplot graphic scripts, etc. , including dedicated ones regarding, \emph{e.g.}, FFAG R/D, 
AGS R/D, spin dynamics. 
In the same spirit, ``python'' interfaces are being developped, some made browsable on web by their authors. 

\bigskip

\noindent  Worth stressing, this manual is intended  to describe the contents of the most
recent version of \zgou. Both the code and its Guide are  far from being  ``finished products'', though. 
...
\section*{INTRODUCTION TO THE 4th  EDITION (1997), EXCERPT}
...
\\
\\
\noindent  An updating was necessary for accompanying the third version of the code which included 
...
\item full account of stochastic photon emission, and its effects on particle dynamics, 
...
\noindent The graphic  interface to \zgou\ (\zpop, Part~D) has also been subject to  extensive  developments, making it 
...
\subsubsection{Extrapolation From Analytical Models of Median Plane Fields} \label{sec2.3.2} %%%%%%  2.3.2.  
...
\noindent A right electric multipole has the same field equations as a 
...
for the calculation of position and velocity of the particle are now defined 
using a 3-D field map, through second degree  polynomial interpolation
...
\subsection{Energy loss and related dynamical effects}\label{secSRLoss}

Most of the content in the present section is drawn from Refs.~\protect\cite{FMSEA-00-01}\label{SRLoss}.
...
\noindent Upon request of SR loss tracking, several optical elements that contain dipole 
magnetic field component (\emph{e.g.}, \textsl{MULTIPOL}) provide a printouot of various 
...
magnet length, and the reference rigidity \BORO (as defined with, \emph{e.g.}, \textsl{OBJET}) \\
...
(\emph{e.g.}, with \textsl{HISTO\index{HISTO}} and \textsl{MCDESINT\index{MCDESINT}}).  
\medskip
...
Indeed, IP2 $>$ IP1 assumes prior filling of \textsl{FNAME} in the course of a run (\emph{e.g.}, multiturn tracking) 
...
\noindent\textbf{KOBJ = 3.03:} As for \textbf{KOBJ=3.01}, except for the format~: 
\begin{alltt}
...
the object definition (\emph{e.g.}~\textsl{SPNTRK\index{SPNTRK}} - spin tracking, 
\textsl{MCDESINT\index{MCDESINT}} -
in-flight decay),  others are normally declared at the end of the data pile (\emph{e.g.} 
...
only supports a limited number of output formatting, \emph{e.g.} from  \textsl{TOSCA}\index{TOSCA} magnet code 
...
\noindent \textsl{FIT} has been implemented  recently~\cite{NedlerMead} and may have some advantages over the original method. 
The  earlier \textsl{FIT2} was drawn from  the matrix transport code BETA~\cite{Biblio10}. One or the other may converge 
...
\paragraph{\textit{CONSTRAINTS}} \index{constraint (FIT, FIT2)|textbf}
...
\noindent  {\it IC=0.1~: } Periodic optical functions~: $\sigma_{11}=\beta_Y, \sigma_{12}=\sigma_{21}=-\alpha_Y, 
...
(\emph{e.g.}, use of \textsl{HISTO\index{HISTO}}), any particle of type~2 (resulting from decay of~1) 
...
- or, upon request (option \textsl{K=99.2}),  execution may exit that pass at a particular second 
    dedicated \LABEL\ placed at arbitrary location between 
...
\noindent \REBELOTE\ can be used for  multi-turn tracking in circular machines (\emph{e.g.}~Synchrotron 
...
with possible extraction, at the last pass, at the location of the second \LABEL.  

\bigskip

\noindent Output prints over \textsl{NPASS+1} runs might result in a
...
\subsubsection{Time varyinf fields} \label{sec4.6.TimeVFields} 
