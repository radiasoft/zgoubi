...
\subsubsection[Extrapolation from  1-D axial field map]%
 ...
 A cylindrically symmetric field (e.g., using \textsl{BREVOL}\index{BREVOL}, \textsl{ELREVOL}\index{ELREVOL}) can be described
...
as the like-order skewed magnetic multipole. Therefore, 
...
\subsection{Energy loss and related dynamical effects~\protect\cite{FMSEA-00-01}\label{SRLoss}}\label{secSRLoss}
...
\begin{figure}[h]
\centering
\includegraphics[width=7cm]{G.eps}
...
- It assumes that the reference particle (e.g., particle \#1 of 11, when using \textsl{OBJET}, \mbox{\KOBJ= 5}) is located on 
...
\noindent \textsl{KPOS} $=3 $ option is available for some 
 magnets  (e.g., \textsl{BEND}, \textsl{MULTIPOL}, \textsl{AGSMM} \index{AGSMM, AGS Main Magnet} 
(AGS main magnet))~; 
