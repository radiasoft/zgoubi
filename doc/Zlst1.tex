...
\CHANGREF\  ``new style''  allows up to 9 successive such elementary transformations, of arbitrary nature, in arbitrary order. 
...
2 \frac{\BORO}{B1} \sin({\theta_Z}/{2}$. However magnet mis-alignement (alignement errors) are handled in a different way, 
as follows. 

All 6 types of misalignements, namely, X-, Y-, Z-shift, X-, Y-, Z-rotation, can be accounted for, in an arbitrary order. 
They are specified using the \textsl{CHANGREF} technic document in page~\pageref{CHANGREFNew}. All three rotations  are 
taken \wrt\ the center of the magnet. 
...
\hangcaption{\label{figPitch}Pitch rotation, using \textsl{KPOS}$=4 $.}
\end{center}
\end{figure}
