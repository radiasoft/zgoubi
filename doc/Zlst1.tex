...
\noindent Note that  the precession vector as well may be re-written under the normalized  form 
...
From the values of the  precession factor $ \vec  \omega (M_0) $ and the spin 
$\vec  S(M_0) $ 
...
Taylor expansion 
 ...
 \begin{equation}
	...
	+ \dfrac{d^4\vec  S }{ ds^5}\, (M_0) \dfrac{\Delta s^5 }{ 5!}
 	\label{eq3-5-S}
 \end{equation}
...
In purely magnetic fields  $ \vec  e=0 $ thus $\vec \omega$ (eq.~\ref{eq3-3-S}) reduces to
...
as involved in these equations are known, subproducts of the integration of the  motion of the 
...
In purely electric fields  $ \vec  b=0, $ thus $\vec \omega$ (eq.~\ref{eq3-3-S}) reduces to
\begin{gather}
...
expansion~(eq.~\ref{eq3-5-S}).  
 The derivatives $ (\Br)^{(n)} =   d^n(\Br ) / ds^n $  above are a subproduct of the integration of the 
...
\noindent The interest in that formulation is that the $\left( \dfrac{1}{\beta} \right)^{(n)}$ are already known from the particle dynamics, eq.~(\ref{eq2-2-17}), 
...
\begin{equation}
 ...
 & \hspace{-8mm} \vec  S \times \vec \omega_e  
 & + \vec  S \times \vec \omega_e   - (\Br )^\prime\vec  S \\[1ex]
 ...
 &  +    \vec  S^{\,\prime}  \times \vec \omega_e + \vec  S \times \vec \omega_e^{\,\prime} 
          -\left(  (\Br )^{\prime\prime}\vec  S + 2 (\Br)^\prime \vec  S^{\,\prime} \right)   \\[1ex]
   	 \multicolumn{3}{l}{\Br\vec  S^{\prime\prime\prime} =}  \\
  & \hspace{-8mm} \underbrace{\large
...
\noindent 2. It may happen that some optical elements won't scale, for source code developement reasons. 
...
scaling coefficients. The two maps can be radially displaced independently of 
one another, just like the 
...
\medskip
  ...
  \textsl{MOD}=0 : a single map, which is superimposition of QF and QD ones, is read for tracking. 

  \textsl{MOD}=1 : a single map, \textsl{interpolated} from  QF[$x_F$] and QD[$x_D$] ones, is read. 

  \textsl{MOD}=22 : a single map, superimposition of QF and QD ones, is read. 

  \textsl{MOD}=24 : field at particle is interpolated from a (QF,QD) pair of maps, closest to  current $(x_F,x_D)$ 

  value, taken from of set of (QF,QD) pairs registered in file FNAME. 


\medskip

\noindent The parameters that move/position the maps, as $(x_F,x_D)$, 
                are accessible from the FIT, allowing to  ajust the cell tunes. 
...
automatic changes of frame, using the KPOS command (see section~\ref{sec4.6.2}, page~\pageref{sec4.6.2}). 
\index{positioning of optical elements} }
\end{figure}
...
extended range of coil geometries (legnth, radius) 
...
0  and $IZ>1$ &     none                &  \multirow{2}{80mm}{3-D map, 1+IZ/2 data files of upper half of magnet, one per (X,Y)$|_{0\leq Z\leq Z_{max}}$ plane, mid-plane symmetry} \\[5ex]
...
\textsl{UNDULATOR} magnet. UNDER DEVELOPEMENT. 
...
variable.  If it is set equal to `S', only secondary particles (they are tagged with an 'S') will be 
...
\begin{eqnarray}
\Delta \phi_Y =  A\!\tan \frac{R_{12}}{(R_{11}\beta_{Y,\textsl{objet}} - R_{12}\alpha_{Y,\textsl{objet}})}, ~ ~ \Delta \phi_Z =  A\!\tan\frac{R_{34}}{(R_{33}\beta_{Z,\textsl{objet}} - R_{34}\alpha_{Z,\textsl{objet}})} , \\[3ex]
...
\noindent \textsl{PICKUPS} computes the beam centroid location, from average value of particle 
...
see \FORTRAN\ routine \texttt{pltdat.f}. 
...
periodical quantities  such as tunes, chromaticies\index{chromaticity}, etc. 
 \textsl{TWISS} is normally placed at the end of the structure~; it will cause a series 
...
- The periodic dispersions so obtained are used to define chromatic closed orbits at $\pm \delta p / p$. A second and a third pass 
...
occur is that of multiturn tracking, where in general strengths have to follow the  
...
In all optical elements, an option $ \IL $\index{IL@{\IL}} is available.  It
...
\noindent When dealing with field  maps (\emph{e.g.}, \textsl{CARTEMES\index{CARTEMES}}, \textsl{ELREVOL
...
\item[-] the R.F.\ frequency of the cavity may be given a timing law $f_{RF}(T)$ by 
...
\noindent Eventually some families of magnets may be given a different timing law 
    for the simulation of special processes (\emph{e.g.}, time varying orbit bimp, 
...
\textsl{KPOS}\index{KPOS|textbf}. \KPOS\ is followed by the positionning parameterss, 
...
\noindent Both incoming and outgoing refernce frames are  tilted w.r.t. the magnet,  
...
Case of Cartesian frame optical element. 
Alignement of a  bend by half the deviation, using  \textsl{KPOS}$=3 $.  
\textsl{$(X_E,Y_E)$} and \textsl{$(X_S, Y_S)$} are respectively the incoming 
 and outgoing  reference frames.
}
%\end{center}
...
\noindent All 6 types of misalignements, namely, X-, Y-, Z-shift, X-, Y-, Z-rotation, can be accounted for, in an arbitrary order. 
