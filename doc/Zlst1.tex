...
\paragraph{\textit{OPTIONS} \index{fit, options|textbf}}

\noindent \textsl{FIT} and \textsl{FIT2} accept various options, as follows. 

\smallskip

\noindent - 'nofinal' option~: \index{fit, options|textbf} \label{nofinal}
...
default in zgoubi.FITVALS.out, or in \textsl{FileName} file if the latter is specified following the `save' instruction.\index{fit, save variables}

\smallskip

\noindent - Penalty~: value of the penalty to be reached, following what the  \textsl{FIT[2]} procedure 
will be considered completed, and thus  stopped. \index{fit, penalty}
...
\noindent - ITER~: maximum allowed number of ``calls to the function'' (namely, (recursive) calls to zgoubi). Once that number is reached, 
 \textsl{FIT[2]} is stopped, regardless of a possible ``penalty'' value. \index{fit, max. number of iterations}
...
Note that, if the `nofinal' option\index{fit, options}  
...
with  \textsl{REBELOTE} thus being used to cause multi-turn tracking, 
  \textsl{PICKUPS} sums up the coordinates, turn after turn.       \index{closed orbit} 
