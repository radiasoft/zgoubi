...
pace of on-going project design studies.   It includes the source files, 
...
\zgoubi\ provides the  simulation of two distinct types of synchrotron radiation (SR) manifestatons 
 namely, on the one hand the particle 
energy loss by stochastic emission of a photon and the ensuing perturbation 
...
Most of the contents of the present section are drawn from Ref.~\protect\cite{FMSEA-00-01}\label{SRLoss}.
...
\noindent That file is expected to contain a copy-paste of the data under the \textsl{FIT} procedure as 
...
rigth before  \textsl{[MC]OBJET} in zgoubi.dat, to allow for its updating. 
...
Nevertheless, when calling upon certain keywords, then, flags, 
counters or other integrating procedures may be involved. 
It may therefore be necessary to reset them. This is the purpose 
of \textsl{RESET\index{RESET}} which normally appears right before the object definition 
...
the simulation of multi-turn\index{multi-turn} tracking with synchrotron or 
...
\noindent\textbf{If $\mathbf{IOPT  =  6}$~:} scaling FFAG \index{FFAG} acceleration.  


\bigskip

\noindent\textbf{If $\mathbf{IOPT  =  7}$~:} EMMA FFAG acceleration, quasi-isochronous acceleration, cyclotron 
\index{cyclotron} acceleration. 
...
\medskip
  ...
  \textsl{MOD}=0 : a single map,  including QF and QD, is read for tracking. 

  \textsl{MOD}=1 : a single map, \textsl{interpolated} from  QF[$Y_F$] and QD[$Y_D$] ones, is built. 
...
\noindent \textsl{FFAG} \index{FFAG} works much like \textsl{DIPOLES} 
...
\noindent \textsl{FFAG-SPI} works much like \textsl{FFAG} \index{FFAG} 
...
\noindent  $\bullet$  \textsl{SPNPRT} can be introduced anywhere in a structure. It produces
a listing (into zgoubi.res) of the initial and actual coordinates and modulus of the spin of the \IMAX\index{IMAX@{\IMAX}}
particles, at the location where it stands, together with their Lorentz factor $\gamma$ and other infos as 
the mean values of the spin components. 

\medskip

\noindent  $\bullet$   \textsl{SPNPRNL} has similar effect to \textsl{SPNPRT}, except that the information is
