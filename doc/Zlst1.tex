...
vector components, precession, etc) are stored in zgoubi.SPNPRT.Out using the keyword '\textsl{SPNPRT}' with label 'PRINT'. 
...
by the fitting procedure. The plot also shows the  helix as it is positioned, off-centered, prior to the fit. 
...
%
 \begin{alignat*}{2}
	 \theta^ \ast 
	       & =   \textrm{acos} (1 - 2R_2) & \qquad & (-\pi   <\theta^\ast \leq \pi ) \\
	\phi 
	       & =   2\pi R_3         & \qquad &  (0<\phi \leq 2\pi ) 
 \end{alignat*}
%
...
\textsl{AUTOREF} positions the new reference frame following 4~options (apart from  $\mathbf{I = 0}$ 
...
\noindent\textbf{If} $\mathbf{I=4}$,  it is equivalent to 

$$\text{\textsl{CHANGREF}} \lbrack 0,<Y>,<T>\rbrack $$
%
 so that the new reference frame is at the position $ (X=0, Y=<Y>) $, 
with $<*>$ denoting the average over the \textsl{IMAX} particles, 
 while all particles have there angle $T \rightarrow T - <T>$. 
...
These data were  stored in zgoubi.impdev.out\index{zgoubi.impdev.out|textbf} during execution, 
