...
Note that, if the `nofinal' option\index{fit procedure!options}  
...
\noindent  Note~:  The initial values  in \textsl{OBJET[KOBJ=5.1]} may be the periodic ones, as obtained, for instance, from a first run using \textsl{MATRIX[IFOC=11]}. 
...
will cause saving of the transported beta functions into 
...
\noindent\textbf{If} $\mathbf{I=4.1}$~: new  beam centroid positionning \textsl{XCE, YCE, ALE, DCE} is provided. 
 The beam is moved by \XCE\ and then centered on \textsl{YCE, ALE}. 
In addition, the beam is centered on a new relative momentum \textsl{DCE}. 
...
\bf IZ & ~ ~ & \bf         MOD   &\bf      .MOD2 &   & \\[1ex]
 \multicolumn{5}{l}{\underline{MOD $\leq$ 19 :    Cartesian mesh}}    \\[1.5ex]
...
&&        3     &     none \textsl{or} .1             &  \multirow{3}{80mm}{AGS main magnet field map, 2-D, mid-plane symmetry assumed. MOD2=1 causes field to be perturbed by $(1+n_1\,dY+n_2\, dY^2+n_3\, dY^3)$ factor.} \\[9ex]
...
$>1$ &&        15    &         .1$-$.6        &  \multirow{2}{80mm}{3-D map data file(s) (up to 6), whole magnet volume each (thus no symmetry assumed). Fields summed up~: at all node, $\vec B = \sum_{i=1}^{i=MOD2} a_i \, \vec B_i $. Each map file contains $B_{X,Y,Z}(X,Y,Z)$ data over  \IZ\ equally $Z$-spaced $(X,Y)$ planes}  \\[14ex]
 \multicolumn{4}{l}{\underline{MOD $\geq$ 20   : Cylindrical mesh}}   \\[1.5ex]
...
In addition, if \textsl{OBJET}, \textsl{KOBJ} = 5.1 is used (hence introducing initial optical function values, 
...
<<<<<<< .mine
\noindent - if  \textsl{IORD} = 1, use \textsl{OBJET}(KOBJ = 5[.N]), 
that generates up to 99*11 sets of initial coordinates. In this case, up to ninety nine matrices 
=======
\noindent - if  \textsl{IORD} = 1, use \textsl{OBJET}(KOBJ = 5[.N, NN=2,99]), 
that generates up to 99*11 sets of initial coordinates. In this case, up to 99 matrices 
>>>>>>> .r499
