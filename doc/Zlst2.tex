...
\\
 ...
 \>$\IC=1,2$ : print field map  \\
  \>$\IL=1,2[\times 10^n],~7$ : print coordinates, fields, etc., along trajectories \>0-2$[\times 10^n]$, 7 \>I\\
        \> in zgoubi.res ($1$),  zgoubi.plt ($2$),  zgoubi.impdev.out ($7$).       \>                   \> \\
...
$ I$             \> 1 : Equivalent to \textsl{CHANGREF} ($XCE=0$, 
 ...
 positioning of the first order focus) \>\>\\
 \\
 ...
 \> (for instance : $I1=$ central trajectory, $I2$  and $I3=$ paraxial trajectories \>\>\\
 \>that intersect at the first order  focus) \>\>\\
 \\
\textbf{If $I=3$}        \>Next record if $I=3$ \>\>\\
 $I1$, $I2$, $I3$      \>Three particle numbers \> 3*(1-\IMAX) \>3*I  \\
\\
\textbf{If $I=4$}        \>Next record  if $I=4$ \>\>\\
 \textsl{XCE, YCE, ALE} \> Centers the beam on \textsl{XCE, YCE, ALE}  \> 2*cm, mrad \>3*E  \\
 \\
\end{tabbing}
...
\\
\\
$N\!F[.J], ~ N\!Col, ~ N\!H\!D\!R$	\> Number of files to convert [\texttt{READ} format type, see below],   \> $\leq20$, $\geq 1$ , $0-9$
                                                \> 3*I1 \\
...
\textbf{FAISTORE}  \label{FAISTORE-B} \index{FAISTORE|textbf}
 ...
 \textsl{FNAME}~~\footnotemark[1] 
       \>Name of storage file (\emph{e.g.}~zgoubi.fai) [~; label(s) of the element(s)  at the exit  \> \>A80, \\ %%
{[,}\textsl{\LABEL(s)}{]}  \index{LABEL@{\LABEL}}   \> of which the store occurs (10 labels maximum)]. If either \textsl{FNAME} or first \textsl{LABEL} \> \>  [, 10*A10] \\ 
     \> is 'none' then no storage occurs. Store occurs at all elements if  first  \> \> \\
     \> \textsl{LABEL} is 'all' or 'ALL'. \> \> \\
       \\
...
~ ~ $ \omega^+$, $\theta$,     $R_1$, $U_1$, $U_2$, $R_2 $     \quad \=
 ...
 \textbf{FIT, FIT2}        \label{FIT-B} \index{FIT|textbf} \index{FIT2|textbf}  \index{constraint (FIT, FIT2)|textbf}\index{variable (FIT, FIT2)|textbf}
          \> \textbf{\FITTitl} \> \> \\
  \\[-2ex]
 $NV$ \textsl{[, nofinal]} \>NV : Number of physical parameters to be varied~; \>$\leq 20$ \>I \\ \index{fit, variable range|textbf} 
  \>'nofinal'~: will avoid final run (default is~: final run performed   \> 'nofinal' \>A7 \\  
  \>using fitted values, once fit is done).  \>  \> \\  
  \\[-1ex]
...
or :     \>    \>    \> \\
 $\IR$, $\IP$, $\XC$, $[V_{min},V_{max}]$  \>
  ...
  \\[2ex]
$ \NC$ ~ \textsl{[, penalty~[,ITER]]~\footnotemark[3]}    \index{fit, penalty|textbf} \index{fit, ITER|textbf} 
       \>Number of constraints [, penalty [, number of iterations]].   \> $\leq 20$ [,$10^{-n}$ [,$>0$]] \> I [, E [, I]] \\
 \\[-2ex]
...
\textsl{YR, TR, ZR, PR, XR, DR}    \>Reference  ($DR=B\rho/\text{\BORO}$)    \>2(cm,mrad),  cm, no dim.   \>  6*E \\
 \\
\textbf{If KOBJ = 2[.01]}      \>All the initial coordinates must be entered explicitly\>\>\\
\\
...
\\
 \textsl{Y, T, Z, P, X, D, LET} 
           \>Coordinates and tagging  of the \IMAX\ particles~; 
            ...
            \> \textsl{If KOBJ = 2.01} input units are different~:  \>2(m,rad), m, no dim.,   \>\\
 \\
...
\textbf{If KOBJ = 6}      \>\textbf{Generation of 61 particles}  (for use with \textsl{MATRIX}\index{MATRIX}, $IORD=2$)  \> \> \\
 ...
 \textsl{PY, PT, PZ, PP, PX, PD} \>Step sizes in $Y$, $T$, $Z$, $P$, $X$ and $D$
     \>2(cm,mrad),  cm, no dim.  \>6*E \\
 \\
 \textsl{YR, TR, ZR, PR, XR, DR} \> Reference trajectory~; $DR= B \rho /\text{\BORO}$  \> 2(cm,mrad),  cm, no dim. \>   6*E\\
 \\
...
%
%
 \textsl{IOPT}, label, \textsl{IMP}        \> $IOPT=0/1$~: Off/On. Transport the beam matrix~;   \> 0-1, string, 0-1 \>I, A, I \\
      \> 'label'~:  Can be  'all', 'ALL', or existing 'LABEL\_1(NOEL)'~;  \>  \> \\
...
\textbf{PICKUPS}         \label{PICKUPS-B} \index{PICKUPS|textbf}         \> \textbf{\PICKUPSTitl}  \> \> \\
 ...
 \\
  \\
\textbf{For I = 1, N}       \>A list of N keywords' labels follows \>\>\\
 \\
 \LABEL1 [,LABEL2, [...]]
          \> The N labels at which beam centroid is to be computed/recorded. \> N string(s) \> N*A10 \\
          \> In case a ``\textsl{LABELi}'' in the list does not exist, it is peacefully ignored. \>  \> \\
 \\
\end{tabbing}
...
\noindent This is a possible way to inhibited an earlier use of  \textsl{PICKUPS} with  ``labelA, labelB, ...'' 
keyword list. 
It is sufficient (and necessary) for that, that no keyword in zgoubi.dat data list has 
``none'' as a its first \LABEL.  \index{LABEL}
}
...
\begin{tabbing}
\mestab
\textbf{REBELOTE} \label{REBELOTE-B} \index{REBELOTE|textbf} \index{multi-particle}\index{multi-turn}
 ...
 \textsl{[IOPT[.NPRM]]},  \>(inhibits) \textsl{FORTRAN WRITE}s to .res and to screen~; \> 0, 99~; [1[.any]]\> 2A10 \\
...
% \>zgoubi.dat data list, see below. \>\>\\
 ...
 \> in a one or a series of optical elements, following data on next line~; \>\>\\
 \> default value for \textsl{NPRM} is 1. \>\>\\
\\
\textbf{If IOPT = 1~\footnotemark[1]}          \>   \>         \>    \\[2ex]
\textsl{NPRM},    \>  Number of parameters to be changed at each of the  NPASS passes (the \>        \>  I  \\[2ex]
 \>  successive parameter values will follow that list).   \>        \>  I  \\[2ex]
\bf \textsl{NPRM } lines with the following sequence (tells parameters concerned, and for each, its successive values)~: \\[2ex]
\textsl{LMNT}, \textsl{PRM}, NV*Val
         ...
         \>  Then follow $1<NV\leq NPASS$ values (if $NV\leq NPASS$ then  the   \>  \\
         \>  last value of the list is maintained over remaining passes). \\
\end{tabbing}
...
\textbf{REBELOTE}, an example \\

The "FIT" procedure is ran 7 times, with each time a different 
value for the particle rigidity.  This finds the initial coordinates of a particle, for a series of different momenta, 
such that each particle (each momenta) will have  its helix accross the helical field map aligned on the field map axis,
following the result schemed in Fig.~\ref{FigHelix}. 
...
\scriptsize
\begin{verbatim}
Using FIT and REBELOTE keywords concurrently :
 'OBJET'                                                                     1
7.20517829566983d3            
2                  ! Defines particles one by one             
1     1            ! Y, T, Z, P, dL, D=dp/p coordinates :
-1.165820199 0. 0. 0. 0. 2.136830E+00 'o'
1 1           
              
 'PARTICUL'        ! Necessary since spin is to be tracked                   2
9.3827203E+02 1.602176487E-19 1.7928474 0 0   
 'SPNTRK'          ! Initial spin vector coordinates                         3
4.1           
0. 0.  1.     
              
 'FAISCEAU'        ! Print out particle coordiantes in zgoubi.res            4
             
 'DRIFT' DRIF   CSNK                                                         5
-20.          
 'TOSCA'           ! Handle field map(s)                                     6
0  0          
10.15 1. 1. 1.                
HEADER_9                      
321 29 29  15.2  0.9606  0.423      ! (15.)2 : two field maps superimposed
./Csnk3D_map/b_ags-full-coilv5-x06-rerun2-x071-integral-x5y5z10mm.table       
./Csnk3D_map/b_ags-full-sold3-only-nodal-x5y5z10mm-wasActually-integral.table 
0 0 0 0       
2             
.1            
2  0.  .00  0.  0.            
...
1 30 0  [-3,3]      
1                  ! One constraint : 
3 1 2 6 0. 1. 0    ! particl 1 has its coordnte j=2=Y satisfying max_Y+min_Y =0, across 
                   ! the whole element ir=6 (TOSCA, i.e. accross teh 3D field map).
                   ! The effect of the constraint is to center the trajectory on 
                   ! the helical field map axis.              
 'SPNPRT'   PRINT  ! Save spin vector in zgoubi.SPNPRT.out, for info        10
              
              
 'REBELOTE'        ! Will re-do the "FIT" for the following list of 6 different values of the  
6  0.1  0  1       ! "D=dp/p" (PRM=35) coordinate of the particle defined in "OBJET" (LMNT=1)
1 35    1. 1.387274 1.502203 1.764428 2.3835574 3.447732 3.6081683             
...
\textbf{For NF=1, NFAM :} \>repeat \textsl{NFAM} times the following sequence : \> \> \\
 ...
 \\
 $SCL(I)$, $I=1$, $NT$ \>Scaling values (a single one, normally 1, if $NT=-1$).    \>relative \>NT*E\\
 \\
...
\\
 \> Print spin coordinates zgoubi.res, at the location where this \\
...
\mestab
 \textbf{SRLOSS}        \label{SRLOSS-B} \index{SRLOSS|textbf}
     \> \textbf{\SRLOSSTitl}\index{synchrotron radiation energy loss} \> \> \\
 \\
\textsl{KSR[.i]}     \> On/off switch~; $i=1$ for info output  into \texttt{zgoubi.SRLOSS.out}       \> $0~or~1[.1]$  \> 2*I \\
\\
\textsl{STR1, STR2} \> Options~: \textsl{STR1 = 'ALL'} or \textsl{'all'} or a particular  KEYWORD~;  \> \> 2*A \\
                    \> \textsl{STR2 = 'scale'} will scale fields with energy loss.              \>  \\
 \\
...
\mestab
 ...
 \\
\textsl{NCMD} \> The number of calls to follow.  \> $\leq 0$ \> I \\
\\
\\
...
\textbf{TWISS}         \label{TWISS-B} \index{TWISS|textbf}
  ...
  \\
  \\
 \textsl{KTW[.KTW2]},  \> $KTW=0/1/2/3$ :  Off / as \textsl{MATRIX} / add computation  of \>0-3[.1], 2*any    \>I,2*E \\
         FacD, FacA    \> chromaticities / add computation of anharmonicities.    \> \>\\
...
-  off-momentum matrices at $\frac{dp}{p} = \pm FacD*\delta D$, 
