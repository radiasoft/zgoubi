...
\newcommand{\mestabOBJ}{$PY$, $PT$, $PZ$, $PP$, $PD$  \quad \= 
        total numbr of particles ; numbr of distinct momenta \quad \=
            IY*IT*IZ*IP*IT*IZ*IP*   \quad \= \kill}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{tabbing}
\mestab
\textbf{AIMANT}  \label{AIMANT-B} \index{AIMANT}
        \>\textbf{\AIMANTTitl}\\
...
the shim; $\lambda$ is unused \>2*cm, 2*deg, cm \>5*E \\
 \\
$\gamma$,  $\alpha$,  $\mu$,  $\beta$            \>geometrical parameters of the shim \>2*deg,  \>4*E \\
 \> \>2*no dim. \> \\
  \\
...
\textbf{AUTOREF}  \label{AUTOREF-B} \index{AUTOREF}
  \> \textbf{\AUTOREFTitl} \\
 \\
 \\
...
% %\vspace{10 truecm}
% %%%Figure 12
% \centerline{\includegraphics{Fig12.ps}}
...
\begin{tabbing}
\mestab
\textbf{BEND}  \label{BEND-B} \index{BEND} \>  \textbf{\BENDTitl} \> \> \\
 \\
 \\
...
\begin{figure}[H]
%%%Figure 14
\centerline{\includegraphics[height=8cm,angle=-90]{Fig14.eps}}
%\medskip
...
\begin{tabbing}
\mestab
\textbf{BINARY} \label{BINARY-B}\index{BINARY}
      \>\textbf{\BINARYTitl}\> \>    \\
\\
\\
$NF.NCol$	\> Number of files to convert, number of data columns \> $\leq 9$,  $\leq 9$  \> I1.I1 \\
\\
The next $NF$ lines: \> \> \\
\textsl{FNAME} \> Name of the file to be converted, \> \> A80 \\
	\> file is assumed binary {\it iff} begins with ``B$_{\_}$'' or ``b$_{\_}$'',  \> \> \\
	\> assumed formmatted otherwise 
 \end{tabbing}
...
\begin{tabbing}
\mestab
\textbf{BREVOL} \label{BREVOL-B}\index{BREVOL}
      \>\textbf{\BREVOLTitl}\> \>    \\
 ...
 \textsl{BNORM, XN}      \> Field and X-coordinate normalization  coeff.
            \> 2*no dim. \> 2*E \\
 \\
 \textsl{TIT}        \>Title (begin with ``FLIP'' to get field map X-flipped)\> \>A80 \\
 \\
 $IX$         \>Number of longitudinal nodes of the map \> $\leq 400$ \> I \\
 \\
\textsl{FNAME [SUM]}\footnotemark[1]$^,$\footnotemark[2]     \> Filename (e.g., solenoid.map) \> \>A80 \\
 \\
...
\begin{alltt}
\footnotetext[1]{\textsl{FNAME} \textrm{contains the field data. These must be formatted according to the following \textsl{FORTRAN} sequence:}  
...
\textrm{where \(X(I)\) and \(BX(I)\) are the longitudinal coordinate and field component at node \((I)\) of the mesh. Binary file names
\textsl{FNAME}  must begin with  B\(\sb{_}\). `Binary' will then automatically be set to `.TRUE.'.}} 
\end{alltt}  

\index{maps, summing} 
\begin{alltt}
\footnotetext[2]{Sumperimposing (summing) field maps is possible. To do so, pile up file names with 'SUM' 
following each name but the last one. e.g., in the following example, 3 field maps are read and summed~: 

myMapFile1  SUM
myMapFile2  SUM
myMapFile3  

(of course, these maps must all have their mesh defined in identical coordinate frame). }
\end{alltt}  

\newpage
\begin{tabbing}  
\mestab
\textbf{CARTEMES} \label{CARTEMES-B} \index{CARTEMES}
        \>\textbf{\CARTEMESTitl}\> \> \\  
\>mid-plane symmetry is assumed \>\>\\
 ...
 \textsl{BNORM, XN,YN}      \> Field and X-,Y-coordinate normalization  coeffs.
            \> 3*no dim. \> 3*E \\
 \\
 \textsl{TIT}        \>Title (begin with ``FLIP'' to get field map X-flipped)\footnotemark[1]  \> \>A80 \\
 \\
 $IX$, $JY$      \>Number of longitudinal ($IX$) and transverse ($JY$) \>$\leq 400$, $\leq 200$ \>2*I \\
 \>nodes of the map \> \> \\
 \\
\textsl{FNAME}\footnotemark[2] \> Filename (e.g., spes2.map) \> \>A80 \\
 \\
...
\footnotetext[1]{Begin ``Title'' with ``FLIP"\index{FLIP} so as to get  the map flipped prior to ray-tracing.}
\begin{alltt}
\footnotetext[2]{\textsl{FNAME} \textrm{contains the field data. These must be formatted 
according to the following \textsl{FORTRAN} sequence:} 
...
of the mesh. For binary files, \textsl{FNAME} must begin with B\(\sb{_}\).`Binary' will then automatically be set to `.TRUE.'}} 
\end{alltt}
\newpage
\vfill
...
\begin{tabbing}
\mestab
\textbf{CAVITE\footnotemark[1]} \label{CAVITE-B} \index{CAVITE}\index{acceleration}\index{synchrotron motion}
     \> \textbf{\CAVITETitl}   \> \> \\
 \> $ \Delta W=qVsin(2\pi hf\Delta t+\varphi_ s) $    \> \> \\
 \\
 \\
 \textsl{IOPT}       \> Option                 \>0-3 \>I \\
 \\
...
\begin{tabbing}
\mestab
\textbf{CHAMBR}  \label{CHAMBR-B} \index{CHAMBR}  
...
defined in polar coordinates (e.g. \textsl{DIPOLE}) $YL$ is the radius and  
...
%\vspace{10.5 truecm}
%%%Figure 16
\centerline{\includegraphics[height=15cm,angle=-90]{FigCHAREFa.eps}}
...
\begin{tabbing}
\mestab
\textbf{CIBLE, TARGET}         \label{CIBLE-B} \index{CIBLE} \label{TARGET-B} \index{TARGET}
     \> \textbf{\CIBLETitl} \>\>\\
 \\
 \\
...
$ T_2$, $\theta$, $\beta $            \>$ Q $ of the reaction; incident particle kinetic \>\>\\
 ...
 \\
 \BORO\index{BORO}              \>New reference rigidity after  \textsl{CIBLE}  \>kG.cm \>E
\end{tabbing}
\vfill
...
\begin{tabbing}
\mestab
\textbf{COLLIMA}         \label{COLLIMA-B} \index{COLLIMA}
...
\textsl{IFORM}, $\alpha$, $\beta$, $\epsilon/\pi$, $N_{\sigma}$ \>\textsl{IFORM} = 11, 14: horizontal collimation; 
 ...
 \>\textsl{IFORM} = 13, 16: longitudinal collimation; {\it to be } \>\>\\
 \> {\it implemented}
\end{tabbing}
...
\begin{tabbing}
\mestab
\textbf{DECAPOLE}         \label{DECAPOLE-B} \index{DECAPOLE} 
  \> \textbf{\DECAPOLETitl}   \> \> \\
  \\
  \\
...
\begin{tabbing}
\mestab
\textbf{DIPOLE}         \label{DIPOLE-B} \index{DIPOLE}
           \>\textbf{\DIPOLETitl}\> \>\\
 ...
 Azimuth for positioning of EFBs; field and field indices \> deg., kG,  \> 5*E\\
 \>                        \> 3*no dim. \> \\
 \\
 \>ENTRANCE FIELD BOUNDARY \> \> \\
 \\
$\lambda$, $\xi$               \>Fringe field extent (normally $\simeq$ gap size); unused. 
 ...
 \>with $ P(s)=C_0+C_1(\frac{s}{\lambda} )+C_2(\frac{s}{\lambda} )^2
             +...+C_5(\frac{s}{\lambda} )^5 $   \>\>\\
 \\
 $NC$, $ C_0-C_5 $, shift     \>unused; $ C_0 $ to $ C_5 $: see above; \>0-6, 6* \>I,7*E\\
 \>EFB shift   \>no dim., cm \> \\
 \\
$ \omega^ +$, $\theta$, $R_1$, $U_1$, $U_2$, $R_2 $   \>Azimuth of entrance EFB with respect
...
$\lambda$, $\xi$               \>Fringe field parameters \>cm, unused\>2*E \\
 $NC$, $ C_0-C_5 $, shift     \>    \>0-6, 6*no \>1, 7*E\\
 \>    \>dim., cm \> \\
 \\
$ \omega^-$, $\theta$, $R_1$, $U_1$, $U_2$, $R_2 $   \>Positioning and shape of the
              exit EFB \>2*deg, 4*cm \>6*E\\
...
\begin{tabbing}
\mestab
\textbf{DIPOLE-M}         \label{DIPOLE-M-B} \index{DIPOLE-M}
           \>\textbf{\DIPOLEMTitl}\> \>\\
 \>$ B_Z= \mathcal{F} B_0 \left(1-N \left(\frac{R-RM}{RM} \right) +B 
 \left(\frac{R-RM}{RM} \right)^2+G \left(\frac{R-RM}{RM} \right)^3 \right) $ \> \> \\
 \\
 ...
 \textsl{IAMAX}, \textsl{IRMAX}   \>Azimuthal and radial number of nodes of the mesh 
         \>$\leq 400$, $\leq 200$  \>2*I\\
 \\
$ B_0$, $N$, $B$, $G $           \>Field and field indices \> kG, 3* \>4*E\\
 \>                        \>no dim. \> \\
 \\
...
\begin{tabbing}
\mestab
\textbf{DIPOLES}         \label{DIPOLES-B} \index{DIPOLES}
           \>\textbf{\DIPOLESTitl}\> \>\\
 \>$ B_Z= \sum_{i=1}^N  \Bz_{0,i} \, \mathcal{F}_i(R,\theta) \, 
\left(  1  +  b_{1_i} (R-RM_{i})/RM_{i} + b_{2_i} (R-RM_{i})^2/RM_{i}^2 +... \right)$ \> \> \\
 \\
 $\IL$    \>$\IL=1,2$: print field and coordinates along trajectories\> $0-2$ \>  I  \\
 \\
 $N$, $AT$,  $RM$       \> Number of magnets in the $N$-uplet;  \> no dim.,  \> I, 2*E\\
       \> total angular extent  of the dipole;  reference radius \> deg, cm \>   \\
 \\
{\it  Repeat $N$ times the following sequence} \rule{40mm}{.1mm} \> \> \> \\
 \\
 \textsl{ACN}, $\delta R\!M$, $ B_0$, \> Positioning of EFBs : azimuth, $R\!M_{i} = R\!M + \delta R\!M$; field;
                                                     \> deg., cm, kG, \> 3*E, I, $ind$*E\\
$ind$, $b_i,~(i=1,ind)$    \> number of,  and field coefficients     \> $(ind+1)$*no dim. \> \\
 \\
 \>ENTRANCE FIELD BOUNDARY \> \> \\
 \\
$g_0$, $\kappa$               \> Fringe field extent ($g=g_0\,(RM/R)^{\kappa}$), sharp edge if $g_0$=0   \> cm, no dim. \>2*E\\
 \>Exponential type fringe field $ F=1\, /\, (1+ \exp (P(s))) $ \>\>\\
...
$ \omega^-$, $\theta$, $R_1$, $U_1$, $U_2$, $R_2 $,  $ R3$   \>  \>2*deg, 5*cm \>7*E\\
 \\
{\it  End of repeat} \rule{80mm}{.1mm} \> \> \> \\
\end{tabbing}
...
\mestab
\textsl{KIRD,~Resol}         
 \>  KIRD=0 :  analytical computation of field derivatives, Resol  \>$>0$; no dim.\>I, E\\ 
 \>   is unused \>  \\ 
 \>  KIRD$>$0 :  numerical interpolation of field derivatives, size \>  \>\\
 \>  of flying mesh \textsl{XPAS/Resol}  \>  \>\\
...
\begin{tabbing}
\mestab
\textbf{DODECAPO}    \label{DODECAPO-B} \index{DODECAPO}
\> \textbf{\DODECAPOTitl}   \> \> \\
 \\
 \\
 $\IL$        \>$\IL=1,2$: print field and coordinates along trajectories \>0-2 \>I \\
...
\mestab
\textbf{DRIFT, ESL}         \label{ESL-B} \index{ESL} \label{DRIFT-B} \index{DRIFT}
       \> \textbf{\ESLTitl}   \> \> \\
  \\
  \\
 $\XL$         \> length \> cm \> E
...
\begin{tabbing}
\mestab
\textbf{EL2TUB\footnotemark[1]}         \label{EL2TUB-B} \index{EL2TUB}
          \>\textbf{\ELTwoTUBTitl}\ \>\>\\
 \\
 \\
 $\IL$          \>$\IL=1,2$: print field and coordinates \>0-2 \>I\\
...
\begin{tabbing}
\mestab
 \textbf{ELMIR}        \label{ELMIR-B} \index{ELMIR}
     \> \textbf{\ELMIRTitl} \> \> \\
  \\
\\
  $\IL$            \>$\IL=1,2$: print field and coordinates \>0-2 \> I  \\
...
\begin{tabbing}
\mestab
 \textbf{ELMIRC}        \label{ELMIRC-B} \index{ELMIRC}
     \> \textbf{\ELMIRCTitl} \> \> \\
  \\
\\
  $\IL$            \>$\IL=1,2$: print field and coordinates \>0-2 \> I  \\
...
\begin{tabbing}
\mestab
\textbf{ELMULT\footnotemark[1]}         \label{ELMULT-B} \index{ELMULT} 
           \> \textbf{\ELMULTTitl }\> \> \\
 \\
 \\
 $\IL$                 \>$\IL=1,2$: print field and coordinates along \>0-2 \>I\\
...
\begin{tabbing}
\mestab
\textbf{ELREVOL\footnotemark[1]} \label{ELREVOL-B} \index{ELREVOL}
         \>\textbf{\ELREVOLTitl  }\> \>    \\
 \>$X$-axis cylindrical symmetry is assumed \>\>\\
 \\
 \\
 ...
 $IX$         \>Number of longitudinal nodes of the map \> $\leq$ 400 \>I \\
 \>\>\>\\
 \textsl{FNAME\footnotemark[2]}    \>Filename (e.g., elens.map) \> \>A80 \\
 \\
 $ID$, $A$, $B$, $C$   \>Integration boundary. Ineffective when $ID=0$.      \>$\geq -1$, 2*no dim., \>I,3*E  \\
 {[}$A'$, $B'$, $C'$   \>$ID=$ -1, 1 or $\geq 2$ : as for  \textsl{CARTEMES} \> cm {[},2*no dim.,\>[,3*E,etc.]\\
 $B''$,etc., if $\left. ID\geq 2\right]$ \>                                  \> cm, etc.]       \> \\
...
\footnotetext[1]{Use \textsl{PARTICUL} to declare mass and charge.} 
\begin{alltt}
\footnotetext[2]{\textrm{\textsl{FNAME} contains the field data. These must be formatted according to the following \textsl{FORTRAN} sequence: }
 ...
 \textrm{where \(X(I)\) and \(EX(I)\) are the longitudinal coordinate and field component at node \((I)\) of the mesh. 
Binary file names \textsl{FNAME} must begin with  B\(\sb{_}\). `Binary' will then automatically be set to `.TRUE.'}
}\end{alltt} 
...
\begin{tabbing}
\mestab
\textbf{FAISCEAU}         \label{FAISCEAU-B} \index{FAISCEAU}
  \> \textbf{\FAISCEAUTitl }   \> \> \\
  \\
  \\
 \> Print particle coordinates at the location where the \\
 \> keyword is introduced in the structure.\\[40pt]
%
\textbf{FAISCNL}  \label{FAISCNL-B} \index{FAISCNL} 
            \> \textbf{Store particle coordinates in file FNAME}   \> \> \\
   \\
   \\
 \textsl{FNAME}$^{ 1}$ 
       \>Name of storage file  \> \>A80 \\
       \>(e.g.,~zgoubi.fai\index{zgoubi.fai}, or b\_zgoubi.fai for binary storage). \> \> \\[40pt] 
%
\textbf{FAISTORE}  \label{FAISTORE-B} \index{FAISTORE} 
          \> \textbf{Store coordinates every $IP$ other pass [,at 
          labeled elements]}   \> \> \\
   \\
   \\
 \textsl{FNAME}$^{ 1}$ 
       \>Name of storage file (e.g.~zgoubi.fai) [; label(s) of the 
       element(s)  \> \>A80\\ %%
{[,} \textsl{\LABEL(s)}]\index{LABEL@{\LABEL}}   \>  at the exit of which the store occurs (10 labels 
       maximum)]. \> \> [, 10*A10] \\
       \\
 $IP$ 	\> Store every $IP$ other pass (when using 
 \textsl{REBELOTE}\index{REBELOTE} \> \>I\\
 \> with \textsl{NPASS}\index{NPASS} $\geq IP-1$).
\end{tabbing}
...
\begin{tabbing}
\mestab
\textbf{FFAG}         \label{FFAG-B} \index{FFAG magnet, radial}
           \>\textbf{\FFAGTitl}   \> \>   \\
\> {\bf UNDER DEVELOPEMENT}\> \>   \\
 ...
 $\IL$    \>$\IL=1,2$: print field and coordinates along trajectories\> $0-2$ \>  I  \\
 \\
 $N$, $AT$,  $RM$       \> Number of dipoles in the FFAG $N$-uplet;  \> no dim.,  \> I, 2*E\\
       \> total angular extent  of the dipole;  reference radius \> deg, cm \>   \\
 \\
{\it  Repeat $N$ times the following sequence} \rule{40mm}{.1mm} \> \> \> \\
 \\
...
\textsl{KIRD,~Resol}         
 \>  KIRD=0 :  analytical computation of field derivatives, Resol  \>$>0$; no dim.\>I, E\\ 
 \>   is unused \>  \\ 
 \>  KIRD$>$0 :  numerical interpolation of field derivatives, mesh \>  \>\\
 \>  size is \textsl{XPAS/Resol}  \>  \>\\
 \>  \hspace{10mm} KIRD=2 or 25 : second degree, 9- or 25-point grid \>  \>\\
 \>  \hspace{10mm} KIRD=4 : fourth degree, 25-point grid \>\>\\
 \\
 \textsl{XPAS}            \>Integration step                                 \>cm \>E \\
...
\begin{tabbing}
\mestab
\textbf{FFAG-SPI}         \label{FFAGSPI-B} \index{FFAG magnet, spiral}
           \>\textbf{\FFAGSPITitl}   \> \>   \\
\> {\bf UNDER DEVELOPEMENT}\> \>   \\
 \>$ B_Z= \sum_{i=1}^N \Bz_{0,i} \, \mathcal{F}_i(R,\theta) \, \left(   R/R_{M,i} \right)^{K_i}  $ \> \> \\
 \\
...
\begin{tabbing}
\mestab
\textbf{FIN, END}       \label{FIN-B} \index{FIN}   \label{END-B} \index{END} 
   \> \textbf{\FINTitl }   \> \> \\
  \\
  \\
...
\begin{tabbing}
\mestab
 \textbf{FIT}        \label{FIT-B} \index{FIT}\index{constraint (FIT)|textbf}\index{variable (FIT)|textbf}
          \> \textbf{\FITTitl} \> \> \\
  \\
  \\
 $NV$           \>Number of physical parameters to be varied  \>$\leq 20$ \>I \\
  \\
\textbf{For I = 1, NV}   \>repeat NV times the following sequence \> \> \\
  \\[-2ex]
 $IR$, $IP$, $XC$, $DV$  \>Number of the element in the structure;
             \>$\leq 200$, $\leq 99$, \>2*I, 2*E\\
 \>number of the physical parameter in the element;\>$\pm$ 200.99,  \> \\
 \>coupling switch (off = 0); variation range ($\pm$)\> relative \> \\
  \\
$ NC$           \>Number of constraints   \> $\leq 20$ \> I \\
 \\
\textbf{For I = 1, NC}  \>repeat NC times the following sequence \>\>\\
 \\[-2ex]
 $\IC$, $I$, $J$, $I\!R$, $V$\footnotemark[1], $WV$,  \>$\IC$, $I$ and $J$  define the type of constraint (see table below); 
                 \>0-5, 3*$>$0, \>4*I, 2*E, \\
$N\!P$, $p_i(i=1,NP)$  \>
$I\!R$~: number of the element after which the constraint applies; \>current unit\footnotemark[1],  \>  I, $N\!P*$E \\
 \> $V$~: value; $W$~: weight (the stronger the lower $WV$)\> no dim., curr. units \> \\
 \> $N\!P$~: number of parameters, $p_i(i=1,N\!P)$~: parameter values \>
\end{tabbing}
...
%
			\multicolumn{1}{|c|}{\textbf{Trajectory}}
			    & 3 & $1 - \IMAX$ & $1 - 6$  & $  F(J,I) $  & \textsl{[MC]OBJET}   \\
			 \multicolumn{1}{|c|}{\textbf{coordinates}}
			    &   &  $-1$      & $1 - 6$  &   $<F(J,i)>_{i=1,\IMAX}$ &   \\
			 \multicolumn{1}{|c|}{\textbf{ }}
			    &   &  $-2$      & $1 - 6$  &   $Sup(|F(J,i)|)_{i=1,\IMAX}$ & \textsl{ }   \\
			 \multicolumn{1}{|c|}{\textbf{ }}
			    &   &  $-3$      & $1 - 6$  &  $Dist|F(J,i)|_{i=1,\IMAX}$ & \textsl{ }   \\
			\multicolumn{1}{|c|}{\textbf{  }}
			    & 3.1 & $1 - \IMAX$ & $1 - 6$  &$|F(J,I) - FO(J,I)|$  &   \\
			\multicolumn{1}{|c|}{\textbf{  }}
			    & 3.2 & $1 - \IMAX$ & $1 - 6$  &$|F(J,I) + FO(J,I)|$  &  \\
                           & & & &  &\\
%
	 ...
	 & \textsl{OBJET/{\footnotesize KOBJ=8}; }  \\
                          \multicolumn{1}{|c|}{\textbf{parameters}} & & & & & \textsl{MCOBJET/{\footnotesize KOBJ=3}}  \\
                          & & & &  &\\
			\multicolumn{1}{|c|}{\textbf{Number of}} 
			    & 5 & $-1$ & any &  Ratio $N_{survived}/\IMAX$  & \textsl{OBJET} \\
			\multicolumn{1}{|c|}{\textbf{particles}} &  & $1 - 3$ & any 
                & $N_{through~\epsilon_{Y,Z,X}}/ N_{survived}$ & \textsl{MCOBJET}   \\
                          & & & &  &\\
			\hline
		\end{tabular}  }
...
\footnotetext[1]{$V$ is in current \zgou\ units. }

\footnotetext[2]{It is advised to use \textsl{OBJET} and \textsl{KOBJ} = 5, for the
definition of the initial coordinates.} 

\footnotetext[3]{It is advised to use \textsl{OBJET} and \textsl{KOBJ} = 6, for the
definition of the initial coordinates.} 

\footnotetext[4]{For use normally with object definition by 
\textsl{OBJET}\index{OBJET}. Thus, $I=$ trajectory number = 1 to 
\IMAX\ if \textsl{KOBJ} $\not = 2$;\\
 $I=$ trajectory number = 1 to 7 if \textsl{KOBJ} = 2.}

\footnotetext[5]{$J=$ coordinate number = 1 to 6 for respectively 
$D$, $Y$, $T$, $Z$, $P$ or $X$.}

\footnotetext[6]{Twiss functions: $\sigma_{11}=\beta_Y, \sigma_{12}=\sigma_{21}=-\alpha_Y, \sigma_{22}=\gamma_Y$, 
$\sigma_{33}=\beta_Z, \sigma_{34}=\sigma_{43}=-\alpha_Z, \sigma_{44}=\gamma_Z$; periodic dispersion: 
$\sigma_{16}=D_Y$, $\sigma_{26}=D'_Y$,  $\sigma_{36}=D_Z, \sigma_{46}=D'_Z$ }
...
\begin{tabbing}
\mestab
\textbf{FOCALE}         \label{FOCALE-B} \index{FOCALE}
        \> \textbf{\FOCALETitl }\>\>\\
 \\
 \\
 $\XL$      \>Distance from the location of the keyword \> cm \> E \\[60pt] 
\textbf{FOCALEZ}    \label{FOCALEZ-B} \index{FOCALEZ} 
...
\begin{tabbing}
\mestab
\textbf{GASCAT}         \label{GASCAT-B} \index{GASCAT}
        \> \textbf{\GASCATTitl}\>\>\\
 \\
 \\
...
\begin{tabbing}
\mestab
\textbf{HISTO}  \label{HISTO-B} \index{HISTO}  \> 
\textbf{\HISTOTitl} \> \> \\
  \\
  \\
...
\begin{tabbing}
\mestab
\textbf{IMAGE}         \label{IMAGE-B} \index{IMAGE}   
         \> \textbf{\IMAGETitl} \\[60pt]
%
\textbf{IMAGES}   \label{IMAGES-B} \index{IMAGES} 
...
%
\textbf{IMAGESZ}   \label{IMAGESZ-B} \index{IMAGESZ} 
          \> \textbf{Localization and size of vertical waists} \> \> \\
 \\
 \> For each momentum group, as classified by \> \> \\
 \> means of \textsl{OBJET}, \textsl{KOBJ} = 1, 2 or 4   \\[60pt]
%
\textbf{IMAGEZ}   \label{IMAGEZ-B} \index{IMAGEZ} 
...
\textbf{MAP2D} \label{MAP2D-B} \index{MAP2D|textbf}
 ...
 \\
 \textsl{FNAME\footnotemark[2]}      \>File name (e.g., magnet.map) \> \>A80 \\
 \\
 $ID$, $A$, $B$, $C$   \>Integration boundary. Ineffective when $ID=0$.      \>$\geq -1$, 2*no dim., \>I,3*E  \\
 {[}$A'$, $B'$, $C'$   \>$ID=$ -1, 1 or $\geq 2$ : as for  \textsl{CARTEMES} \> cm {[},2*no dim.,\>[,3*E,etc.]\\
 $B''$,etc., if $\left. ID\geq 2\right]$ \>                                  \> cm, etc.]       \> \\
...
\footnotetext[1]{Begin ``Title'' with ``FLIP"\index{FLIP} so as to get  the map flipped prior to ray-tracing.}
\begin{alltt}
\footnotetext[2]{\textrm{\textsl{FNAME} contains the field map data. These must be formatted 
according to the following \textsl{FORTRAN} read sequence 
(normally compatible with \textsl{TOSCA} code \textsl{OUTPUTS}):} 
...
at nodes \((I,J)\) of the mesh, $Z(1)$ is the vertical elevation of the map, and \(BX\), \(BY\), \(BZ\) 
are the components of the field.}

\textrm{For binary files, \textsl{FNAME} must begin with  B\(\sb{_}\);  
...
\textbf{MAP2D-E} \label{MAP2D-E-B} \index{MAP2D-E|textbf} 
 ...
 $IX$, $JY$        \>Number of longitudinal and horizontal-transverse \>$\leq 400$, $\leq 200$  \>2*I \\
                   \>nodes of the mesh (the Z elevation is arbitrary) \>\>\\
 \\
 \textsl{FNAME\footnotemark[2]}      \>File name (e.g., mirror.map) \> \>A80 \\
 \\
 $ID$, $A$, $B$, $C$   \>Integration boundary. Ineffective when $ID=0$.      \>$\geq -1$, 2*no dim., \>I,3*E  \\
 {[}$A'$, $B'$, $C'$   \>$ID=$ -1, 1 or $\geq 2$ : as for  \textsl{CARTEMES} \> cm {[},2*no dim.,\>[,3*E,etc.]\\
 $B''$,etc., if $\left. ID\geq 2\right]$ \>                                  \> cm, etc.]       \> \\
...
\begin{tabbing}
\mestab
\textbf{MARKER}         \label{MARKER-B} \index{MARKER}
  \> \textbf{\MARKERTitl }   \> \> \\
  \\
  \\
...
\begin{tabbing}
 \mestab
 \textbf{MATPROD}        \label{MATPROD-B} \index{MATPROD|textbf}
     \> \textbf{\MATPRODTitl} \> \> \\
 \\
 \\
...
\begin{tabbing}
\mestab
\textbf{MATRIX}         \label{MATRIX-B} \index{MATRIX}
             \> \textbf{\MATRIXTitl} \> \> \\
  \\
  \\
 \textsl{IORD, IFOC} \>Options :             \>0-2, 0-1 or \>2*I \\
 \>\textsl{IORD} = 0: Same effect as \textsl{FAISCEAU} \>$>10$ \> \\
 \>     1: First order transfer matrix; periodic beam matrix,  \> \> \\
 \>    tune numbers if \textsl{IFOC} $> 0$ \> \> \\
 \>     2: First order transfer matrix $ R_{ij} $, second \>\>\\
 \>order array $ T_{ijk} $ and higher order transfer  \>\>\\
 ...
 \>     1: matrix at the closest first order horizontal focus,\>\>\\
 \>        reference $\equiv$ particle \# 1 \>\>\\
 \>     10 + NPER: same as \textsl{IFOC} = 0, and also calculates\>\>\\
...
\begin{tabbing}
\mestab
 \textbf{MCDESINT\footnotemark[1]}        \label{MCDESINT-B} \index{MCDESINT}
                       \> \textbf{\MCDESINTTitl} \> \> \\
     \> M1 $\rightarrow$ M2 + M3    \>     \> \\
  \\
  \\
 $M2$, $M3$       \>Masses of the two decay products \>2*MeV/$ c^2 $ \>2*E\\
 \\
...
\begin{tabbing}
\mestab
\textbf{MCOBJET}         \label{MCOBJET-B} \index{MCOBJET}
   \> \textbf{\MCOBJETTitl}\index{Monte Carlo}
   \index{negative charge}\index{negative momentum} \> \> \\
 \\
 \\
 \textsl{BORO}\index{BORO}      \>Reference rigidity \> kG.cm \>E\\
 \\
...
\begin{itemize}
\item[1:] uniform, $p(x)=1$ if $-\delta x \leq x \leq \delta x$
...
\begin{itemize}
\item[1:] uniform, $p(D)=1$ if $-\delta D \leq x \leq \delta D$
...
$ \alpha_ Y$, $\beta_ Y$, $\varepsilon_Y/\pi$, $N_{\sigma_{\epsilon_Y}} $   
		\>Ellipse parameters and    \>no dim., m/rad, \>3*E, I [, I]\\ 
...
With Gaussian density type only: sorting within the ellipse frontier
...
\begin{tabbing}
 \mestab
 \textbf{MULTIPOL}        \label{MULTIPOL-B} \index{MULTIPOL}
...
\begin{tabbing}
 \mestabOBJ
\textbf{OBJET}         \label{OBJET-B} \index{OBJET} \index{negative charge}\index{negative momentum}
          \>\textbf{Generation of an object} \> \> \\
 \\
 \\
 \BORO\index{BORO}           \>Reference rigidity      \>kG.cm  \>E \\
 \\
 \KOBJ           \>Option index            \>1-6    \>I \\
 \\
...
\\
 ...
 \>is not calculated \>\>\\
 \\
\textbf{If KOBJ=3[.I, I=0,1]}      \>Reads coordinates from a storage file \> \> \\
                     \> I=0 : zgoubi.fai like data file FORMAT  \> \> \\
                     \> I=1 : read FORMAT is \texttt{\small``READ(NL,*,ERR=97,END=95) Y,T,Z,P,S,DP''}   \> \> \\
 \\
...
\textsl{YF, TF, ZF, PF,}    \> Scaling factor. TAG-ing letter : no effect if '*',    \> 7*no.dim, char.\>  7*E, A1 \\
 \textsl{XF, DF, TF, TAG}  \> otherwise only particles with TAG$\equiv$LET are retained. \>     \>\\
  \\
\textsl{YR, TR, ZR, PR,}   \>Reference. Given the next line of data,     \>2(cm, mrad), \>  7*E \\
\textsl{XR, DR, TR}       \> all coordinate C is transformed to C*CF+CR \> cm, no dim.,$\mu$s     \>\\
\\
 \textsl{InitC}     \> 0 to force new starting coordinates to old initial ones 
              \> 0-1 \> I\\
                \> 1 to force new starting coordinates to old final ones \\
 \\
 \textsl{FNAME}        \>File name (e.g., zgoubi.fai\index{zgoubi.fai})  \> \>A80 \\
                       \> (KOBJ=3 or KOBJ=3.1 determines storage FORMAT)
\end{tabbing}
...
\textbf{If KOBJ = 5[.I, I=1,9]}\>\textbf{Generation of 11 particles, or 11*(I-1) if $I\ge 2$} 
...
\textit{If KOBJ = 5.1}      \> additional data line~:  \>\>\\
 $\alpha_Y, \beta_Y$, $\alpha_Z, \beta_Z$, $\alpha_X, \beta_X$,  \> Initial beam ellipse parameters\footnotemark[1]  
         \> 2(no dim.,m),  ?, ?, \>  6*E, \\
 $D_Y, D'_Y$, $D_Z, D'_Z$  \>       \> 2(m,rad)\>  4*E \\
 \\
\textit{If KOBJ = 5.I, I=2-9}      \> i = 1 to 8 (if, resp$^{ly}$, I=2 to 9) additional data lines~:  \>\>\\
 \textsl{YR, TR, ZR, PR, XR, DR}  \> 
...
\begin{tabbing}
 \mestabOBJ
\textbf{OBJETA}         \label{OBJETA-B} \index{OBJETA}
...
\\
 ...
 \\
$ M_1-M_5 $        \>Rest masses of the bodies           \> 5*GeV/c$ ^2 $\>5*E\\
 \\
...
\begin{tabbing}
\mestab
\textbf{OCTUPOLE}         \label{OCTUPOLE-B} \index{OCTUPOLE}
        \> \textbf{\OCTUPOLETitl}    \> \> \\
 \\
 $\IL$  \>$\IL=1,2$: print field and coordinates along trajectories \>0-2\>I\\
 \\
...
$IO=4$: (default option) expansions of $ \vec  R $ and $ \vec  u $ up to $ \vec u^{(4)} $ \qquad \= 2*cm, 2*deg, cm\quad  \= \kill
 %%%%
\textbf{ORDRE}         \label{ORDRE-B} \index{ORDRE}     
       \> \textbf{\ORDRETitl} \> \> \\
 \\
 \\
...
%%%
\textbf{PARTICUL} \label{PARTICUL-B} \index{PARTICUL}   
\>\textbf{\PARTICULTitl } \> \> \\
  \\
  \\
...
\begin{tabbing}
\mestab
\textbf{PICKUPS}         \label{PICKUPS-B} \index{PICKUPS}         \> \textbf{\PICKUPSTitl}  \> \> \\
 \\
 \\
 $N$       \> 0: inactive \> \> \\
...
\textbf{For I = 1, N}       \>A list of N records follows \>\>\\
 \\
 \LABEL's
          \> N labels at which beam centroid is to be recorded \> strings \> N*A8 \\
 \\
\end{tabbing}
...
\begin{tabbing}
\mestab
\textbf{PLOTDATA}\label{PLOTDATA-B} \index{PLOTDATA}
...
%%%
\textbf{POISSON}         \label{POISSON-B} \index{POISSON}
      \>\textbf{\POISSONTitl} \>\>\\
 \\
 \\
 $\IC$, $\IL$      \>$\IC=1,2$: print the field map \>0-2, 0-2 \>2*I\\
 ...
 \> of the uniform mesh \> \> \\
 \\
 \textsl{FNAME\footnotemark[1]}    \>Filename (normally, outpoi.lis)  \> \>A80 \\
 \\
 $ID$, $A$, $B$, $C$   \>Integration boundary. Ineffective when $ID=0$.      \>$\geq -1$, 2*no dim., \>I,3*E  \\
 {[}$A'$, $B'$, $C'$   \>$ID=$ -1, 1 or $\geq 2$ : as for  \textsl{CARTEMES} \> cm {[},2*no dim.,\>[,3*E,etc.]\\
 $B''$,etc., if $\left. ID\geq 2\right]$ \>                                  \> cm, etc.]       \> \\
...
\begin{alltt}
\footnotetext[1]{\textrm{\textsl{FNAME} contains the field map data. These must be formatted according to the following \textsl{FORTRAN} 
read sequence:} 
...
\begin{tabbing}  
\mestab
\textbf{POLARMES} \label{POLARMES-B} \index{POLARMES}
        \>\textbf{\POLARMESTitl }\> \> \\  
\>mid-plane symmetry is assumed \>\>\\
 \\
 \\
 ...
 \\
  \textsl{BNORM, AN,RN}      \> Field and A-,R-coordinate normalization  coeffs.
            \> 3*no dim. \> 3*E \\
 \\
 \textsl{TIT}        \>Title   (begin with ``FLIP'' to get field map A-flipped)\> \>A80 \\
 \\
 $IA$, $JR$      \>Number of angular  and radial nodes of the mesh \>$\leq 400$, $\leq 200$ \>2*I \\
 \\
\textsl{FNAME}\footnotemark[1] \> Filename (e.g., spes2.map) \> \>A80 \\
 \\
 $ID$, $A$, $B$, $C$   \>Integration boundary. Ineffective when $ID=0$.      \>$\geq -1$, 2*no dim., \>I,3*E  \\
 {[}$A'$, $B'$, $C'$   \>$ID=$ -1, 1 or $\geq 2$ : as for  \textsl{CARTEMES} \> cm {[},2*no dim.,\>[,3*E,etc.]\\
 $B''$,etc., if $\left. ID\geq 2\right]$ \>                                  \> cm, etc.]       \> \\
...
\begin{tabbing}
\mestab
 \textbf{PS170}        \label{PS170-B} \index{PS170}
          \>\textbf{\PSusoTitl} \>\>\\
 \\
 \\
 $\IL$   \>$\IL=1,2$: print field and coordinates along trajectories \>0-2 \>I\\
...
%%%
\textbf{QUADISEX}         \label{QUADISEX-B} \index{QUADISEX}
      \>\textbf{\QUADISEXTitl}  \> \> \\
...
\begin{tabbing}
\mestab
 \textbf{QUADRUPO}        \label{QUADRUPO-B} \index{QUADRUPO}
  \> \textbf{\QUADRUPOTitl }  \> \> \\
 \\
  \\
 $\IL$        \>$\IL=1,2$: print field and coordinates along trajectories \>0-2 \>I \\
...
\begin{tabbing}
\mestab
\textbf{REBELOTE} \label{REBELOTE-B} \index{REBELOTE} \index{multiparticle}\index{multiturn}
		\index{acceleration}\index{synchrotron motion}
     \>\textbf{Jump to the beginning of \zgoubi input data file}\>\>\\
 \\
 \\
 \textsl{NPASS}\index{NPASS}, \textsl{KWRIT}, $K$ 
        \>Number of runs; \textsl{KWRIT} = 0 inhibits the  \>arbitrary, \>3*I\\
        \>\textsl{FORTRAN WRITE} statements; $K$ = option \>0-1, 0 or 99 \> \\
 ...
 \>used (e.g. in \textsl{MCOBJET}\index{MCOBJET}) their seeds will not be reset \>\>\\
 \>$K=99$: the coordinates resulting from the previous \>\>\\
 \>run are used as initial coordinates for the \>\>\\
 \>next run; idem for spin components.  
\end{tabbing}
...
\begin{tabbing}
\mestab
\textbf{RESET} \label{RESET-B} \index{RESET}  \> \textbf{\RESETTitl} \>\>\\
 \\
 \\
...
\begin{tabbing}
\mestab
\textbf{SCALING} \label{SCALING-B} \index{SCALING}  \index{acceleration}
...
\textbf{For NF=1, NFAM:} \>repeat \textsl{NFAM} times the following sequence: \> \> \\
 \> \> \> \\
 \textsl{NAMEF [, Lbl [, Lbl]]}       \>Name of family (i.e., keyword of concern), up to 2 labels \> \>A8 [,A8[,A8]] \\
 \\
 $NT$          \>Number of timings   \>1-10 \>I \\
 \\
 $SCL(I)$, $I=1$, $NT$ \>Scaling values     \>relative \>NT*E\\
 \\
 $TIM(I)$, $I=1$, $NT$ \>Corresponding timings. Out of this range, \>turn 
number \>NT*I \\
...
\begin{tabbing}
\mestab
\textbf{SEPARA\footnotemark[1]}         \label{SEPARA-B} \index{SEPARA}
...
$N$, $EB1$, $EB2$, $EG1$, $EG2$\quad \=
         $\IL=1,2$: print field and coordinates along trajectories\quad \=
             1-2, 2*cm, rad\quad \= \kill
%%%             
\textbf{SEXQUAD}         \label{SEXQUAD-B} \index{SEXQUAD}        
...
\begin{tabbing}
\mestab
\textbf{SEXTUPOL }           \label{SEXTUPOL-B} \index{SEXTUPOL}
...
\begin{tabbing}
\mestab
 \textbf{SOLENOID}        \label{SOLENOID-B} \index{SOLENOID}
     \> \textbf{Solenoid} \> \> \\
 \\
 \\
 $\IL$  \> $\IL=1,2$: print field and coordinates along trajectories \>0-2\>I\\
...
% \> keyword is introduced in the structure. \\[60pt]
 \textbf{SPNPRNL}        \label{SPNPRNL-B} \index{SPNPRNL}
     \> \textbf{Store spin coordinates in file \textsl{FNAME}} \> \> \\
 \\
 \\
\textsl{FNAME\footnotemark[1]} \> Name of storage file (e.g. zgoubi.spn\index{zgoubi.spn}) \>\> A80 \\[60pt]
 \textbf{SPNPRNLA}        \label{SPNPRNLA-B} \index{SPNPRNLA}
     \> \textbf{Store spin coordinates every $IP$ other pass} \> \> \\
\\
 \\
\textsl{FNAME\footnotemark[1]} \> Name of storage file (e.g. zgoubi.spn) \>\> A80 \\
\\
$IP$ \> Store every $IP$ other pass (when using \>$\leq NPASS$ \> I \\
\> \REBELOTE\index{REBELOTE} with \textsl{NPASS}\index{NPASS} $\geq IP -1$) \\[60pt]
...
\begin{alltt}
\footnotetext[1]{\textsl{FNAME} \textrm{contains the spin coordinates and other informations, stored following the \textsl{FORTRAN} sequence below:} 

	 OPEN (UNIT = NL, FILE = FNAME)
	 DO  1  I = 1, IMAX
	    WRITE(NL,100) LET(I),IEX(I),SXO(I),SYO(I),SZO(I),SO(I),SX(I),SY(I),SZ(I),S(I),\(\gamma\),I,IMAX,IPASS,NOEL
  100	    FORMAT(1X, A1, I2, 1P , 6E15.7, /, E15.7, 2I3, I6)
   1     CONTINUE
 
\textrm{where \(SX\), \(SY\), \(SZ\) are the spin components (suffix O stands for origin). \(S=(SX\sp{2}+SY\sp{2}+SZ\sp{2})\sp{1/2}\), \(\gamma =\) Lorentz factor, 
\(I=\) particle number, \IMAX\ = total number of particles per pass, \textsl{IPASS} = pass number (as incremented by \textsl{REBELOTE}), 
\textsl{NOEL} = position of the keyword \textsl{SPNPRNL[A]} in the zgoubi.dat\index{zgoubi.dat} data list. See \textsl{OBJET} and \textsl{SPNTRK} for more details.}} 
\end{alltt}
...
\begin{tabbing}
\mestab
 \textbf{SPNTRK\footnotemark[1]}        \label{SPNTRK-B} \index{SPNTRK}\index{spin tracking}
     \> \textbf{Spin tracking} \> \> \\
 \\
...
\textbf{If KSO = 1 --  3} \> \textsl{KSO} = 1 (respectively 2, 3): all particles \\
      ...
      \> and (0,0,1) -- vertical] \\
\\
\textbf{If KSO = 4}    \>  Repeat \IMAX times (corresponding to \IMAX \\
      \> particles, cd `\textsl{OBJET}') the following sequence: \\
\\
$S_x$, $S_y$, $S_z$   \> $X$, $Y$ and $Z$ components of the spin \> 3*no dim. \> 3*E \\
\\
...
%%%Figure 7
\begin{center}
\includegraphics[height=10.5cm]{Fig7.ps}\\[20pt]
\begin{minipage}[t]{13cm}
Spin distribution as obtained with option \textsl{KSO} = 5\\
The spins are distributed within an annular strip $\delta A$ (standard deviation)  \\
...
\mestab
 ...
 \\
\textsl{KSR}, IR \> Switch; seed \> $0-1$, $>10^5$\> 2*I \\
...
\begin{tabbing}
\mestab
 \textbf{TOSCA}        \label{TOSCA-B} \index{TOSCA}
     \> \textbf{\TOSCATitl} \> \> \\
 \\
 \\
 $\IC$, $\IL$      \> see \textsl{CARTEMES} \>0-2, 0-2 \>2*I\\
 \\
 \textsl{BNORM,}      \> Field and X- (if $IZ=1$ below), or X-,Y-,Z-   \>  \>  \\
 \textsl{XN [,YN,ZN]}      \>   (if $IZ\ne 1$)  -coordinate normalization coeffs. 
            \> 2[4]*no dim. \> 2[4]*E \\
 \\
 \textsl{TIT}        \>Title (begin with ``FLIP'' to get field map X-flipped)\footnotemark[1]  \> \>A80 \\
 \\
 $IX$, $IY$, $IZ$, \textsl{MOD}      \>Number of nodes of the mesh in the $X$, $Y$    \>$\leq 400$, $\leq 200$, \>3*I \\
                              \> and $Z$ directions,  $IZ=1$ for 2-D maps;     \>  $3\leq |IZ|$ \> \\
                              \> \textsl{MOD} is a map format/lecture mode \> \> \\
                              \> \textsl{MOD}=0,~1 : Cartesian mesh, see below ; \> \> \\
                              \> \textsl{MOD}=20 : cylindrical mesh. \> \> \\
 \\
\textsl{FNAME}\footnotemark[2] \>   Names of the $NF=IZ$ files containing the maps,   \> \>A80 \\
($K=1$, $NF$) \> ordered from $Z(1)$ to $Z(NF)$. \\
      \> If \textsl{MOD}=0: $NF = 1+ [IZ/2]$, the $NF$ maps are symmetrized\\
      \> with respect to the $Z(1)=0$ plane.\\
...
\footnotetext[1]{Begin ``Title'' with ``FLIP"\index{FLIP} so as to get  the map flipped prior to ray-tracing.}
\begin{alltt}
\footnotetext[2]{\textrm{Each file \textsl{FNAME(K)} contains the field map at elevation \(Z(K)\) and must be 
formatted according to the following \textsl{FORTRAN} read sequence 
(that normally fits \textsl{TOSCA} code \textsl{OUTPUTS}):} 
...
\textrm{where \(X(I)\), \(Y(J)\), \(Z(K)\) are the longitudinal, horizontal and vertical coordinates and \(BX\), 
\(BY\), \(BZ\) are the components of the field at node \((I,J,K)\) of the mesh. For 2-D~maps 
 $B_X$ and $B_Y$ are assumed  zero at all nodes of the 2D mesh, regardless of BX(J,1,I), BY(J,1,I) values. 
For binary files, \textsl{FNAME} must begin with  B\(\sb{_}\).`Binary' will then automatically be set to `.TRUE.'}}
\end{alltt}
...
\begin{tabbing}
\mestab
 \textbf{TRAROT}        \label{TRAROT-B} \index{TRAROT}
...
\begin{tabbing}
\mestab
\textbf{TWISS}         \label{TWISS-B} \index{TWISS}
             \> \textbf{\TWISSTitl} \> \> \\
  \\
  \\
 \textsl{KTWISS} \>Options :             \>0-2 \>2*I \\
 \>     0: No effect  \> \> \\
 \>     1: First and second order quantities are computed \> \> \\
 \>     2: Higher orders  \>\>\\
\end{tabbing}
...
\begin{tabbing}
\mestab
\textbf{UNDULATOR}  \label{UNDULATOR-B} \index{UNDULATOR} \>\textbf{Undulator magnet} \> \> \\
 \\
 \\
{\it To be documented}
% $\IL$            \>$\IL=1,2$: print field and coordinates \>0-2 \> I  \\
...
\begin{tabbing}
\mestab
 \textbf{UNIPOT}        \label{UNIPOT-B} \index{UNIPOT}
...
\begin{tabbing}
\mestab
 \textbf{VENUS}        \label{VENUS-B} \index{VENUS}
...
\begin{tabbing}
\mestab
 \textbf{WIENFILT\footnotemark[1]}        \label{WIENFILT-B} \index{WIENFILT}
     \> \textbf{Wien filter} \> \> \\
 \\
 \\
  $\IL$            \>$\IL=1,2$: print field and coordinates \>0-2 \> I  \\
...
\begin{tabbing}
\mestab
 \textbf{YMY}        \label{YMY-B} \index{YMY}
