...
\textsl{FNAME} \> Name of the file to be converted, \> \> A80 \\
	\> file is assumed binary \textsl{iff} begins with ``B$_{\_}$'' or ``b$_{\_}$'',  \> \> \\
...
\index{maps, summing} 
\begin{alltt}
\footnotetext[2]{Sumperimposing (summing) field maps is possible. To do so, pile up file names with 'SUM' following each name but the last one. e.g., in the following example, 3 field maps are read and summed : 
...
\begin{alltt}
\footnotetext[2]{\textsl{FNAME} \textrm{contains the field data. These must be formatted according to the following \textsl{FORTRAN} sequence :} 
...
\textrm{where \(X(I)\) and \(Y(J)\) are the longitudinal and transverse coordinates and \textsl{BMES} is the \(Z\) field component  at a node \((I,J)\) of the mesh. For binary files, \textsl{FNAME} must begin with B\(\sb{_}\).`Binary' will then automatically be set to `.TRUE.'}} 
\end{alltt}
\newpage
\vfill
...
\footnotetext[2]{\textrm{\textsl{FNAME} contains the field data. These must be formatted according to the following \textsl{FORTRAN} sequence : }
 ...
 \textrm{where \(X(I)\) and \(EX(I)\) are the longitudinal coordinate and field component at node \((I)\) of the mesh. Binary file names \textsl{FNAME} must begin with  B\(\sb{_}\). `Binary' will then automatically be set to `.TRUE.'}
}\end{alltt} 
...
\textbf{MAP2D} \label{MAP2D-B} \index{MAP2D|textbf}
 ...
 \\
 \textsl{FNAME\footnotemark[1]}      \>File name (e.g., magnet.map) \> \>A80 \\
 \\
 $ID$, $A$, $B$, $C$   \>Integration boundary. Ineffective when $ID=0$.      \>$\geq -1$, 2*no dim., \>I,3*E  \\
 {[}$A'$, $B'$, $C'$   \>$ID=$ -1, 1 or $\geq 2$ : as for  \textsl{CARTEMES} \> cm {[},2*no dim.,\>[,3*E,etc.]\\
 $B''$,etc., if $\left. ID\geq 2\right]$ \>                                  \> cm, etc.]       \> \\
...
\begin{alltt}
\footnotetext[1]{\textrm{\textsl{FNAME} contains the field map data. These must be formatted according to the following \textsl{FORTRAN} read sequence (normally compatible with \textsl{TOSCA} code \textsl{OUTPUTS}) :} 
...
\textrm{For binary files, \textsl{FNAME} must begin with  B\(\sb{_}\) ; 
 a logical flag  'Binary' will then automatically be set to '.TRUE.'}} 
\end{alltt}
...
\textbf{MAP2D-E} \label{MAP2D-E-B} \index{MAP2D-E|textbf} 
 ...
 $IX$, $JY$        \>Number of longitudinal and horizontal-transverse \>$\leq 400$, $\leq 200$  \>2*I \\
                   \>nodes of the mesh (the Z elevation is arbitrary) \>\>\\
 \\
 \textsl{FNAME\footnotemark[1]}      \>File name (e.g., mirror.map) \> \>A80 \\
 \\
 $ID$, $A$, $B$, $C$   \>Integration boundary. Ineffective when $ID=0$.      \>$\geq -1$, 2*no dim., \>I,3*E  \\
 {[}$A'$, $B'$, $C'$   \>$ID=$ -1, 1 or $\geq 2$ : as for  \textsl{CARTEMES} \> cm {[},2*no dim.,\>[,3*E,etc.]\\
 $B''$,etc., if $\left. ID\geq 2\right]$ \>                                  \> cm, etc.]       \> \\
...
\begin{alltt}
\footnotetext[1]{\textrm{\textsl{FNAME} contains the field map data. These must be formatted 
...
\mestab
 \textbf{MCDESINT\footnotemark[1]}        \label{MCDESINT-B} \index{MCDESINT}
  ...
  \\
  \\
$[$INFO,$]$\footnotemark[2] $M2$, $M3$   \> $[$Switch,$]$ ; masses of the two decay products \> 2*MeV/$ c^2 $ \> $[$A4,$]$ 2*E\\
 \\
...
\mestab
 ...
 \\
 $IX$, $IY$, $IZ$, \textsl{MOD[.i]}      \>Number of nodes of the mesh in the $X$, $Y$    \>$\leq 400$, $\leq 200$, \>3*I \\
                              \> and $Z$ directions,  $IZ=1$ for single 2-D map ;     \>  $\ge 1$, $\ge 0$[.1-9] \> \\
                              \> \textsl{MOD} : operational and map \textsl{FORMAT} reading mode\footnotemark[1] \> \> \\
                              \> \textsl{MOD}$\le$19 : Cartesian mesh ; \> \> \\
                              \> \textsl{MOD}$\ge$20 : cylindrical mesh ; \> \> \\
                              \> \textsl{.i}, optional, tells the reading \textsl{FORMAT}, default is '*'. \> \> \\
...
\begin{alltt}
\footnotetext[1]{\textrm{\small Each file \textsl{FNAME(K)} contains the field specific to elevation \(Z(K)\) and must be formatted according to the following \textsl{FORTRAN} read sequence (that usually fits \textsl{TOSCA} code \textsl{OUTPUTS}) :  }}
{\tiny
   DO K = 1, NF
...
\textrm{\small Note : for 2-D~maps  BX and BY are assumed  zero at all nodes of the 2-D mesh, regardless of BX(J,1,I), BY(J,1,I) values. For binary files, \textsl{FNAME} must begin with B\(\sb{_}\). `Binary' will then automatically be set to `.TRUE.'}
\end{alltt}
