...
~ ~ $ \omega^+$, $\theta$,     $R_1$, $U_1$, $U_2$, $R_2 $     \quad \=
 ...
 \textbf{FIT, FIT2}        \label{FIT-B} \index{FIT|textbf} \index{FIT2|textbf}  \index{constraint (FIT, FIT2)|textbf}\index{variable (FIT, FIT2)|textbf}
          \> \textbf{\FITTitl} \> \> \\
  \\[-2ex]
 $NV$           \>Number of physical parameters to be varied  \>$\leq 20$ \>I \\    \index{fit, variable range|textbf} 
  \\[-2ex]
...
\textbf{MATRIX}         \label{MATRIX-B} \index{MATRIX|textbf}
 ...
 \textsl{[, zgoubi.MATRIX.out]} \> \textsl{IORD} = 0 : Same effect as \textsl{FAISCEAU} \>  \> \\
  \textsl{[, coupled]} \> \textsl{IORD} = 1 (normally using \textsl{OBJET}, \textsl{KOBJ} = 5) : First order transfer \> \> \\
...
\\
 \>  Including '\textsl{zgoubi.MATRIX.out}' will cause printout to  zgoubi.MATRIX.out file.   \>\>\\
\\
 \>  Including '\textsl{coupled}', in the periodic case,  will cause  use of coupled formalism.   \>\>\\
\end{tabbing}
