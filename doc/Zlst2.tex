...
or :     \>    \>    \> \\
 $\IR$, $\IP$, $\XC$, $[V_{min},V_{max}]$  \>
  ...
  \\[2ex]
$ \NC$ ~ \textsl{[, penalty~\footnotemark[3]]}    \index{fit, penalty|textbf} 
       \>Number of constraints [, penalty].   \> $\leq 20$ [, $\sim 10^{-n}$]\> I ~ [, E] \\
 \\[-2ex]
...
\footnotetext[1]{~ K=22 is compatible with use of the \textsl{FIT} \index{FIT} procedure~: \emph{e.g.},  allows successive  \textsl{FIT}s  in a run, with successive sets of optical parameters.} 
