...
$B''$,etc., if $\left. ID\geq 2\right]$\>entrance boundary defined by $AX+BY+C=0$.\> cm, etc.]     \>           \\
 ...
 \\
 \textsl{IORDRE}     \>  Degree of interpolation polynomial (see \textsl{DIPOLE-M})   \>2, 4 or 25 \>I\\
 \\
 \textsl{XPAS}          \>Integration step  \>cm \>E \\
 \\
 \textsl{KPOS}, \textsl{XCE},     \>\textsl{KPOS}=1 : element aligned, 2 : misaligned~; 
...
$ \hat  V$, $\phi_ s $       \>R.F. peak voltage~; synchronous phase \>V, rad \>2*E \\
 \\
\textbf{If IOPT$\geq 4$}  \> Various acceleration modes~: FFAG, cyclotron, etc., see routine \texttt{cavite.f}.
\end{tabbing}
...
\mestab
\textsl{KIRD,~Resol}         
 \>  KIRD=0 :  analytical computation of field derivatives~;  \>0, 2, 4 or 25~; \>I, E\\
