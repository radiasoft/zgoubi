...
\\
\\
 \textsl{NFACE}, $\IC$, $\IL$        \>Number of field boundaries \>2-3, 0-2, 0-2$[\times 10^n]$ \>3*I\\
...
\textbf{BREVOL} \label{BREVOL-B}\index{BREVOL|textbf}
 ...
 \\
 \\
 $\IC$, $\IL$      \>$\IC=1,2$ : print the map \>0-2, 0-2$[\times 10^n]$ \>2*I\\
 \>$\IL=1,2[\times 10^n]$ : print field and coordinates along trajectories.  \> \> \\
 \\
...
%%%%%%%%%
\textsl{KIRD[.n],~Resol}         
 \>  KIRD=0 :  analytical computation of field derivatives~;  \>0, 2, 25 or 4~; no dim. \>I, E\\
 \>    n=0~: default, $ B_Z$ formula (i) above, n=1~: $ B_Z$ formula (ii). \>  \\ 
 \>   Resol = 2/4 for 2nd/4th order field derivatives computation \>  \\ 
 \>  KIRD=2, 25 or 4~:  numerical interpolation of field derivatives~; \>  \>\\
...
\mestab
\textsl{KIRD,~Resol}         
 \>  KIRD=0 :  analytical computation of field derivatives~;  \>0, 2, 25 or 4~; \>I, E\\
...
%
         ...
         & & & & & \textsl{[MC]OBJET}   \\
			...
			\multicolumn{1}{|c|}{\textbf{  }}
			    & 3.3 & $1 - \IMAX$ & $1 - 7$  & min. or max.  of $F(J,I)$ & 1 & 1 or 2 & & &  \\
			\multicolumn{1}{|c|}{\textbf{  }}
			    & 3.4 & $1 - \IMAX$ & $1 - 7$  &$|F(J,I) - F(J,K)|$  & 1 & $K\leq \IMAX$ & & &  \\
			\multicolumn{1}{|c|}{\textbf{  }}
			    & 3.5 & $1 - \IMAX$ & $1 - 7$  &$(F(J,I) - F(J,K))/F(J,K)$  & 1 & $K\leq \IMAX$ & & &  \\
                            & & & & & & & & &  \\
%
...
\textbf{MAP2D} \label{MAP2D-B} \index{MAP2D|textbf}
  ...
  \\
  \\
 $\IC$, $\IL$        \> $\IC=1,2$ : print the field map \>0-2, 0-2$[\times 10^n]$ \>2*I \\
 \>$\IL=1,2[\times 10^n]$ : print field and coordinates along trajectories \>\>\\
 \\
 \textsl{BNORM, XN,YN}      \> Field and X-,Y-coordinate normalization  coeffs.
            \> 3*no dim. \> 3*E \\
...
\\
\textbf{If K = 22~\footnotemark[1]}          \>   \>         \>    \\[2ex]
\textsl{NPRM}   \>  Number of parameters to be changed for next runs  \>        \>  I  \\[2ex]
\textsl{\bf Repeat $NPR$ times the following sequence (tells parameters concerned, and for each its successive values)~:} \\[2ex]
\textsl{LMNT}, \textsl{PRM}, NV*Val
         \>  Keyword \# in zgoubi.dat list~; parameter \# under that Keyword  \>  -, -, NV*dim\footnotemark[2] \>  2*I, NV*E \\
         \>  (same as for FIT[2], see page~\pageref{FIT-B})~; \textsl{NV} successive values (if  \>       \\ 
         \>   $NV<NPASS$ then  last value is maintained over remaining passes).
\end{tabbing}

\footnotetext[1]{~ K=22 is compatible with use of the \textsl{FIT[2]} \index{FIT} \index{FIT2} procedure~: \emph{e.g.},  allows successive  \textsl{FIT}s  in a run, with successive sets of optical parameters.} 
...
\\
 \> Print spin coordinates into zgoubi.res, at the location where this \\
...
\mestab
 \textbf{SPNTRK~\footnotemark[1]}        \label{SPNTRK-B} \index{SPNTRK|textbf}\index{spin tracking}
     \> \textbf{Spin tracking} \> \> \\
 \\
\textsl{KSO} \> \textsl{KSO=0}~: spin tracking [switched] off~; \textsl{KSO=-1}~: spin  tracking  \> -1 or 0 or  \> I \\
...
\textsl{STR1, STR2} \> Options~: \textsl{STR1 = 'ALL'} or \textsl{'all'} or a particular  KEYWORD~;  \> 2*A \\
 ...
 \\
\textsl{Option}, seed \>  Option~: 0 / inhibited, 1 / photon entails dp only,   \> $1-3$,  $>10^5$ \> I \\
                \>  2 / photon entails dp and angle kick.  \> \\
