...
\textbf{FAISTORE}  \label{FAISTORE-B} \index{FAISTORE} 
 ...
 \textsl{FNAME}~~\footnotemark[1]$^,$~\footnotemark[2] 
       \>Name of storage file (e.g.~zgoubi.fai) [~; label(s) of the element(s)  at the exit  \> \>A80, [, 10*A10] \\ %%
...
\begin{tabbing}
\mestab
 \textbf{FIT, FIT2}        \label{FIT-B} \index{FIT} \index{FIT2}  \index{constraint (FIT, FIT2)|textbf}\index{variable (FIT, FIT2)|textbf}
          \> \textbf{\FITTitl} \> \> \\
  \\[-2ex]
...
\begin{alltt}
\footnotetext[1]{\textrm{\textsl{FNAME} (\emph{e.g.},  magnet.map) contains the field map data. These must be formatted according to the following \textsl{FORTRAN} read sequence (normally compatible with \textsl{TOSCA} code \textsl{OUTPUTS}) :} 
...
\begin{alltt}
\footnotetext[1]{\textrm{\textsl{FNAME} (\emph{e.g.},  mirror.map)  contains the field map data. These must be formatted 
...
\begin{tabbing}
\mestab
\textbf{OPTICS}         \label{OPTICS-B} \index{OPTICS}
  \> \textbf{\OPTICSTitl }   \> \> \\
  \\
  \\
 \> \\[20pt]
%
%
 $IOPT$, label        \> 0/1/11~: Off / transported beam matrix / local Twiss matrix~;   \>  \>I, A \\
      \> 'label'~: If specified, 'label' can be  'all' or 'LABEL(NOEL)'   \>  \> \\
      \> (default (i.e., if 'label' is not specified) is  label='all'. )\>  \> \\
...
\begin{alltt}
\footnotetext[1]{\textrm{\textsl{FNAME} (normally, outpoi.lis)  contains the field map data. These must be formatted according to the following \textsl{FORTRAN} 
read sequence :} 
...
\begin{alltt}
\footnotetext[1]{\textsl{FNAME} (\emph{e.g.},  spes2.map) \textrm{contains the field data. These must be formatted according to the following \textsl{FORTRAN} sequence :} 
...
\textbf{REBELOTE} \label{REBELOTE-B} \index{REBELOTE} \index{multiparticle}\index{multiturn}
 ...
 \textsl{[, Label1 [, Label2]]}        \>(inhibits) \textsl{FORTRAN WRITE}s to .res and to screen~; \> 0-1~; 0, 22, 99 \> 2A8 \\
        \> $K$  option~:  \>\> \\
...
\textbf{For NF=1, NFAM :} \>repeat \textsl{NFAM} times the following sequence : \> \> \\
 \> \> \> \\
 \textsl{NAMEF [, Lbl [, Lbl]]}       \>Name of family (\emph{i.e.}, keyword of concern), up to 2 labels \> \>A8 [,A8[,A8]] \\
 \\
...
\textbf{TWISS}         \label{TWISS-B} \index{TWISS}
 ...
 \>     0 : No effect  \> \> \\
 \>     1 : First and second order periodic quantities are computed \> \> \\
% \>     2 : Higher orders  \>\>\\
\end{tabbing}
