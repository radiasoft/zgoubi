...
\newcommand{\mestabOBJ}{$PY$, $PT$, $PZ$, $PP$, $PD$  \quad \= 
        total number of particles~; numbr of distinct momenta \quad \=
...
\textsl{FNAME}  must begin with  'B\(\sb{_}\)' or 'b\(\sb{_}\)'. `Binary' will then automatically be set to `.TRUE.'.}} 
\end{alltt}  
...
%
			\multicolumn{1}{|c|}{\textbf{Spin}}
			    & 10 & $1 - \IMAX$ & $1 - 4$  & $  S(J,I=1,3), ~ |\vec S(I)| $  
         & & & & & \textsl{[MC]OBJET}   \\
			\multicolumn{1}{|c|}{\textbf{  }}
			    & 10.1 & $1 -\IMAX$ & $1 -3$  &$|S(J,I) -S_{initial}(J,I)|_{I=1,3}$
         & & & & & \textsl{+SPNTRK}   \\
			\hline
			\hline
		\end{tabular}  }
...
\textbf{MATRIX}         \label{MATRIX-B} \index{MATRIX|textbf}
  ...
  \\
  \\
 \textsl{IORD, IFOC [, zgoubi.MATRIX.out]} \>Options :                                            \>0-2, 0-1 or \>2*I \\
...
% \>coefficients~;  periodic parameters, chromaticities\index{Cromaticity}, etc.   if \textsl{IFOC} $> 0$ \>\>\\
 ...
 \>  Addition of 'zgoubi.MATRIX.out' will cause output stacking into   zgoubi.MATRIX.out file   \>\>\\
 \\
...
\textrm{\small Note : for 2-D maps  BX and BY are assumed  zero at all nodes of the 2-D mesh, regardless of BX(J,1,I), BY(J,1,I) values. For binary files, \textsl{FNAME} must begin with 'B\(\sb{_}\)' or 'b\(\sb{_}\)'.  `Binary' will then automatically be set to `.TRUE.'}
\end{alltt}
