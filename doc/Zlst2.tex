...
%\begin{alltt}
\footnotetext[1]{ 
\textsl{MOD=1}~:   centered multipol model~; \textsl{MOD=2}~: long-shifted dipole model~;
...
\mestab
 \textbf{SPNTRK~\footnotemark[1]}        \label{SPNTRK-B} \index{SPNTRK|textbf}\index{spin tracking}
     \> \textbf{Spin tracking} \> \> \\
 \\
\textsl{KSO} \> Variuos options for nitial conditions \> 1-5 \> I \\
\\
...
$A''$, etc., if $\left. ID\geq 2\right]$ \>$ID=$ -1, 1 or $\geq 2$ : as for \textsl{CARTEMES}\>2*no dim. [,idem]\>[,3*E,etc.]\\
 \\
