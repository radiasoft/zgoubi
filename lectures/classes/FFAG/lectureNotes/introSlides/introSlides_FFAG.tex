...
\markboth{\large  ICFA-HB2004}{\large  ICFA-HB2004}
...
~~~~~~~\hfill \large   \bf CEA DAPNIA/SACM 

~~~~~\hfill \large   \bf \& CERN AB/ABP
}
...
\begin{center} 
\Huge  
\bf  Fixed Field Alternating Gradient Synchrotrons 
\end{center} 
...
\hspace{20mm} \includegraphics[width=12.cm]{./figures/ring3Draw.eps}
...
\begin{itemize}
 ...
 {\Large$B=B_0 (\frac{r}{r_0})^K $ }
  ...
  \end{itemize}
  \item Transverse motion stability is insured by AG focussing
  \vspace{-2mm}
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item with ratio {\Large $|\int B_D \, ds| \approx \frac{2}{3} \times \int B_F \, ds$},  to insure axial focussing
    \end{itemize}
  \end{itemize}
...
%    \item scalloping is small~: orbit length = $r \times (2 \pi+\epsilon) $, $\epsilon$ small 
    \item {\Large $\alpha = 1/(1+K) \rightarrow$} larger K insures smaller {\Large $r_{max}-r_{min}$}
...
\medskip
\includegraphics*[bbllx=14,bblly=44,bburx=250,bbury=230,width=5.cm]{./figures/ringSpiralMagDraw.eps}
...
\subsection*{\Large   Optics  in the spiral FFAG }

\large \sf 

The idea in the spiral FFAG was to superpose a positive field on top of the alternating sign one of the 
radial sector, so as to always have the right curvature and decrease the circumference factor. 
...
The logarythmic spiral edge %{\Large ~ ~ $r = r_0 \exp(Nw \theta)$~ ~}
 insures constant  angle between  spiral sector edges and closed orbits. 
...
\subsection*{\Large  Second radial sector, 50~MeV,  2-way}
...
\vspace{-70mm} \hspace{-20mm}\includegraphics[width=4.cm]{./figures/popMagnet.eps}
...
\multicolumn{2}{l}{\it  \underline{Acceleration}}&&                &  \fbox{ \bf broad band, high E RF} \\
      & swing      & MHz &   0.6 - 1.4      &  \\
      &harmonic    &     &          1       &  \\
...
\hspace{-10mm}\includegraphics[bbllx=20,bblly=120,bburx=567,bbury=480,width=9.00cm,height=4cm]{./figures/fieldOnCO.eps}
...
\section{\LARGE From 1964 to today's trends}
...
%&   K            &        &       $21$              & . so to insure $\gamma_{tr} > \gamma_{max}$   \\
...
\subsection*{\LARGE    Today's trends}

\large   


\begin{itemize}
 \item  {\it still } candidate for high energy (multi-GeV range), high power (MW range) proton beams
 \item  candidate for compact, low cost, low-energy machines, e (MeV range) and p (sub-GeV range) e.g., medical \\
\end{itemize}

Ingredients for that, nowadays,  are

 \begin{itemize}
%  \item  high repetition rate due to fixed field, $>100$~Hz 
  \item the technology has progressed~:  magnets (SC), RF ~ ~ ~  ~ ~ $\rightarrow$  see Y. Mori, today
  \item  new concepts, {non-scaling, semi-scaling  optics}  ~ ~ ~  ~ ~ $\rightarrow$  see S. Ruggiero, today
\end{itemize}
...
\hspace{-1cm}\includegraphics[angle=-90,width=8cm]{./figures/g3D-gnuplot.eps}
...
\section*{\LARGE Conclusion, back to the 50's : an e-model of a muon FFAG}
