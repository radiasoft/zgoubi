...
\paragraph{\textit{RECOVERING FROM A CRASH}}\index{OBJET!recovering from a crash}
...
<<<<<<< .mine
\noindent\textbf{KOBJ = 5.N~:} Like \textsl{KOBJ = 5}, except for  $N \geq 2$ references needed in this case 
(thus \textsl{N-1} additional input data lines), rather than just one. 
...
=======
\noindent\textbf{KOBJ = 5.N~:} Like \textsl{KOBJ = 5}, except for  $NN = 2 - 99$ references needed in this case 
...
\noindent  \textsl{MULTIPOL} can be injected field defects. 
...
\paragraph{\textit{CONSTRAINTS}} \index{constraint (FIT, FIT2)|textbf} \index{fit procedure!constraint|textbf}
...
\label{TabFITZlst1}
    {\renewcommand{\arraystretch}{1}
			\begin{tabular}{|>{\bfseries}p{\LL}|c|c|c|c|c|l|c|c|p{\LL}|}
			\hline
			\hline
         ...
         & & & & & \multicolumn{1}{c|}{\NP} & \multicolumn{1}{l|}{   $\PV_{1 - N\!P}$ value(s)} & \\
			\hline
                          & & & & & & &   \\
...
%
         ...
         & & &  \textsl{[MC]OBJET}   \\[0.4ex]
			 \multicolumn{1}{|c|}{\textbf{coordinates}}
			    &   &  $-1$      & $1 - 7$  &   $<\! F(J,i)\! >_{i=I1,I2}$ &  \hspace{-1.5ex} {\large $ \left\{ \right. $} \hspace{-3.ex}  $\begin{array}{l} 0\\[-.8ex] 2 \end{array} $ &  \hspace{-1.5ex} $\begin{array}{l} \\[-.8ex] I_1 \end{array}$     $\begin{array}{l} \\[-.8ex] I_2  \end{array} $  &  \scriptsize $\begin{array}{l} \Leftrightarrow 1,~\IMAX\\[-.2ex] 1\! \leq \! I_1 \! \leq \! I_2\! \leq\! \IMAX \end{array} $   \\[0.4ex]
...
\paragraph{\textit{OPTIONS} \index{fit procedure!options|textbf}}
...
\paragraph{\textit{RECOVERING FROM A CRASH}}\index{REBELOTE!recovering from a crash}
...
\noindent Used in conjunction with \textsl{REBELOTE\index{REBELOTE}} for the simulation of multi-turn tracking, 
 \textsl{SCALING} allows to change fields in an arbitrary, user defined manner during the ramp. 
Fields can as well be imposed to follow an acceleration ramp caused by \textsl{CAVITE}, 
turn-by-turn (for instance the main magnets), this can be obtained 
by simply using \textsl{NT=-1} as the first argument in \textsl{SCALING}. 
...
- if each instance addresses a different parameter in the optical elements concenred, both cases are applied, 

- if each instance addresses the same parameter in the optical elements, the last instance will prevail. 
...
\noindent\begin{tabular}{lll}
  ...
  \textsl{MULTIPOL}   &          &- All \textsl{MULTIPOL} keywords, with or without labels \\
  2                      &       &      $NT = 2$ timings  \\
...
\paragraph{\textit{RECOVERING FROM A CRASH}}\index{SCALING!recovering from a crash} \label{CrashSCALING}
...
\paragraph{\textit{CLOSED ORBIT COMPUTATION}}     \index{closed orbit!computation}  
...
\paragraph{\textit{RECOVERING FROM A CRASH}}\index{CAVITE!recovering from a crash} \label{CrashCAVITE}
...
\noindent The file name may be followed by up to two strings 'LBL\_A', 'LBL\_B'. 

- LBL\_A is the first label of some 
keyword in the include file, from which the include process will start (whatever precedes will be ignored), 
if no 'LBL\_A' is declared, the include process starts from the begining of the include file~; 

- LBL\_B is the first label of some 
keyword in the include file, at which the include process will end (whatever follows will be ignored), 
if no 'LBL\_B' is declared, the  include file will be included down to its end. 
...
to include all INCLUDE files), as a consequence the input file to zgoubi is no longer allowed the name zgoubi.dat 
if it contains  \textsl{INCLUDE}  keyword(s) (so to avoid changing the content of the user's 
input file). This is overcome by using the command 'zgoubi -inFile myFile' instead, with 'myFile'$\neq$'zgoubi.dat'. 
...
in the \textsl{common} folder). Instead, possible presence of \textsl{INCLUDE} in the input data file is dealt with by 
...
\bf IZ & ~ ~ & \bf         MOD   &\bf      .MOD2 &   & \\[1ex]
 1   & & 0 &     none                &  \multirow{2}{80mm}{2-D map, a single  data file for  $B_Z(X,Y)|_{Z=0}$, mid-plane symmetry.} \\[4ex]
$>1$ & & 0   &     none                &  \multirow{2}{80mm}{3-D map, 1+IZ/2 data files of upper half of magnet, one per (X,Y)$|_{0\leq Z\leq Z_{max}}$ plane, mid-plane symmetry.} \\[5ex]
&&        0     &    .1, .2, .3              &  As previous case, just  different reading formats. \\[3ex]
1 &&        1     &  none                 &  \multirow{2}{80mm}{2-D map, one mid-plane $B_Y(X,Y)$ data field map.} \\[4ex]
$>1$ &&        1     &  none              &  \multirow{2}{80mm}{3-D map, IZ files containing $B_{X,Y,Z}(X,Y,Z)$ data, one per (X,Y) plane, no symmetry assumed.} \\[5ex]
&&        1     &    .1, .2, .3              &  As previous case, just  different reading formats. \\[3ex]
1 &&        3     &     none \textsl{or} .1             &  \multirow{3}{80mm}{2-D AGS main magnet field map, mid-plane symmetry assumed. MOD2=1 causes field to be perturbed by $(1+n_1\,dY+n_2\, dY^2+n_3\, dY^3)$ factor.} \\[9ex]
...
$>1$ &&        15    &         .1$-$.6        &  \multirow{2}{80mm}{3-D map data file(s) (up to 6), whole magnet volume each (thus no symmetry assumed). Fields summed up~: at all node, $\vec B = \sum_{i=1}^{i=MOD2} a_i \, \vec B_i $. Each map file contains $B_{X,Y,Z}(X,Y,Z)$ data over  \IZ\ equally $Z$-spaced $(X,Y)$ planes}  \\[14ex]
...
in sequence, optical functions and  strengths at all elements. 
