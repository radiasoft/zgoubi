%%!TEX root = Zgoubi.tex
	...
	% \part{Description of software contents}{Description of software contents}
                                  ...
                                  \section*{PREFACE TO THE BNL EDITION, 2015+ } %\index{PREFACE}
...
AGS and RHIC studies, synchrotron radiation energy losses and their effects. 
	...
	\begin{aligned}
		...
		\vec u^{\,\prime\prime} 
		       & =  \left(\dfrac{1}{v}\right)^\prime \vec  E 
		            ...
		            \vert_{ \Br}  -2 \dfrac{(\Br )^\prime }{ \Br} 
		             \vec  u^{\,\prime} -
		             \dfrac{(\Br)^{\prime\prime}}{\Br}  \vec  u \\
	    \vec  u^{\,\prime\prime\prime} 
	           &  = \left( \dfrac{1}{v}\right)^{\prime\prime}\vec  E
	               +2\left(\dfrac{1}{v}\right)^\prime \vec  E^{\,\prime} 
	               \vert_{ \Br} +\dfrac{1}{v} \vec  E^{\,\prime\prime} 
	               \vert_{ \Br} -3 \dfrac{(\Br )^\prime }{\Br}  \vec  u^{\,\prime\prime} 
	               -3 \dfrac{(\Br )^{\prime\prime}}{\Br}  \vec u^{\,\prime} 
	               - \dfrac{(\Br )^{\prime\prime\prime} }{\Br}  \vec  u \\
		\vec  u^{\quaprime}
		      & =  \left(\dfrac{1}{v}\right)^{\prime\prime\prime}  \vec  E 
		           +3\left(\dfrac{1}{v}\right)^{\prime\prime}\vec E^{\,\prime} \vert_{ \Br} 
		           +3\left( \dfrac{1}{v}\right)^\prime \vec  E^{\,\prime\prime} \vert_{\Br} 
		           +\left(\dfrac{1}{v}\right)\,\vec  E^{\,\prime\prime\prime} \vert_{\Br} \\
		%
             ...
             \vec  u^{\cinqprime}
		      & =  \left(\dfrac{1}{v}\right)^{\quaprime}  \vec  E 
		           +4\left(\dfrac{1}{v}\right)^{\prime\prime\prime}\vec E^{\,\prime} \vert_{ \Br} 
		           +6\left(\dfrac{1}{v}\right)^{\prime\prime}\vec E^{\,\prime\prime} \vert_{ \Br} 
		           +4\left( \dfrac{1}{v}\right)^\prime \vec  E^{\,\prime\prime\prime} \vert_{\Br} 
		           +\left(\dfrac{1}{v}\right)\,\vec  E^{\,\quaprime} \vert_{\Br} \\
		%
...
\end{equation}
%
where $ \vec  E=\dfrac{\vec  e}{\Br} $, and $(~)^{(n)}\vert_{ \Br} $ 
denotes differentiation at constant $ \Br$~:  
$ \vec E^{(n)}\vert_{ \Br} =   \dfrac{ 1}{\Br} \dfrac{d^n\vec  e }{ds^n}$.  
...
\begin{equation}
	\begin{aligned}
		...
		\vec  u^{\,\prime\prime}  
		   &   =  \left(\dfrac{1 }{ v}\right)^\prime\vec  E 
		      + \left(\dfrac{1 }{ v}\right)\,\vec  E{^{\,\prime}} \vert_{ \Br} 
		      +(\vec u\times\vec  B)^\prime \vert_{ \Br}
		      -2 \dfrac{(\Br )^\prime }{ \Br} \vec  u^{\,\prime} 
		      - \dfrac{(\Br )^{\prime\prime} }{ \Br}  \vec  u \\
		\vec  u^{\,\prime\prime\prime} 
		   &  =   \left(\dfrac{1 }{v}\right)^{\prime\prime}\vec  E
		      +2 \left(\dfrac{1 }{ v}\right)^\prime\vec  E^{\,\prime} \vert_{ \Br}
		      +\dfrac{1 }{ v} \vec  E^{\,\prime\prime} \vert_{ \Br} 
		      +(\vec  u\times\vec B)^{\prime\prime} \vert_{ \Br}  
		...
		\vec  u^{\quaprime}  
		   & =    \left(\dfrac{1}{ v}\right)^{\prime\prime\prime}  \vec  E
		      +3 \left(\dfrac{1 }{ v}\right)^{\prime\prime}\vec E^{\,\prime} \vert_{ \Br} 
		      +3( \dfrac{1 }{ v})^\prime\vec  E^{\,\prime\prime} \vert_{\Br} 
		      +\left(\dfrac{1 }{ v}\right)\, \vec  E^{\,\prime\prime\prime} \vert_{ \Br} \\
		%
		  &  \quad + (\vec  u\times\vec B)^{\prime\prime\prime} \vert_{ \Br} 
...
$ \vec  B =\dfrac{\vec  b }{ \Br} $, and $^{(n)}\vert_{ \Br} $ denotes differentiation at constant $ \Br$ 
 \begin{equation}
	 \vec  E^{(n)}\vert_{ \Br} = \dfrac{1 }{ \Br}  \dfrac{d^n\vec  e }{ ds^n}
	 \quad \text{and} \quad 
	 (\vec  u\times\vec  B)^{(n)}\vert_{ \Br} = \dfrac{1}{\Br}  (\vec  u\times\vec  b)^{(n)}.
 	\label{eq2-2-20}
 \end{equation}
...
electric field, however, eq.~(\ref{eq2-2-Time}) reduces to the simple form 
%
 \begin{equation}
...
\subsection{Introduction \label{SecSpinIntro}}
...
\noindent The process is then completed by applying the $\Delta s$ push, eq.~(\ref{eq3-5-S}). 
 ...
 \subsection{Radiative polarization} \label{secRadiatPolar}

Radiative spin transition is accounted for in  a classical manner 
with an additional term in the Thomas-BMT differential equation for the motion of the polarization 
vector~\cite{SYLeeSpinDyn}, namely, see Eqs.~\ref{eq3-1-S},~\ref{eq3-2-S} (in the absence of electric fields), 
\begin{gather}
 \dfrac{d\vec  S }{ dt} = \dfrac{q}{  m} \, \left( \vec  S\times\vec  (1+ a \gamma)\vec  b + a(1-\gamma )\vec  b_{\paral} \right) 
    - \dfrac{1}{\tau_{ST}} \left(  \vec S - \dfrac{2}{9} (\vec S \cdot \vec u) \vec u + \dfrac{8}{5\sqrt{3}} \dfrac{\vec u \times \dot{\vec u}}{|\dot{\vec u}|}      \right)
	     \label{eqRadPolar}  
\end{gather}
where the electron notation for the magnetic anomaly, $a$, replaces hadron's $G$ (and $q=-e$ with $e$ the elementary charge,  
for electrons), 
$\vec u$ is the unit velocity vector of the particle and $\dot{\vec u} = d\vec u/dt = v \vec u'$ (eq.~\ref{eq2-2-2}). 
The Sokolov-Ternov polarization equilibrium  time constant has been introduced, 
\begin{equation}
\label{eqSokolovTernovTime}
\tau_{ST}= \dfrac{4}{5 \sqrt{3}} \dfrac{E}{\hbar \omega_c} \tau_0
\end{equation}
with $E=\gamma m_0$ the particle energy and 
 $\hbar \omega_c = h \times 3\gamma^3 c\, / \, 2\rho$  the critical photon energy. 

This radiative term is only added if the particle does radiate~\cite{SYLPrivComm2017}, 
that event being determined by the Monte Carlo photon emission simulation process described in Sec.~\ref{secSRLoss}. 
It has the effect of slightly changing the electron polarization, under the effect of radiation 
induced spin transition. 

This radiative correction process is under installation in \zgou. Its benchmarking includes comparison with statistical 
averages of quantum spin physics. 

As to solving the differential eq.~\ref{eqRadPolar}, the technique follows the \zgou\ method, namely, 
introducing derivatives with respect to the path variable $s$, and fields normalized to $B\rho$, 
\begin{gather}
 \dfrac{d\vec  S }{ ds} =  \left( \vec  S\times\vec  (1+ a \gamma)\vec  B + a(1-\gamma )\vec  B_{\paral} \right) 
    - \dfrac{1}{\tau_{ST}} \left(  \vec S - \dfrac{2}{9} (\vec S \cdot \vec \beta) \vec \beta + \dfrac{8}{5\sqrt{3}} \dfrac{\vec \beta \times \dot{\vec \beta}}{|\dot{\vec \beta}|}      \right)
	     \label{eqRadPolarSol}  
\end{gather}
...
The derivatives of the unit velocity vector $\vec u$ are outcomes of the solution of particle motion as 
detailed in Sec.~\ref{sec2.2}. The derivatives of the polarization vector are computed as detailed in ec.~\ref{SecSpinIntro}. 



 \subsection{Spin Diffusion} \label{secSpinDiff}
...
\section{SPACE CHARGE }\label{secSpach}\index{SPACECHARG|textbf} 
...
\zgoubi\ provides models for the  simulation of space charge effects. This capability 
has been first developped in the context of a PhD work regarding  
 fixed field ring proton driver design studies and accelerator-driven subcritical reactor 
applications. 
This section of the guide is essentially drawn from that PhD dissertation~\cite{MalekPhD}, 
which can be referred to for additional details -~the document provides benchmarking proofs 
in both KV and Gaussian density models (see below),   
 including bunch transport in a drift, transport in a beam line, bunch tracking in an FFAG ring 
represented using either \zgou's \textsl{FFAG} keyword or magnetic field maps. 

~

\centerline{TO BE DOCUMENTED FURTHER}
...
\section{ORBIT CORRECTION }\label{secSpach}\index{Orbit correction|textbf} 
...
\zgoubi\ provides an orbit correction method.  It is activate by means of \textsl{IOPT=2} in \textsl{REBELOTE}. 

One method only is implemented, SVD. The algorithm is the following~:

\noindent (i) initial orbit hypothesis:

- in the case of a periodic structure, first the periodic closed orbit is found

- in the case of a beam line, intial coordinates in OBJET are assumed that of the reference orbit


\noindent (ii) the algorithm will compute the SVD matrix by injecting a non-zero angle in all
orbit correctors, one by one, and save the BPMs readings

\noindent (iii) the SVD matrix will then be computed and stored in zgoubi.SVDMatrix.out.

\medskip

\noindent A typical way to use the orbit correction algorithm  is the following:

{\small
\begin{verbatim}
Title.
 'OBJET'                                                                                                      1
1000.000000     !    reference rigidity (kG.cm) =    299792458.0000  ,  G.gamma =            1.8821
2
1 1
0.000000E+00    0.000000E+00    0.000000E+00    0.000000E+00   0. 1. 'o'
1
! Start optical sequence here
...
! Some vertical corrector beloging in 'VKIV' family
 'MULTIPOL' VKIV
[MULTIPOL data list]
...
! Some zero-length horizontal BPM beloging in 'HMON' family
 'DRIFT'    HMON   
0.0000
...
! End optical sequence here
...
1 33 0 [-.1,.1]   ! Vary P0
4   
3.1 1 2 #End 0. 1. 0    ! Yfinal = Y0
3.1 1 2 #End 0. 1. 0    ! Tfinal = T0
3.1 1 2 #End 0. 1. 0    ! Zfinal = Z0
3.1 1 2 #End 0. 1. 0    ! Pfinal = P0
 
 'REBELOTE'                                      
258  0.1  0  2  ! NRBL = numb. of correctors: 78 'MONI', 87 'HMON', 93 'VMON',  total=258
HMON MONI       ! PUH names (max is 5)                             
HKIC  1e-5      ! corrH name, kick (rad)
VMON MONI       ! PUV names (max is 5)
VKIC  1e-5      ! corrV names, kick (rad)
 'END'                                           
\end{verbatim}
}
...
\noindent Once the \textsl{REBELOTE} algorithm above is completed, the SVD matrix is then known (stored in zgoubi.SVDMatrix.out),
defects can be applied (activated for instance from within zgoubi using \textsl{ERRORS} keyword).
The new orbit is then generated by zgoubi, its value at all BPMs of concern is  then read,
and the SVD matrix (read from zgoubi.SVDMatrix.out) is applied to all correctors...
this should take care of minimozing the orbit.

\medskip

\noindent The process requires two zgoubi  data  files for the optical sequence of concern, as follows~:


- the first zgoubi data file to be used is as shown above: it runs \textsl{REBELOTE} with \textsl{IOPT=2}
and gets teh SVD matrix.

- the second file, with the same sequence, activates \textsl{ERRORS}, computes the resulting orbit,
sets the corrector strengths using the SVD matrix, re-computes the resulting orbit -~presumably improved from
the initial, uncorrected one~! and delivers it. 
...
\noindent\textbf{KOBJ  = 3~:} Distribution of \IMAX\ particles in a
6-D ellipsoid (with possible momentum correlation via dispersion) defined by the three sets of data (one set per 
 2-D phase-space)
...
$$            
%
where $\alpha_{X,Y,Z}$, $\beta_{X,Y,Z}$, $D_{Y,T}$, $D_{Z,P}$ are the ellipse parameters and the dispersions and derivatives,  
 $\varepsilon_{X,Y,Z}/ \pi$ are the \rms\ emittances. 
This defines for each plane an elliptical  frontier~:
$$\dfrac{1 + \alpha^2_Y}{\beta_Y} Y^2 + 2 \alpha_Y YT + \beta_Y T^2 = \varepsilon_Y / \pi, $$ 
$$\dfrac{1 + \alpha^2_Z}{\beta_Z} Z^2 + 2 \alpha_Z ZP + \beta_Z P^2 = \varepsilon_Z / \pi, $$ 
$$\dfrac{1 + \alpha^2_l}{\beta_l} l^2 + 2 \alpha_l l\delta + \beta_l \delta^2 = \varepsilon_l / \pi, $$ 
with $l$ the bunch length variable (unit is cm) and $\delta$ a short notation for $\delta p/p$, momentum spread. 

\noindent Sorting cut-offs $N_{\varepsilon_ Y}$, $N_{\varepsilon_ Z}$ and $N_{\varepsilon_ X}$ are accounted for if 
distributions are Gaussian ($KY, KT, ...=2$)~; in this case \textsl{rms} values are 
$$\sigma_Y = \sqrt{\beta_Y \times \varepsilon_Y / \pi} ~ \textrm{ and}~ \sigma_T = \sqrt{\gamma_Y \times \varepsilon_Y / \pi},$$ 
$$\sigma_Z = \sqrt{\beta_Z \times \varepsilon_Z / \pi} ~\textrm{and}~ \sigma_P = \sqrt{\gamma_Z \times \varepsilon_Z / \pi},$$
$$\sigma_l = \sqrt{\beta_l \times \varepsilon_l / \pi} ~ \textrm{(bunch ~length, ~unit ~cm), ~and} ~  \sigma_{\delta} = \sqrt{\gamma_l \times \varepsilon_l / \pi} ~\textrm{momentum ~ spread}.$$

\noindent For uniform sorting, the distribution extends over 
the intervals, for respectively the radial, axial and longitudinal distribution, 
$$\pm \sqrt{\varepsilon_Y / \pi \times \beta_Y} ~ \textrm{and} ~ \pm \sqrt{ \varepsilon_Y / \pi \times \gamma_Y},$$
$$\pm \sqrt{\varepsilon_Z / \pi \times \beta_Z} ~ \textrm{and} ~ \pm \sqrt{ \varepsilon_Z / \pi \times \gamma_Z},$$
$$\pm \sqrt{\varepsilon_l / \pi \times \beta_l} ~ \textrm{and} ~ \pm \sqrt{ \varepsilon_l / \pi \times \gamma_l}.$$

\noindent  $\bullet ~ N_\varepsilon$ may be negative. The meaning of this is for  the sorting to fill up 
  an elliptical ring that extends from $\left| N_\varepsilon \right|$ to 
$N'_\varepsilon$ (rather than the inner region determined by the $N_\varepsilon$ 
cut-off as discussed above, when $N_\varepsilon>0$).
...
%%%%%%%%%%%%%%figure %%%%%%%%%%%%%
 \begin{figure}[H]
  % \vspace*{19 truecm}
\centerline{\includegraphics[width=12cm,angle=-90]{Fig5.ps}}  %% now 6
...
be non-compatible with the use of some procedures (\emph{e.g.}~\textsl{MCDESINT\index{MCDESINT}}, which 
involves random processes) or with the use of non mid-plane symmetric fields.
...
\noindent\textbf{KOBJ = 1.1~:} The initial $ Z $ and $ P $ conditions are 
$$  \begin{array}{*{5}{r<{,}}}
...
\noindent\textbf{KOBJ = 1.2~:} Same as \KOBJ = 1, but the mid-plane symmetry of the tracking is lifted, all
...
\noindent Note that in this option  \textbf{KOBJ =  2[.]]}, only the first 7 trajectories  can possibly have their coodinates varied when using the \textsl{FIT[2]} procedure. 
...
\paragraph{\textsl{RECOVERING FROM A CRASH}}\index{OBJET!- recovering from a crash}\index{recovering from a crash}

\noindent \textsl{OBJET} with option \KOBJ=3 can be used to recover from a job 
crash, which would occur when multi-turn tracking  -~due to computer crash or else. This is discussed in the 
...
\noindent Note that in the option  \textsl{\KOBJ = 5.N, $N \geq 2$}, only the first 7 reference trajectories  can possibly have their coodinates varied when using the \textsl{FIT[2]} procedure. 
...
- \textsl{FIT\index{FIT[2]}}~: fitting procedure - can also appear before \textsl{REBELOTE}
...
\noindent  \textsl{MULTIPOL} can be injected multipole field defects on any one of its dipole to 20-pole component.
The defect can be relative (to the current field value) or absolute (adds to the current field value). 
...
- if \textsl{REBELOTE} is used in multi-turn mode (\ie, its $K$ argument value is $99$), then the same defect series is 
...
requires modification in \texttt{errors.f} program.
...
\subsubsection{FIT, FIT2~:  \FITTitl}  \label{FIT}  \index{FIT[2]|textbf} 
\medskip

 The keywords \textsl{FIT, FIT2} allow the automatic adjustment of up to 20 
variables\index{variable (FIT, FIT2)}, for fitting up to 20 constraints. 
...
\noindent The first input data in \textsl{FIT[2]}\index{FIT[2]} is the number of variables
...
\paragraph{Numbering of the Elements (\textsl{IR})~:   \index{FIT[2]!- numbering, keywords}} 
...
the \textsl{FIT[2]\index{FIT[2]}} procedure uses. 
...
\paragraph{Numbering of the Physical Parameters (\textsl{IP})~:  \index{FIT[2]!- numbering, parameters}} 
...
\renewcommand{\arraystretch}{1}
   ...
   \textsl{XPAS}      && 50    \\
   if \textsl{KPOS=1}~: \\[-.4ex]
   \textsl{KPOS, XCE, YCE, ALE }     && 60,61,62,63 \\
   if \textsl{KPOS=2}~: \\[-.4ex]
   \textsl{KPOS }     && 60    \\
   \textsl{KPOS, XCE, YCE, ALE }     && 70,71,72,73
\end{tabular}
...
\noindent A different numbering,  fully sequential, has been adopted in the following elements~: \\[1ex]
...
\textsl{MULTIPOL}. \index{MULTIPOL!- parameter numbering} \\[1ex]
It is  illustrated  here after 
in the case of \textsl{MULTIPOL} and \textsl{DIPOLE-M}. \index{DIPOLE-M!- parameter numbering} \\[1ex]
\begin{minipage}{0.99\linewidth}
\begin{center}
{\renewcommand{\arraystretch}{1}
	\begin{tabular}{lcl}
...
\paragraph{Coupled Variables  ($XC$)  \index{FIT[2]!- coupled variables|textbf} } 
...
variable declared in the data list of the \textsl{FIT[2]\index{FIT[2]}} keyword, needs not appear 
...
\paragraph{Variable Variation Range  \index{FIT[2]!- variable values!- range|textbf}} 
...
\paragraph{\textsl{CONSTRAINTS} \index{FIT[2]!- constraint|textbf} }
...
\begin{center}
	...
	\label{RefIR}
        $\IR$       &  =  &  number of the keyword at the exit of which the constraint applies~;  instead of numeral, \\[-.3ex]
                    &     & can also be '\#End', so the constraint applies at the last keyword preceeding '\textsl{FIT[2]}' \\
...
\noindent  {\it IC=10~: } If $1\leq I\leq \IMAX$ then the constrint is the value of coordinate type $J$ ($J=1,3$ for respectively 
 $S_X, S_Y, S_Z$) of particle number $I$.
...
\noindent  {\it IC=10.2~: } 
Constraint on spin rotation angle for particles of momentum group \#K.
   This requires \textsl{OBJET/KOBJ=2}, with groups of 3 particles, 
             all particles in a group have the same momentum, and their spins in direction, respectively, X, Y, Z.

\smallskip

\noindent  {\it IC=10.3~: } 
Constraint on spin rotation axis for particles of momentum group \#K.
   This requires \textsl{OBJET/KOBJ=2}, with groups of 3 particles, 
             all particles in a group have the same momentum, and their spins in direction, respectively, X, Y, Z.
...
\index{REBELOTE!- combined with FIT[2]}   
\index{FIT[2]!- combined with REBELOTE}   
\label{FITREB}

\noindent \textsl{FIT[2]} may be followed by the keyword  \textsl{REBELOTE}.  This allows for instance executing again 
the  fitting procedure, following a change, by  \textsl{REBELOTE}, in  the value of some parameter(s) in zgoubi.dat data list. 
More on that can be found under \textsl{REBELOTE}, Sec.~\ref{REBELOTE}, page~\pageref{REBFIT}. 

\medskip

\noindent An example is given page~\pageref{ExaFITREBELOTE}. 

\medskip

\noindent Conversely,  \textsl{REBELOTE} may be followed by the keyword \textsl{FIT[2]}.  This allows for instance 
optimization of constraints that require multi-turn tracking (\eg, optimized tune-jump quadrupole settings for beam 
acceleration through depolarizing resonances). 


\bigskip

\paragraph{\textsl{FIT[2] OPTIONS} \index{FIT[2]!- options|textbf}}

\noindent \textsl{FIT} and \textsl{FIT2} accept various options, as follows. 

\smallskip

\noindent - 'nofinal' option~: \label{nofinal} \index{FIT[2]!- no final run}
...
not appear in the   \textsl{FIT[2]} data list, then that final run does occur. This is necessary in particular
when it is desired to store resulting particle data, using \textsl{FAISTORE}, at the sole very final pass. 
\index{FAISTORE!- in presence of \textsl{FIT[2]}}
\index{FIT[2]!- in presence of FAISTORE}
...
default in zgoubi.FITVALS.out, or in \textsl{FileName} file if the latter is specified following the `save' instruction.\index{FIT[2]!- variable values!- save|textbf}

\smallskip

\noindent - Penalty~: value of the penalty to be reached, following what the  \textsl{FIT[2]} procedure 
will be considered completed, and thus  stopped. \index{FIT[2]!- penalty|textbf}
...
\noindent - ITER~: maximum allowed number of ``calls to the function'' (namely, (recursive) calls to zgoubi). Once that number is reached, 
 \textsl{FIT[2]} is stopped, regardless of a possible ``penalty'' value. \index{FIT[2]!- max. number of iterations|textbf}
...
\subsubsection{GETFITVAL~: \GETFITVALTitl \label{GETFITVAL} \index{GETFITVAL|textbf} \index{FIT[2]!- variable values!- get|textbf}}
\medskip
...
This can be inhibited by indicating `nofinal' option\index{FIT[2]!- options}  
...
\label{MCDESINT}\index{MCDESINT|textbf}\index{Monte Carlo}
 ...
 When \textsl{MCDESINT}  is met in a structure
...
and \textsl{COM} lifetime $\tau_1$ of the parent particle. 
...
rigidity, defined in \textsl{[MC]OBJET\index{OBJET}\index{MCOBJET}}).
%,  and position $ Y_1,Z_1 $ in the \zgou\ frame. 
...
\noindent The average distance   $ s_1 $ at which a particle will decay is related to its center of 
mass lifetime $ \tau_ 1 $ by $$ s_1=c\tau_ 1 \sqrt{ \gamma^ 2_1 -1} $$
The actual path length $ s $ up to the decay point is calculated, 
for each one of the particles defined by \textsl{[MC]OBJET} and prior t ray-tracing, 
from a random number $ 0<R_1\leq 1 $ by using the exponential decay formula
$$ s = - s_1 \ell n R_1 $$

\noindent After decay of the parent particle~1, particle~2  will  be ray-traced with assumed positive
...
$M_2 $ is the very location of  the parent particle decay, $ (Y_1 , Z_1, s_1) $.   
...
\subsubsection{OPTICS~: \OPTICSTitl} \label{OPTICS} \index{OPTICS|textbf} \index{MATRIX!- beam matrix}
...
\noindent PRINT~: 
 An optional third argument, \textsl{IMP=1}, or equivalently the command 'PRINT'\index{PRINT!- OPTICS}, 
...
``WRITE ~ ~   ON'' sets back to normal (the 'normal' state of write outs may depend on 
the status of other commands, such as \textsl{REBELOTE}, on-going \textsl{FIT[2]}, etc.). 

~

\noindent - ``CONSTY ~ ~   ON or OFF'' \index{OPTIONS!- CONSTY}

Role~: force to constant \textsl{Y} and \textsl{Z} coordinates during stepwise integration. 

This allows for instance checking fields, or  fabricating field maps from 
\zgou's analytical models of optical elements, as follows.

\smallskip 

\noindent Concurrent use of 
with $\IL=1, ~ 2, ~ \textrm{or} ~ 7 $ (\cf\ Sec.~\ref{sec4.6.1}, p.~\pageref{sec4.6.1}) will allow logging 
 $\vec B$ and/or $\vec E$ field(s) (and their derivatives) 
experienced across optical elements, in zgoubi.res, zgoubi.plt or zgoubi.impdev.out respectively. 

\smallskip 

\noindent As a consequence, forcing  \textsl{Y} and \textsl{Z} to constant values, using  ``CONSTY ~ ~   ON'', 
allows checking of fields (and derivatives) at constant \textsl{Y} and \textsl{Z}, as determined by 
\textsl{OBJET},  across 
(individual or short series of) optical elements. 
This may be helpful when setting up input data in \zgou. It can be for instance of great help 
...
radius value in the case of optical elements defined in polar coordinates (\eg, \textsl{DIPOLES}, polar field maps, etc.).  
...
\noindent  An example of \textsl{FIT} preceding \textsl{REBELOTE} is given page~\pageref{ExaFITREBELOTE}. 

\bigskip


\paragraph{\textsl{RECOVERING FROM A CRASH}}\index{REBELOTE!- recovering from a crash}\index{recovering from a crash}
...
from the latest records (in [b\_]zboubi.fai storage file),  using \textsl{OBJET}, \KOBJ = 3.
...
{\renewcommand{\arraystretch}{1}
\noindent	\begin{tabular}{lll}
\textsl{MULTIPOL} \textsl{COH1}  & & Scaling applies to all multipole field components, \\[-.6ex]
               & & in \textsl{MULTIPOL} keywords having '\textsl{COH1}' as first label. \\
1.10 \textsl{[,FAC]}    &   & Scaling is read from a file, file name \textsl{e.g.} 'scalingFile'; FAC       \\[-.6ex]
                         & & is an additional scaling factor. \\
scalingFile & ~ & File name (file is expected to contain at least N2 columns). \\
1   N  &     &  Column number~N2 gives the scaling factor at rigidity values given in  \\[-.6ex]
       &      &   column number~1; current scaling is obtained by interpolation in this table \\[-.6ex]
       & & (and multiplied by \textsl{FAC} if specified). \\
	\end{tabular}   }
}

\medskip

 \noindent $\bullet ~ NT = 1.11$~: TO BE DOCUMENTED. 
...
\medskip
 ...
 \noindent $\bullet ~ NT = 1.12$~: allows taking the scaling law from an external data file, as in the following example~:
...
{\renewcommand{\arraystretch}{1}
\noindent	\begin{tabular}{lll}
\textsl{AGSQUAD}  \textsl{QH\_A17} & & Scaling applies to \textsl{AGSQUAD} keyword having '\textsl{COH1}' as first label. \\[-.6ex]
1.12       &   & Scaling is read from a file, file name \textsl{e.g.} 'scalingFile'.      \\
scalingFile & ~ & File name (file is expected to contain at least 2+2*N2 columns). \\
N1, N2, p11, p12,  &     &  N1=number of pairs, see next;  N2=  ; \\[-.6ex]
 p21, p22, ...      &      &  Scaling is obtained from interpolation in column pi1 and applied to parameter pi2 of \textsl{AGSQUAD} . \\[-.6ex]
      &      &  . \\
	\end{tabular}   }
}
...
\medskip
 ...
 \noindent $\bullet ~ NT = 1.13$~: TO BE DOCUMENTED. 
...
\paragraph{\textsl{RECOVERING FROM A CRASH}}\index{SCALING!- recovering from a crash} \label{CrashSCALING}\index{recovering from a crash}
...
Now, if the job  stops during the multiturn tracking -~due to computer crash or else~-  it is possible to resume the tracking 
from the latest records (in [b\_]zboubi.fai storage file),  using \textsl{OBJET}, \KOBJ = 3.
In the presence of acceleration using \textsl{CAVITE},  since there is no reference particle in \zgou, 
  \textsl{CAVITE} will read from [b\_]zboubi.fai the reference rigidity at the corresponding time/turn when tracking crashed/resumes. 
Note that, as far as \textsl{CAVITE} is concerned, this recovery process in only installed for \textsl{CAVITE/Option=2},
a option often used for synchronous acceleration
(recovery for additional \textsl{CAVITE} options can be added in \texttt{cavite.f} program). 


\newpage

\subsubsection{SPACECHARG~: \SPACECHARGTitl}\label{SPACECHARG}\index{SPACECHARG|textbf}
\medskip 

\noindent The keyword \textsl{SPACECHARG} enables (or disables) the calculation of 
space charge effects.
It must be 
preceded by \textsl{PARTICUL}\index{PARTICUL} for definition of  mass and charge values, as they enter 
in the definition of space charge parameters. 

\bigskip

\noindent \textsl{SPACECHARG} is supposed to appear a first time at the 
location where space charge effects should start being taken into account, 
with the first data \textsl{LMNT} set 
to~'all' or a keyword. 
...
\noindent\textsl{SPACECHARG} is supposed to appear a second time at the 
location where calculations should stop, with \textsl{LMNT} set 
to~'none'. It results in summary  outputs in zgoubi.res. 

\bigskip

\noindent Occurence of \textsl{PRINT} amongst \textsl{SPACECHARG}  arguments 
 results in storage of computational data and 
other relevant informations in zgoubi.SPACECHARG.out, as space charge computation 
proceeds. 
...
\noindent\textbf{KSO = 4.2~:} \textsl{under development}. Read spin components from a file, file name has to be provided. 
(that file at present can be zgoubi.res~:  KSO=4.2 looks for key ``(deg)       (deg)`` that appears 
at start of the spin coordinates list under the keyword \textsl{SPNPRT}). 
\medskip
...
\hangcaption[Fig7]{\label{fig7}Spin distribution as obtained with option \textsl{KSO} = 5.\\
         ...
         axis of mean polarization ($\vec S$) defined by $T_0$  and $P_0$.} 
}\end{figure}
%%%%%%%%%%%%%%figure%%%%%%%%%%%%%%
...
\medskip 
\index{synchrotron radiation energy loss}
...
\textsl{BEAMBEAM} is a beam-beam lens simulation, a thin-lens transform~\cite{BBSW}, 
 in the weak-strong approximation. 
...
\noindent \texttt{rbb.f} reads the data from zgoubi.dat;
\texttt{bb.f} and programs therein do the beam (and spin when requested) dynamics.
...
\subsubsection*{BETATRON~: \BETATRONTitl} \label{BETATRON}\index{BETATRON|textbf}
\medskip

\textsl{BETATRON} simulates a betatron yoke, for instance for slow extraction. It just imparts the particle
a momentum kick, normally small.
...
$A$, $B$, $C$, etc.  \index{integration boundary}
...
\noindent Entrance and/or exit integration boundaries (so-called ``droite de coupure'' in the code) 
\index{droite de coupure: see integration boundary} \index{integration boundary}
can be defined with the flag $ID$\index{ID@{\textsl{ID}}|textbf}, as follows (Fig.~\ref{fig15}).
\bigskip
...
It can also be used to simulate recirculating linacs and energy-recovery linacs. 
...
\begin{itemize}
\item[\footnotesize $\bullet$]  \textbf{If $\mathbf{IOPT  =  0}$~:}   \textsl{CAVITE} is switched off.  
\medskip

\item[\footnotesize $\bullet$] \textbf{If $\mathbf{IOPT  =  1}$~:}  \textsl{CAVITE} simulates the RF~cavity of a 
...
structure, otherwise its phasing has to be provided. 
...
\noindent  Finally, the initial conditions for the mapping, at the first turn, are the following  
%
\begin{itemize}
...
\item[\footnotesize $\bullet$] \textbf{If $\mathbf{IOPT  =  2~:}$} the same simulation of a synchrotron RF cavity 
...
\noindent where the synchronous phase $ \phi_ s $ is part of the input data to \textsl{CAVITE}.
...
\index{double RF system} \index{CAVITE!- double RF system} 
\noindent \textsl{IOPT  =  2} handles double RF systems. Respective harmonic value and voltage amplitudes are $h_1$, $h_2$ 
and $V_1$, $V_2$.
It follows CERN ISR-TH-RF/80-26~\cite{ISRTHRF8026}. In particular, the stable phase $\phi_n$ of the higher harmonic
voltage is taken to be 
$$\rm \phi_n = atan (\tan (\phi_s)/n) / n$$
wherein    $n = h_2 / h_1$. The synchronous energy gain is
$$      \Delta W_s = q\hat V\, ( \sin(\phi_s)  + k \sin(n \phi_n))$$
with    $k = V_2 / V_1$. 

\medskip

\noindent The option \textsl{IOPT  =  2}  handles synchrotron radiation energy loss 
in the case of stationary bucket (see \textsl{IOPT  =  11} for accelerating bucket).
 \index{synchrotron radiation energy loss!- treatment in CAVITE} 
Compensation is derived from the  synchronous phase $\phi_s$, via
$ \Delta W_{SR}= q\hat  V \,\sin\phi_ s $, therefore assuming appropriate value

$$\phi_s = {\rm asin}(\Delta W_{SR} / q\hat  V)$$

\noindent with $\Delta W_{SR} = $ the energy loss between passes in the cavity. 


\medskip

\item[\footnotesize $\bullet$] \textbf{If $\mathbf{IOPT  =  3}$~:} sine RF law, acceleration without synchrotron motion. 
...
\noindent The option \textsl{IOPT  =  3}  handles synchrotron radiation energy loss 
in the case of stationary bucket (as in  \textsl{IOPT  =  2}). 
 \index{synchrotron radiation energy loss!- treatment in CAVITE} 


\medskip

\item[\footnotesize $\bullet$] \textbf{If $\mathbf{IOPT  =  4}$~:} to be documented.

\medskip

\item[\footnotesize $\bullet$] \textbf{If $\mathbf{IOPT  =  5}$~:} to be documented.

\medskip

\item[\footnotesize $\bullet$] \textbf{If $\mathbf{IOPT  =  6}$~:}  
...
\item[\footnotesize $\bullet$] \textbf{If $\mathbf{IOPT  =  7}$~:} fixed frequency RF, quasi- or isochronous acceleration. Was 
...
\item[\footnotesize $\bullet$] \textbf{If $\mathbf{IOPT  =  8}$~:} to be documented.

\medskip

\item[\footnotesize $\bullet$] \textbf{If $\mathbf{IOPT  =  9}$~:} to be documented.

\medskip


\item[\footnotesize $\bullet$] \textbf{If $\mathbf{IOPT  =  10}$~:} fixed frequency RF. Was 
...
of the particles (linearly). An option ('\text{IOP}' flag) allows small $\Delta W$ approximation of Chambers matrices, as 
well as removing the longitudinal damping (thus geometrical emittance is conserved), as follows~: 

\begin{itemize}
\item[-] \text{IOP} = 1~:  Chambers cavity with  $\Delta W/W$ aproximation,  namely, for both planes, 
\begin{equation}
...
\end{equation}
\noindent with   $W_i$, $ W_o$ respectively the incoming and outgoing kinetic energies. 

\item[-] \text{IOP} = -1~: Same as 1, with matrix in both planes re-normalized  to determinant=1 
(\ie, $R_{ij} \rightarrow R_{ij} /\sqrt{R_{11}R_{12} - R_{21}R_{22}}$),

\item[-] \text{IOP} = 2~: Chambers cavity without aproximation on  $\Delta W/W$, namely, for both planes, 
\begin{equation}
      \left(  \begin{array}{c} x \\ x'  \end{array} \right)_{out} = 
      \left(  \begin{array}{cc} 
            \cos(u)-\sqrt{2} \sin(u) \cos(\phi) &   v W_i  \sin(u) \cos(\phi)  \\
            -\frac{\textstyle{\sin(u)}}{\textstyle{v W_o}} (2 \cos(\phi)+ \frac{\textstyle{1}}{\textstyle{\cos(\phi)}}) & \frac{\textstyle{W_i}}{\textstyle{W_o}} (\cos(u)+\sqrt{2} \sin(u) \cos(\phi))
      \end{array} \right)
      \left(  \begin{array}{c} x \\ x'  \end{array} \right)_{in} 
\end{equation}
\noindent with   $u=\log(W_o/W_i)/(\sqrt{8} \cos(\phi))$, $v=\sqrt{8} L/(W_o-W_i)$, $\phi$  the particle phase at the cavity. 

\item[-] \text{IOP} = -2~: Same as 1, with matrix in both planes re-normalized  to determinant=1.

\item[-] \text{IOP} = 0~: Transverse matrix is that of a drift with length $L$.
\end{itemize}

\item[\footnotesize $\bullet$] \textbf{If $\mathbf{IOPT  =  11}$~:} under development. 

 The same simulation of a synchrotron RF cavity 
as for $\mathbf{IOPT  =  2}$ is performed, but in addition 
 \textsl{IOPT  =  10}  handles synchrotron radiation energy loss, both stationnary or accelerated bucket. 
 \index{synchrotron radiation energy loss!- treatment in CAVITE} 
...
\paragraph{\textsl{RECOVERING FROM A CRASH}}\index{CAVITE!- recovering from a crash} \label{CrashCAVITE}\index{recovering from a crash}
...
\noindent PRINT~: 
 The optional command 'PRINT'\index{PRINT!- CAVITE}, following the first argument, \textsl{'IOPT'}, 
...
file \index{zgoubi.CAVITE.out|textbf}zgoubi.CAVITE.Out.

\bigskip

\paragraph{\textsl{For software developers}}  

\noindent Cavity simulations are done in \texttt{cavite.f}. Management of the reference rigidity 
$B\rho_{ref}$ is ensured for part in \texttt{cavite.f}, and for part in routines associated with '\textsl{SCALING}', 
essentially \texttt{scaler.f}. 
...
\noindent Entrance and/or exit integration boundaries \index{integration boundary}
...
\subsubsection*{FFAG~: \FFAGTitl~\cite{reportNIMFFAG,reportICFAFFAG}} \label{FFAG} \index{FFAG magnet!- radial|textbf} \index{FFAG|textbf} 
\medskip
...
\subsubsection*{FFAG-SPI~: \FFAGSPITitl~\cite{reportICFAFFAG,reportNIMFFAGSPI}} \label{FFAG-SPI} \index{FFAG magnet!- spiral|textbf} \index{FFAG-SPI|textbf} 
\medskip
...
it allows a fragment of an input data list to be taken from a separate file~; 
when  \textsl{INCLUDE} is met in the input data file, the content of the separate file, 
or part of it, is inserted in the data sequence. 
...
\noindent The file name may be followed by '\textsl{[LBL\_1A,LBL\_2A:LBL\_1B,LBL\_2B]}', with any of these four 
labels optional, as follows~: 

- LBL\_1A  [LBL\_2A] identify to the first (and possibly second) label of some 
...
- LBL\_1B [LBL\_2B]  identify to the first (and second) label of some 
...
and as  the presence of \textsl{INCLUDE} will cause the input data list to be changed (it is expanded  
to include the content of all included files),  the user's input data file is no longer allowed the name zgoubi.dat 
if it contains  \textsl{INCLUDE}  keyword(s) (zgoubi.dat is reserved as the name of the expanded
input file to zgoubi). This is overcome by using the command 'zgoubi -fileIn myFile' instead, with myFile$\neq$zgoubi.dat 
...
prdata program (\texttt{prdata.f} source file) in a simple way~: the input data file is expanded so eliminating 
...
\noindent An example of an input data file of a recirculating ring, structured with \textsl{INCLUDE} statements, 
can be found in page~\pageref{ExaINCLUDE-GOTO}. 
...
\noindent Entrance and/or   \index{integration boundary} 
...
\noindent {\bf Method~1~:}   uses the on-axis field model
\begin{equation}
\label{EqSol}
B_X(X,r=0) = \dfrac{B_0 }{2 } \left[ \dfrac{\XL/2-X}{\sqrt{(\XL/2-X)^2+R_0^2}} 
+  \dfrac{\XL/2+X}{\sqrt{(\XL/2+X)^2+R_0^2}} \right]
\end{equation}
with $X=r=0$ taken at the center of the  solenoid.
This  model assumes  that the coil thickness is small  compared to the mean radius $ R_0 $. 
In this model, the magnetic length is 
\begin{equation}
\label{EqSol2}
...
$$B_X(\textrm{center}) \equiv B_X(X=r=0) = 
      \dfrac{\mu_0 N I }{\XL\sqrt{1+\dfrac{4R_0^2}{\XL^2}}} \ \ \stackrel{R_0 \ll XL}{\xrightarrow{\hspace{6ex}}} \ \ \dfrac{\mu_0 N I }{\XL} $$ 

\medskip

\noindent From  eq.~\ref{EqSol}, 
 the field  and its derivatives at all $(X,Y,Z)$ are extrapolated, following the method 
...
\noindent {\bf Method~2~:} computes  $ \vec  B(X,r)$ (with $r=(Y^2+Z^2)^{1/2} $) and its derivatives up to 
second order  at all $(X,Y,Z)$ following the technique in Ref.~\cite{Biblio17}, %%% [17]
...
%\vspace{14 truecm}
%%%Figure 30
\centerline{\includegraphics[height=10cm,angle=-90]{Fig30.ps}}
...
\noindent Solenoid parameters are read from zgoubi.dat by \texttt{rsolen.f}. Initializations prior to tracking done in \texttt{soleno.f}.  Field computation during tracking is in \texttt{solenf.f}. 
...
\noindent \textbf{If $\mathbf{IOPT  =  1}$~:} the precession is defined by its axis, at angle  $\phi$ with 
respect to \zgou's X-axis and assumed in the horizontal plane,  and by its value, $\mu$. 

\bigskip

\noindent \textbf{If $\mathbf{IOPT  =  2}$~:}  given the spin precession axis direction $\phi$ in the (X,Y) plane, 
...
\subsubsection*{TOSCA~: \TOSCATitl\ \rm (page~\pageref{TOSCA-B} in PART~B)} \label{TOSCA} \index{TOSCA|textbf}
...
(transverse horizontal, vertical) nodes of the 3-D uniform mesh. For letting \zgou\ know in case these are binary files, 
...
field and its derivatives are calculated as follows~:
...
\noindent In the Cartesian mesh case, entrance and/or exit integration boundaries 
\index{integration boundary}
between which the trajectories
are integrated in the field may be defined, in the same way as in  \textsl{CARTEMES\index{CARTEMES}}. 

\medskip  

\noindent A '\textsl{TITL}' (a line of comments) is part  of the arguments of the keyword \textsl{TOSCA}. 
It allows introducing options, for instance~:     

$\diamond$ including `HEADER n'  allows specifying the number of header lines ('n' non-data lines) 
at the top of the field map file, 

$\diamond$ including `FLIP' in \textsl{TITL} causes the field map to be X-flipped, 

$\diamond$ including  `ZroBXY'  forces $B_X=B_Y=0$ at all Z=0 nodes of the field map mesh (only applies with 
\textsl{MOD}=15 and \textsl{MOD}=24), 

$\diamond$ including  `RHIC\_helix' will normalize \textsl{BNORM} to measured B field value  
  versus helix current (measured B(I) data are hard-coded in program \texttt{toscac.f}). In that case, 
\textsl{BNORM} may be given the current value, in Amps, while a(1) in mode 
\textsl{MOD.MOD2=15.1} is  used to normalize the field map (namely, $a(1)=1/B_{max}$ with $B_{max}$ the
maximum field value on helix axis). 
...
\noindent $\bullet$  \textsl{FAISCNL} is an older, less performing, version of \textsl{FAISTORE}, see below for the latter.
It produces a lot more information on particles at current location, including 
...
\noindent $\bullet$  \textsl{FAISTORE} has an effect similar to \textsl{FAISCNL}, with more features. 
...
\noindent For instance the following data list in zgoubi.dat~: 

{\small
\begin{verbatim}
	FAISTORE
	zgoubi.fai    HPCKUP   VPCKUP
	12 
\end{verbatim}
}
...
\noindent  \textbf{Case of Binary storage~:} \index{FAISTORE!- binary storage}
it can be obtained from \textsl{FAISCNL} and \textsl{FAISTORE}. This is for 
...
\noindent \textbf{Case of \textsl{FIT[2]} procedure~:} \index{FAISTORE!- in presence of \textsl{FIT[2]}}
it may not be desired to store
during the FITting process, but instead only when the FITtinig is completed. It is sufficient
for that to  (i)~put 'AtFITfinal' as the first label following   \textsl{FAISTORE} keyword, this will inhibit all storage until the
final run following a FIT procedure, and (ii)~avoid using the 'nofinal' instruction in \textsl{FIT[2]} (see p.~\pageref{nofinal})).
...
\index{MATRIX!- transport coefficients} \index{MATRIX!- beam matrix}
\medskip
...
\noindent PRINT~: 
  Addition of ``\textsl{PRINT}''\index{PRINT!- MATRIX} following \textsl{IORD, IFOC [, coupled]} will cause 
...
\noindent \textsl{PRINT}~: 
If  ``\textsl{PRINT}''\index{PRINT!- SPNPRT} appears after \textsl{SPNPRT}, then spin data will  be stored in 
zgoubi.SPNPRT.Out. The latter is opened at the first occurence of ``\textsl{PRINT}''. In particular, if \textsl{REBELOTE} is used then data will be stacked in zgoubi.SPNPRT.Out. 


\bigskip

\noindent \textsl{MATRIX}~: 
If  ``\textsl{MATRIX}''\index{MATRIX!- spin rotation matrix} appears after \textsl{SPNPRT}, then spin rotation matrix (or matrices) will be 
computed. This is done assuming appropriate initial particle and spin sampling has been defined 
(by respectively \textsl{OBJET} and \textsl{SPNTRK}), as follows~: 

\medskip

- three particles are needed for \textsl{SPNPRT} to compute a spin matrix. 
All three have identical initial coordinates (for instance, the local 
stable precession axis coordinates) and rigidity, and their spins are on a direct triedra, for instance, respectively, \\
\centerline{$(S_X, S_Y, S_Z) = (1,0,0),~ (0,1,0),~ (0,0,1),~ $}

\medskip 

 - there may be several groups of three particles, each group with a particular rigidity (\ie, the three 
particles in a group have the same momentum). \text{OBJET} with \textsl{KOBJ=1} or \textsl{KOBJ=2} can be used for 
instance to generate these particles by groups of like rigidity. \textsl{SPNPRT} will thus compute 
as many matrices.
...
\subsubsection{SRPRNT~: \SRPRNTTitl}\label{SRPRNT}\index{SRPRNT|textbf}\index{synchrotron radiation energy loss}
\medskip 

\noindent\textsl{SRPRNT} may be introduced anywhere in a structure. It allows switching on 
synchrotron radiation energy loss computation. It produces in addition 
...
\subsubsection{TWISS~: \TWISSTitl}\label{TWISS} \index{TWISS|textbf} \index{MATRIX!- periodic beam matrix}
\medskip

\textsl{TWISS}   causes the calculation of  transport coefficients and various other global parameters, in particular 
...
because  \textsl{TWISS} does not look for the closed orbit by itself. \index{closed orbit} 
A simple way to first find the orbit is to have \textsl{TWISS} preceded by a dedicated \textsl{FIT[2]}, 
see example in PART~B, page~\pageref{TWISS-B}.  \index{TWISS!- find orbit}
...
\noindent The execution of  \textsl{TWISS} will cause printout to the file
zgoubi.TWISS.out\index{zgoubi.TWISS.out}, a MADX type ``twiss'' file, 
...
\noindent The program \texttt{beamat} ensures the transport of the beam matrix along the 
zgoubi.dat sequence, at the orgin of the content of zgoubi.TWISS.out. \texttt{beamat}  is called by \texttt{opticc}, 
 itself called after each optical element, in the  program \texttt{zgoubi}. 
...
\index{BORO@{\BORO}$\times D_{ref}$|textbf} \index{BORORef@$B\rho_{ref}$}
$$B\rho_{ref} = \BORO \longrightarrow B\rho_{ref} = \BORO + \delta \Br_s$$ 
\noindent with  $\delta \Br_s$ the synchronous rigidity increase (or decrease). A typical configuration where this would 
...
- $ \IL=1 $ causes a print  in zgoubi.res \index{zgoubi.res} of particle 
...
into the dedicated file  zgoubi.plt\index{zgoubi.plt}, 
one line of data per article, at each integration step. This information  can further be processed 
(using \textbf{zpop}\footnote{See Part D of the Guide.}\index{zpop} or gnuplot, for instance). In order to minimize  the volume of 
...
- $ \IL=7 $ causes similar outputs to  $ \IL=1 $, and more, in particular derivatives to higher order, yet 
 into the dedicated file  zgoubi.impdev.out\index{zgoubi.impdev.out|textbf}, 
...
- $ \IC=1 $ causes a print of the field map in zgoubi.res. \index{zgoubi.res}

\smallskip 

- $ \IC=2 $ will cause a print   of the field map into the file  zgoubi.map \index{zgoubi.map}
...
\noindent \textsl{KPOS} $=3 $ option is available for some optical elements
  (\emph{e.g.}, \textsl{BEND}, \textsl{MULTIPOL}, \textsl{AGSMM}\index{AGSMM})~; 
...
\noindent Both incoming and outgoing reference frames are  tilted w.r.t. the magnet,  
 ...
 $\bullet$ or, if ALE=0  by  half the \textsl{Z}-rotation $\alpha_Z/2$ such that $ L = 
2 \dfrac{\BORO}{B1} \sin({\alpha_Z}/{2})$,  wherein $L$ = geometrical length, 
...
\noindent Next, the optical element is  Y-shifted by $\delta Y= \YCE$ (\textsl{XCE} is not used) in a 
...
Case of Cartesian frame optical element. 
  Left~: positioning an optical element using  \textsl{KPOS}$=1 $. 
...
\begin{figure}[h]
\begin{center}
\includegraphics[height=7cm]{FigKPOS3.eps}

\hangcaption{\label{figKPOS3} 
Alignment in \textsl{BEND} by half the deviation, $\alpha_Z/2$, using  \textsl{KPOS}$=3 $.  
\textsl{$(X_E,Y_E)$} and \textsl{$(X_S, Y_S)$} are respectively the incoming 
 and outgoing  reference frames.
Trajectory coordinates inside the magnet are defined in the magnet frame, \textsl{$(X_{ME},Y_{ME})$}. 
}
%\end{center}
%\end{minipage}
%\end{center}
...
\noindent \textsl{KPOS} $=4 $ applies to 
\textsl{AGSMM} (AGS main magnet) and \textsl{MULTIPOL}. \index{AGSMM} 
It orients the magnet in a way similar 
to  \textsl{KPOS} $=3 $, with reference frame Z-rotated by $\alpha_Z/2$ as drawn from $ L = 
2 \dfrac{\BORO}{B1} \sin({\alpha_Z}/{2})$. 

\noindent However additional magnet mis-alignments (alignment errors) are available (Fig.~\ref{figYZmisal})~:
 Z-rotation $\theta_Z$, 
 Z-shift  $\delta Z$, 
 Y-rotation $\theta_Y$. Rotations apply at the center of the magnet. 
...
\includegraphics[height=7cm]{FigKPOS4A.eps}

\includegraphics[height=4.5cm]{FigKPOS4B.eps}

\hangcaption{\label{figYZmisal}
Additional magnet mis-alignement with \textsl{KPOS=4}.
}
