...
$>1$ &&        15    &         .1$-$.4        &  \multirow{2}{80mm}{3-D map data file(s) (up to 4), whole magnet volume each (thus no symmetry assumed). Fields summed up~: at all node, $\vec B = \sum_{i=1}^{i=MOD2} a_i \, \vec B_i $. Each map file contains $B_{X,Y,Z}(X,Y,Z)$ data over  \IZ\ equally $Z$-spaced $(X,Y)$ planes}  \\[14ex]
