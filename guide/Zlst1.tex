...
\noindent \textbf{If $\mathbf{IOPT  =  1}$~:} the precession is defined by its axis, at angle  $\mu$ with 
respect to \zgou's X-axis and assumed in the horizontal plane,  and by its value, $\phi$. 

\bigskip

\noindent \textbf{If $\mathbf{IOPT  =  2}$~:}  given the spin precession axis direction $\mu$ in the (X,Y) plane, 
   the spin precession angle follows a function of the \textsl{reference} Lorentz factor~: 
  $$\phi (\gamma) = \left(\frac{B}{B_0}\right)^2 \times \left( C_0 + \frac{C_1}{\gamma} + \frac{C_2}{\gamma^2}  + \frac{C_3}{\gamma^3} \right) $$
