...
C---------------------------------------------------------
      ...
      INCLUDE "C.RIGID.H"     ! COMMON/RIGID/ BORO,DPREF,DPPP,QBR,BRI
...
C-----  a list of NBMAG.LE.NM magnets
 ...
 227    CE(KMAG,I) = A(NOEL,NP)
      ...
      IF(NRES.GT.0) THEN
        KGAP = 2
        IF(QSIE(KMAG).EQ.0) KGAP=1
        WRITE(NRES,106) 'Entrance',LAMBDE(KMAG),TYPGAP(KGAP)
106     FORMAT (/,5X,A8,'  EFB',/,10X,
     >  'Fringe  field  :  gap at R0 is',F7.2,' cm,    type is : ',A)
...
C Exit Fringe Field
 ...
 228    CS(KMAG,I) = A(NOEL,NP)
      ...
      IF(NRES.GT.0) THEN
        KGAP = 2
        IF(QSIS(KMAG).EQ.0) KGAP=1
        WRITE(NRES,106) 'Exit',LAMBDS(KMAG),TYPGAP(KGAP)
 ...
 229    CC(KMAG,I) = A(NOEL,49+I)
      ...
      SHIFT3(KMAG) = A(NOEL,NP)
       ...
       NBFACE(KMAG)=2
       IF(QSIL(KMAG) .GE. 0) NBFACE(KMAG)=3

       IF(NRES.GT.0) THEN
         IF(QSIL(KMAG).LT.0) THEN
            WRITE(NRES,*)
...
113        FORMAT(20X,' EFB is centred on direction ACENT+OMEGA,  at ',
     >     G12.4,' cm'/)
           WRITE(NRES,103)UMEGA3(KMAG),THETA3(KMAG),R1L(KMAG),U13(KMAG),
     >        U23(KMAG),R2L(KMAG)
           IF(R1L(KMAG)*R2L(KMAG) .EQ. 0.D0) WRITE(NRES,123)
         ENDIF
       ENDIF
...
139       FORMAT(/,20X,' FACE 3, ATTENTION : ABS(UT3) > 90 DEG. =>',
     >    ' VALEUR AMBIGUE POUR LE CALCUL DE LA CARTE DE Champ')
        ...
        XC3 = U13(KMAG) * COSO3 + XB3
        YC3 = YB3 + U13(KMAG) * SINO3
        XD3 = XC3 + R1L(KMAG) * SINO3
        YD3 = YC3 - R1L(KMAG) * COSO3
        XE3 = XA3 + R2L(KMAG) * SINO3
        YE3 = YA3 - R2L(KMAG) * COSO3
        SIN23 = SINO3**2
        COS23 = COSO3**2
        SICO3 = SINO3 * COSO3
      ENDIF
...
C           ERREUR  DE  DONNEES  FACE  ENTREE
 ...
 104        FORMAT(/,5X,10('*'),' ERREUR PARAMETRES FACE ENTREE',/)
          ...
          IF(LAMBDE(KMAG) .EQ. 0.D0) THEN
            ...
            ENDIF
          ELSE
            IF(QSIE(KMAG).EQ.0) THEN
              GAP = LAMBDE(KMAG)
            ELSE
              GAP = LAMBDE(KMAG)* (RM/ROJ)**QSIE(KMAG)
            ENDIF
            D = D/GAP
...
114         FORMAT(/,5X,10('*'),' ERREUR PARAMETRES FACE SORTIE',/)
          ...
          IF(LAMBDS(KMAG) .EQ. 0.D0) THEN
            ...
            ENDIF
          ELSE
            IF(QSIS(KMAG).EQ.0) THEN
              GAP = LAMBDS(KMAG)
            ELSE
              GAP = LAMBDS(KMAG)* (RM/ROJ)**QSIS(KMAG)
            ENDIF
            D = D/GAP
...
134             FORMAT(/,5X,7G12.4)
            ...
            IF(LAMBD3(KMAG) .EQ. 0.D0) THEN
              ...
              ENDIF
            ELSE
              IF(QSIL(KMAG).EQ.0) THEN
                GAP = LAMBD3(KMAG)
              ELSE
                GAP = LAMBD3(KMAG)* (RM/ROJ)**QSIL(KMAG)
              ENDIF
C              GAP = LAMBD3(KMAG) * (RM/ROJ)**COEFN(KMAG)
              D = D/GAP
