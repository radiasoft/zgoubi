C  D L A I N / ALGEBRE LINEAIRE RUBRIQUE L,L301A          MINI BBL MATH         
C  FORTRAN                                                                      
      SUBROUTINE DLAIN ( ID,NZ,Z,N0,B,IER )                                     
C         ----- INVERSION D'UNE MATRICE REELLE Z D'ORDRE NZ -----               
      DOUBLE PRECISION AZ,B,B1,EPS,Z                                            
...
C                                                                               
C  LES ELEMENTS DE L'INVERSE INFERIEURS A 1.D-32 SONT ANNULES.                  
C  LES CALCULS SUR Z SONT FAITS AVEC 2 INDICES                                  
C  SOUS-PROGRAMME APPELE : DABS                                                 
C  ----------------------------------------- D L A I N -----------------        
...
C                                                                               
   ...
   90 B(J)=Z(1,J+1)/Z(1,1)                                                      
      ...
      IF ( DABS( Z(I,J) ).LT.EPS ) Z(I,J) = 0.D0                                
  100 CONTINUE                                                                  
      Z(I,N) = -Z(IP1,1) * B(N)                                                 
      IF ( DABS( Z(I,N) ).LT.EPS ) Z(I,N) = 0.D0                                
  110 CONTINUE                                                                  
C                                                                               
      DO 120 J=1,N                                                              
  120 Z(N,J)=B(J)                                                               
...
C                                                                               
  150 PRINT 1001                                                                
 1001 FORMAT (//' DLAIN * MATRICE SINGULIERE ****')                             
      IF ( IER.NE.-1 ) STOP                                                     
      IER = 1                                                                   
  160 RETURN                                                                    
C                                                                               
  170 IF(Z(1,1).EQ.0.D0) GO TO 150                                              
      Z(1,1)=1.D0/Z(1,1)                                                        
      RETURN                                                                    
      END                                                                       
