...
C CORRECTION FOR B-DOT EFFECTS IS ADDED IN HERE.  K.BROWN (8/31/98)
      ...
      IF    (LBL1(LAST-1:LAST) .EQ. 'BF') THEN
        ...
        ELSE
          ...
          PNOW(1) = P
C FM. "DB1F = 0.D0" has to be placed here in order to be actualized for each magnet
          DB1F = 0.D0
          IF( P .LT. AP(1) ) THEN
            DB1F = ADBF(1)
...
C ZgoubiFromSnaprampCmd has "DB1F = 0.D0" 
          DB1F = 0.D0
          P2 = P*P
          P3 = P*P2
          P4 = P*P3
...
C  K2BF :
     ...
     >    +TKBF6*P6 - TKFBDCOEF*BDOT*ALB/ALA
      ...
      ELSEIF(LBL1(LAST-1:LAST) .EQ. 'CD') THEN
        ...
        ELSE
          PNOW(2) = P
C FM. "DB1D = 0.D0" has to be placed here in order to be actualized for each magnet
          DB1D = 0.D0
          IF( P .LT. AP(1) ) THEN
            DB1D = ADBD(1)
          ELSEIF( P .GT. AP(ISCLMAX)) THEN 
...
C ZgoubiFromSnaprampCmd has "DB1D = 0.D0" 
          DB1D = 0.D0
          P2 = P*P
          P3 = P*P2
          P4 = P*P3
