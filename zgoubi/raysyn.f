...
C Called by SRPRNT. Can be anytime along the .dat sequence
 ...
 55   CONTINUE
      IF(NRES.GT.0) THEN
C        PP = BORO*CL*1.D-9*Q/QEL
C        PP = BORO*CL*1.D-9*Q
        PP = BORO*CL9*Q
        EE = SQRT(PP*PP+AMASS*AMASS)
        WRITE(NRES,FMT='(/,3X,
...
C     >  (TSRLT-TTLOS2)/DBLE(IMAX) *1.D3
     ...
     >      SQRT(TL2/TTPHOT-(TSRLT/TTPHOT)**2) *1.D3
        WRITE(NRES,FMT='(5X,'' Smallest, BIGEST photon :'',1P,
