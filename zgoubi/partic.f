...
C        ENDIF
     ...
     >  PRTCL(DEBSTR(PRTCL):FINSTR(PRTCL))
        IF(AMASS.NE.0.D0) THEN
          IF(NM .EQ. 1) THEN
            WRITE(NRES,100) ' Mass          = ',AMASS, '  MeV/c2'
          ELSEIF(NM .EQ. 2) THEN
            WRITE(NRES,100)'Type  1  mass      = ',AMASS, '  MeV/c2'
 ...
 100    FORMAT(T20,A18,1P,G14.6,A10)
        IF(AMASS .NE. 0.D0) THEN
          IF(Q .EQ. 0.D0) Q = 1
          PREF = BORO*CL9*Q
          ENRG = SQRT(PREF*PREF+AMASS*AMASS)
          BTA = PREF / ENRG
          GAM =  ENRG / AMASS
          WRITE(NRES,FMT='(/,1P,
     >    T15,''Reference  data :'',
     ...
     >       )') BORO,AMASS,PREF, ENRG, ENRG-AMASS, BTA, GAM, BTA*GAM
