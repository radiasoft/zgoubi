...
C      VLT= A(NOEL,20)
      ...
      IF(Q*AMASS .EQ. 0.D0) CALL ENDJOB('Pgm cavite. Give mass & charge'
...
C Orbit length between 2 cavities, RF freq., phase of 1st cavity (ph0=0 is at V(t)=0)
     ...
     >           DUM,SCUM,TCUM)
      ...
      HARM = 1.D0
      OMRF = 2.D0 * PI * FCAV
      WS0 = SQRT(PS**2 +AMASS**2) - AMASS

      DWS = QV * COS(PH0)
      WSF = WS0 +  DWS
      PSF = SQRT((WSF+AMASS)**2 - AM2)
