...
#    5. Reports whether any such array appears in a COMMON block inside the ../include directory.
#    6. Reports whether any such array appears in an EQUIVALENCE statement inside the present working directory.
...
grep "$dimension" -i *.f | grep DIMENSION | sed '/.f:C/d' | 
{
  ...
  for dynamic_array in "${dynamic_array_list[@]}" ; do
    ...
    if [[ ! -z "$dynamic_array_in_common" ]]; then
      export arrays_in_common+=("$dynamic_array")
    fi
  done
  if [[ -z "${arrays_in_common:-}" ]]; then
    arrays_in_common+=(" (none) ")
  fi
  echo "arrays_in_common=${arrays_in_common[@]}"

  for dynamic_array in "${dynamic_array_list[@]}" ; do
    dynamic_array_in_equivalence=$(grep EQUIVALENCE -i *.f | grep "${dynamic_array}") || true
    if [[ ! -z "$dynamic_array_in_equivalence" ]]; then
      export arrays_in_equivalence+=("$dynamic_array")
    fi
  done
  if [[ -z "${arrays_in_equivalence:-}" ]]; then
    arrays_in_equivalence+=(" (none) ")
  fi
  echo "arrays_in_equivalence=${arrays_in_equivalence[@]}"
}
