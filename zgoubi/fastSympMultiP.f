...
C  -------
      SUBROUTINE fastSympMultiP(NRES, newIntegQ)
      IMPLICIT NONE
C---------------------------------------------------------
C     (1) Optical elements defined in cartesian frame
C     (2) Fast drift-kick-drift symplectic particle motion integrator
C         for a general magnetic multipole
C---------------------------------------------------------
      ...
      INTEGER KUASEX, NRES, newIntegQ, NSTEP, J, IT, NCE, NCS
...
C-------- Get the parameters for the magnetic multipole----------------
      ...
      IF(NRES.GT.0) THEN
        WRITE(NRES, 100) XL, RO, Bfield(1), Bfield(2), Bfield(3),
...
C---------Option 1: Drift-kick-drift motion integrator------------------
        CALL DKD_MULTIP(NSTEP, step, b0g0, eb, mass, IMAX, F, l0, w0)
      ELSE
